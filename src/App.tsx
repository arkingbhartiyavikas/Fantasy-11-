...
export const syncWalletToBackend = async (
  dbInstance: any,
  userId: string,
  data: any,
) => {
...
const AdminMatchEditCard: React.FC<{
  match: Match;
  onUpdate: (m: Match) => void;
  onDelete: () => void;
  onStatusChange: (status: "Upcoming" | "Live" | "Completed") => void;
  onLineupToggle: () => void;
}> = ({ match, onUpdate, onDelete, onStatusChange, onLineupToggle }) => {
  ...
  const handleUpdate = () => {
    ...
    if (editDate === formatDateISO(today)) dayStr = "Today";
      ...
      dayStr = new Date(editDate).toLocaleDateString("en-US", {
        weekday: "long",
      });
    ...
    <div className="bg-black/60 border border-slate-700 hover:border-[#e5c158]/30 rounded-xl p-4 flex flex-col gap-3 transition-colors">
      <div className="flex justify-between items-center border-b border-[#e5c158]/20 pb-3">
        <div className="font-bold text-slate-200 text-sm">
          {match?.team1?.shortFrame} vs {match?.team2?.shortFrame}
        </div>
        <div
          className={`text-[10px] font-black tracking-widest uppercase px-2 py-1 rounded-sm border ${match.status === "Upcoming" ? "bg-blue-900/30 text-blue-400 border-blue-500/30" : match.status === "Live" ? "bg-red-900/40 text-red-400 border-red-500/50 shadow-[0_0_8px_rgba(239,68,68,0.4)] animate-pulse" : "bg-green-900/30 text-green-400 border-green-500/30"}`}
        >
          {match.status}
        </div>
      </div>
      ...
      <div className="flex gap-2 items-end mt-1">
        <div className="flex-1">
          <label className="text-[10px] text-[#e5c158]/70 uppercase font-black tracking-widest pl-0.5">
            Match Date
          </label>
          <select
            value={editDate}
          ...
          >
            <option value={formatDateISO(today)}>
              Today ({formatDateISO(today)})
            </option>
            <option value={formatDateISO(tomorrow)}>
              Tomorrow ({formatDateISO(tomorrow)})
            </option>
            <option value={formatDateISO(dayAfter)}>
              Day After ({formatDateISO(dayAfter)})
            </option>
          </select>
        </div>
      ...
      <div className="flex gap-2 text-xs font-bold mt-2">
        <button
          ...
          className={`flex-1 py-2 rounded-lg border transition-colors ${match.status === "Upcoming" ? "bg-blue-900/30 text-blue-400 border-blue-500/50" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
        >
          Upcoming
        </button>
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg border transition-colors ${match.status === "Live" ? "bg-red-900/40 text-red-400 border-red-500/50 shadow-[0_0_8px_rgba(239,68,68,0.2)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
        >
          Go Live
        </button>
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg border transition-colors ${match.status === "Completed" ? "bg-green-900/40 text-green-400 border-green-500/50 shadow-[0_0_8px_rgba(34,197,94,0.2)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
        >
          Complete
        </button>
      </div>
      <div className="mt-2 text-center">
        <button
          onClick={onDelete}
          className="text-red-500/80 hover:text-red-400 font-bold text-[10px] uppercase tracking-widest w-full py-1.5 transition-colors"
        >
          Delete Match
        </button>
      </div>
      <div className="mt-1 border-t border-slate-700 pt-3 flex gap-2 text-xs font-bold">
        ...
        >
          {match.lineupStatus === "OUT"
            ? "Lineups OUT (Click to make Red)"
            : "Lineups NOT OUT (Click to make Green)"}
        </button>
      </div>
    </div>
  );
...
const ContestDetailsView = ({
  ...
  onSetLivePayouts?: (
    payouts: { rank: string; amount: string | number }[],
    rankedTeams: any[],
  ) => void;
}) => {
  const [activeTab, setActiveTab] = useState<"WINNINGS" | "LEADERBOARD">(
    "WINNINGS",
  );
  const [showLivePayouts, setShowLivePayouts] = useState<boolean>(false);
  const [liveCustomPayouts, setLiveCustomPayouts] = useState<
    { rankFrom: string; rankTo: string; amount: string }[]
  >([]);

  const contestTeams = useMemo(() => {
    let filtered = savedTeams.filter(
      (t) =>
        t.match?.id === activeMatch?.id &&
        (t.contestId
          ? t.contestId === contest.id
          : t.contestName === contest.name),
    );
    if (instanceId != null) {
      filtered = filtered.filter((t) =>
        typeof t.instanceId === "number" ? t.instanceId === instanceId : true,
      );
    }
    return filtered;
  ...
  const sortedTeams = useMemo(() => {
    ...
    const teamsWithPoints = contestTeams.map((t) => {
      ...
      const botKey =
        t.userId === "admin_bot" || t.userId === "admin_bot_boot"
          ? `${t.match?.id}_${t.botVariationId || "old"}_${t.isWinnerBot ? "win" : "norm"}`
          : null;

      if (botKey && memoizedBotPoints[botKey] !== undefined) {
        computedPoints = memoizedBotPoints[botKey];
      } else {
        computedPoints = (t.players || []).reduce(
          (acc: number, player: Player) => {
            const livePlayer =
              appPlayers.find((p) => p.id === player.id) || player;
            let mult = 1;
    ...
    const currentUserTeams = rankedTeams.filter(
      (t) => currentUser && t.userId === currentUser.id,
    );
    const otherTeams = rankedTeams.filter(
      (t) => !(currentUser && t.userId === currentUser.id),
    );
    ...
    <div className="flex flex-col h-full bg-app-bg">
      <header className="bg-app-bg text-app-text border-b border-app-border p-4 shadow-md z-10">
        <div className="flex items-center justify-between mb-4">
          <button
            onClick={onBack}
            className="p-1 -ml-1 active:bg-app-card/10 rounded-full transition-colors"
          >
            <ArrowLeft size={24} />
          </button>
          ...
          <button
            onClick={onAddCash}
            className="flex items-center gap-1 bg-app-card/20 px-2 py-0.5 rounded text-xs"
          >
            <Wallet size={12} /> ₹
      ...
      {(isAdmin || activeMatch?.status === "Upcoming") && (
        <div className="bg-app-card p-4 shadow-sm z-10 w-full relative">
          <div className="flex justify-between items-start mb-2">
            <div>
              <p className="text-xs text-app-text-muted font-semibold uppercase">
                Prize Pool
              </p>
              <p className="text-xl font-black text-app-text">
                {currentCollected > 0 && contest.type === "Mega"
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-app-text">
                {Math.max(0, contest.spots - contestTeams.length)}
              </p>
            </div>
          </div>
          <div className="bg-app-bg h-1.5 rounded-full mb-2 overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, (contestTeams.length / contest.spots) * 100)}%`,
              }}
            ></div>
          </div>
          ...
          {isAdmin && activeMatch?.status !== "Upcoming" ? (
            <button
              onClick={() => {
                setLiveCustomPayouts(
                  payouts.map((p) => {
                    const rankRanges = p.rank
                      .toString()
                      .replace("#", "")
                      .trim()
                      .split("-");
                    return {
                      ...
                      rankTo:
                        rankRanges[1]?.trim() || rankRanges[0]?.trim() || "",
      ...
      <div className="flex bg-[#ffe2e2] text-[#c94b4b] mt-2 shadow-sm border-b border-[#ffe2e2]/50">
        <button
          ...
          className={`flex-1 py-3 text-sm font-bold text-center border-b-[3px] transition-colors ${activeTab === "WINNINGS" ? "border-[#c94b4b] text-[#c94b4b]" : "border-transparent text-[#c94b4b]/60"}`}
        >
          Winning Breakup
        </button>
        <button
          onClick={() => setActiveTab("LEADERBOARD")}
          className={`flex-1 py-3 text-sm font-bold text-center border-b-[3px] transition-colors ${activeTab === "LEADERBOARD" ? "border-[#c94b4b] text-[#c94b4b]" : "border-transparent text-[#c94b4b]/60"}`}
        >
          Leaderboard
        </button>
      </div>
        ...
        {activeTab === "WINNINGS" ? (
            ...
            {(!contest.payouts || contest.payouts.length === 0) &&
              contest.type === "Mega" && (
                <div className="bg-blue-50 border border-blue-100 text-blue-800 text-xs p-3 rounded-lg text-center font-medium shadow-sm">
                  <b>Dynamic Prize Pool:</b> Users receive {winningPercentage}%
                  of total entry fees collected. The remaining{" "}
                  {100 - winningPercentage}% is kept as platform fee. As more
                  teams join, the winning amounts automatically increase!
                </div>
              )}
            <div className="bg-app-card rounded-lg border border-app-border overflow-hidden">
              ...
              {payouts.length > 0 ? (
                payouts.map((p, i) => (
                  <div
                    key={i}
                    className="flex justify-between px-4 py-3 border-b border-app-border last:border-0 items-center"
                  >
                    <span className="font-bold text-app-text text-sm">
                      {typeof p.rank === "string" && p.rank.startsWith("#")
                        ? ""
                        : "#"}
                      {p.rank}
                    </span>
          ...
          <div className="flex flex-col bg-white dark:bg-app-bg">
            ...
            <div className="flex justify-between items-center px-4 py-2 text-xs font-bold text-[#b8a8a8] bg-[#ffeaea] dark:bg-app-card-inner border-b border-[#ffe2e2] dark:border-app-border">
              <span className="flex-1 text-[13px] font-medium text-[#a09090] dark:text-app-text-muted">
                All Teams({sortedTeams.length})
              </span>
              <div className="flex gap-4 w-32 justify-end shrink-0">
                <span className="w-14 text-center text-[13px] font-medium text-[#a09090] dark:text-app-text-muted">
                  Points
                </span>
                <span className="w-10 text-right text-[13px] font-medium text-[#a09090] dark:text-app-text-muted">
                  #Rank
                </span>
              </div>
            </div>
              ...
              {sortedTeams.length > 0 ? (
                <>
                  {sortedTeams.slice(0, 100).map((t, i) => {
                    const isCurrentUser =
                      currentUser && t.userId === currentUser.id;
                    return (
                      <div
                        key={i}
                        onClick={() => {
                        ...
                        className={`flex justify-between px-3 py-2.5 border-b border-gray-200 dark:border-app-border items-center z-10 transition-colors ${
                          !isCurrentUser && activeMatch?.status === "Upcoming"
                            ? "cursor-default"
                            : "cursor-pointer hover:bg-gray-50 dark:hover:bg-app-card-hover/50"
                        } ${
                          isCurrentUser
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 overflow-hidden">
                          <div className="w-8 h-8 rounded-sm bg-[#faeee8] border border-[#ee5723]/30 dark:bg-slate-700 font-bold text-[10px] text-app-text flex items-center justify-center shrink-0 uppercase overflow-hidden">
                          ...
                          <div className="flex flex-col overflow-hidden w-full">
                            <span className="font-semibold text-black dark:text-app-text text-[13px] whitespace-nowrap overflow-hidden text-ellipsis max-w-[130px]">
                              {t.userName || t.userId || "Guest Player"}
                            </span>
                            {t.amountWon > 0 && (
                              <span className="text-[9px] text-green-600 dark:text-green-500 font-bold tracking-tight mt-0.5">
                                WON ₹
                                {Number(t.amountWon)
                                  .toFixed(2)
                                  .replace(/\.00$/, "")}
                        ...
                        <div className="flex gap-4 w-32 justify-end items-center shrink-0">
                          <span className="w-14 text-center text-[13px] text-[#7a7a7a] font-semibold">
                            {Number(t.points) % 1 !== 0
                              ? Number(t.points).toFixed(1)
                              : t.points}
                          </span>
                          <span className="w-10 text-right font-bold text-black dark:text-app-text text-[13px]">
                            #{t.rank}{" "}
                  ...
                  {sortedTeams.length > 100 && (
                    <div className="text-center py-6 text-app-text-muted text-xs font-bold border-t border-app-border z-10 relative">
                      Showing top 100 of{" "}
                      {sortedTeams.length.toLocaleString("en-IN")} teams
                    </div>
                  )}
                </>
              ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1a1c24] w-full max-w-md rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-4 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-black/40">
              ...
              <button
                onClick={() => setShowLivePayouts(false)}
              ...
              {liveCustomPayouts.map((cp, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                  ...
                  <button
                    onClick={() => {
                      setLiveCustomPayouts(
                        liveCustomPayouts.filter((_, i) => i !== idx),
                      );
                    }}
            ...
            <div className="p-4 border-t border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-black/40">
              <button
                onClick={() => {
                  if (onSetLivePayouts) {
                    const formatted = liveCustomPayouts
                      .filter((c) => c.rankFrom && c.amount)
                      .map((c) => {
                        let rank =
                          c.rankFrom === c.rankTo || !c.rankTo
                            ? `# ${c.rankFrom}`
                            : `# ${c.rankFrom} - ${c.rankTo}`;
...
export default function App() {
  ...
  const handleFsError = (e: any, operation?: string, path?: string) => {
    console.error(
      `Firestore Error [${operation || "unknown"} @ ${path || "unknown"}]:`,
      e,
    );
    if (e?.message?.includes("offline")) {
      console.warn(
        "Notice: Firestore is currently offline or unreachable. Working from local cache if possible.",
      );
    }
  };
  ...
  const [themeMode, setThemeMode] = useState<"Dark" | "Light">(
    () => (localStorage.getItem("dreamApp_themeMode") as any) || "Dark",
  );
  const [themeColor, setThemeColor] = useState<"Red" | "Blue" | "Green">(
    () => (localStorage.getItem("dreamApp_themeColor") as any) || "Blue",
  );
  ...
  const [activeContestDetails, setActiveContestDetails] =
    useState<Contest | null>(null);
  const [activeContestInstanceId, setActiveContestInstanceId] = useState<
    number | null
  >(null);
  ...
  const [authMode, setAuthMode] = useState<"LOGIN" | "SIGNUP" | "OTP">(
    typeof window !== "undefined" &&
      localStorage.getItem("dreamApp_hasSignedUp") === "true"
      ? "LOGIN"
      : "SIGNUP",
  );
  ...
  const [oneClickCreds, setOneClickCreds] = useState<{
    userId: string;
    pass: string;
  } | null>(null);
  ...
  const [showPhonePermissionDialog, setShowPhonePermissionDialog] =
    useState(false);
  const [phonePermissionGranted, setPhonePermissionGranted] = useState(
    () => localStorage.getItem("dreamApp_phonePermission") === "true",
  );
  ...
  const [claimedLevelsLoaded, setClaimedLevelsLoaded] = useState<string | null>(
    null,
  );
    ...
    if (user?.id) {
      if (claimedLevelsLoaded !== user.id) {
        ...
        localStorage.setItem(
          `dreamApp_claimedLevels_${user.id}`,
  ...
  useEffect(() => {
    ...
    if (hash && hash.includes("error_description=")) {
      ...
      if (errorDesc) {
        alert(
          "Login failed: " + decodeURIComponent(errorDesc).replace(/\+/g, " "),
        );
      ...
      if (fbUser) {
        ...
        try {
          ...
          if (
            !legacyMobile &&
            fbUser.email &&
            fbUser.email.endsWith("@dreamapp.com")
          ) {
          ...
          if (!isExisting) {
            ...
            let numericId = Math.floor(
              1000000000 + Math.random() * 9000000000,
            ).toString(); // 10 digits
            if (
              fbUser.email &&
              (fbUser.email.endsWith("@fantasy11.local") ||
                fbUser.email.endsWith("@dreamapp.com"))
            ) {
              const extracted = fbUser.email.split("@")[0];
            ...
            let randomPass = "";
            randomPass += upperChars.charAt(
              Math.floor(Math.random() * upperChars.length),
            );
            randomPass += lowerChars.charAt(
              Math.floor(Math.random() * lowerChars.length),
            );
            randomPass += numChars.charAt(
              Math.floor(Math.random() * numChars.length),
            );
            const allChars = upperChars + lowerChars + numChars;
            for (let i = 0; i < 7; i++)
              randomPass += allChars.charAt(
            ...
            localStorage.setItem(
              `dreamApp_numericId_${existingDocId}`,
              numericId,
            );
            localStorage.setItem("dreamApp_hasSignedUp", "true");
          ...
          let numericId =
            localStorage.getItem(`dreamApp_numericId_${uid}`) || "";
          const newUser = {
            email: fbUser.email || "",
  ...
  const [wallet, setWallet] = useState<{
    ...
    profits?: number;
    wins?: number;
  }>({ deposit: 0, winning: 0, bonus: 0, profits: 0, wins: 0 });
  ...
  useEffect(() => {
    ...
    const unsubDep = onSnapshot(
      ...
      (snap) => {
        if (isSubscribed)
          setDepositRequests(snap.docs.map((d) => d.data() as DepositRequest));
      },
    ...
    const unsubWd = onSnapshot(
      ...
      (snap) => {
        if (isSubscribed)
          setWithdrawRequests(
            snap.docs.map((d) => d.data() as WithdrawRequest),
          );
      },
    ...
    const unsubBank = onSnapshot(
      ...
      (snap) => {
        if (isSubscribed)
          setBankAccounts(snap.docs.map((d) => d.data() as BankAccount));
      },
    ...
    const unsubUserTeams = onSnapshot(
      teamsQuery,
      (snap) => {
      ...
      const fetchAdminData = async () => {
        try {
          ...
          wSnap.docs.forEach((d) => {
            walletDocs[d.id] = d.data();
          });
          uSnap.docs.forEach((d) => {
            metaDocs[d.id] = d.data();
          });
  ...
  useEffect(() => {
    ...
    if (
      !firestoreQuotaExceeded &&
      (view === "CONTEST_DETAILS" || view === "MATCH") &&
      activeMatch
    ) {
      const q = query(
        collection(db, "userTeams"),
        where("match.id", "==", activeMatch.id),
      );
      unsub = onSnapshot(
        q,
        (snap) => {
    ...
    setWallet((prev: any) => {
      ...
      setTimeout(() => {
        if (user?.id) {
          ...
          setDoc(doc(db, "wallets", user.id), next, { merge: true }).catch(
            (err) => {
  ...
  const balance =
    (wallet.deposit || 0) + (wallet.winning || 0) + (wallet.bonus || 0);

  const setBalance = (updater: number | ((prev: number) => number)) => {
    updateWallet((prev: any) => {
      const currentTotal =
        (prev.deposit || 0) + (prev.winning || 0) + (prev.bonus || 0);
      let newTotal =
        typeof updater === "function" ? updater(currentTotal) : updater;
      ...
      if (rem > 0 && bon > 0) {
        const d = Math.min(rem, bon);
        bon -= d;
        rem -= d;
      }
      let dep = prev.deposit || 0;
      if (rem > 0 && dep > 0) {
        const d = Math.min(rem, dep);
        dep -= d;
        rem -= d;
      }
      let win = prev.winning || 0;
      if (rem > 0 && win > 0) {
        const d = Math.min(rem, win);
        win -= d;
        rem -= d;
      }
  ...
  const renderWithdraw = () => {
    ...
    const userKycObj = kycRequests.find(
      (k) => k.userId === (user?.id || "guest"),
    );
        ...
        <div className="flex flex-col h-full bg-[#0B1221] text-white">
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center space-y-4">
            ...
            <p className="text-gray-400">
              You must complete your KYC verification before you can withdraw
              funds.
            </p>
            <button
              onClick={() => setView("KYC")}
              className="mt-6 bg-app-accent text-white px-8 py-3 rounded-full font-bold shadow-lg"
            >
              Complete KYC Now
            </button>
          </div>
    ...
    if (showAddBankMode) {
      return (
        <div className="flex flex-col h-full bg-[#0B1221] text-white">
          <header className="p-4 flex items-center gap-4">
            <button
              onClick={() => setShowAddBankMode(false)}
      ...
      <div className="flex flex-col h-full bg-[#0B1221] text-white">
        ...
        <div className="p-4 flex-1 overflow-y-auto pb-20">
          ...
          <div className="bg-[#152e23] border border-green-800/50 rounded-xl overflow-hidden mb-6">
            ...
            <div className="flex p-4 text-center divide-x divide-green-800/30">
              ...
              <div className="flex-1">
                <div className="text-2xl font-bold">
                  ₹
                  {Number(wallet.winning || 0).toLocaleString("en-IN", {
                    maximumFractionDigits: 0,
                  })}
                </div>
          ...
          <div className="grid grid-cols-4 gap-2 mb-6">
            {[110, 200, 500, 1000, 2000, 5000, 10000, 20000, 35000].map(
              (amt) => (
                <button
                  key={amt}
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {String(bank.accountNumber).substring(0, 4)}XXXXXXX (IFSC:{" "}
                      {bank.ifscCode})
                    </p>
                  </div>
          ...
          <div className="mt-6 mb-4 bg-[#e5c158]/10 border border-[#e5c158]/30 rounded-xl p-4 flex gap-3 text-gray-300 text-xs">
            ...
            <p>
              Maximum withdrawal time is{" "}
              <strong className="text-[#e5c158]">24 hours</strong>. If it is not
              credited within this time, please contact our Help Center.
            </p>
          </div>
          ...
          <button
            onClick={() => {
              ...
              const newWalletState = {
                ...wallet,
                winning: wallet.winning - withdrawAmount,
              };
              batch.set(
                doc(db, "wallets", user?.id || ""),
                { winning: newWalletState.winning },
                { merge: true },
              );
          ...
          <div className="mt-8 bg-[#e5c158]/10 border border-[#e5c158]/30 rounded-xl p-4 flex gap-3 text-app-text-muted text-xs">
            ...
            <p>
              Maximum withdrawal time is{" "}
              <strong className="text-[#e5c158]">24 hours</strong>. If your
              withdrawal is not processed within this time, please contact our
              Help Center.
            </p>
          </div>
        </div>
          ...
          for (let i = 0; i < t.bulkCount; i++) {
            expanded.push({
              ...
              id:
                ...
                i,
              teamId:
                t.userId === "admin_bot_boot"
                  ? `BOT-${Date.now().toString().slice(-4)}${i}`
                  : `T${Math.floor(Math.random() * 8999) + 1000}`,
              userName:
                t.userId === "admin_bot_boot"
                  ? `BOOT ${Date.now().toString().slice(-3)}${i}`
  ...
  const [appContests, setAppContests] = useState<Contest[]>(() => {
    ...
    try {
      const p = JSON.parse(saved);
      return Array.isArray(p) ? p : DEFAULT_CONTESTS;
    } catch (e) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedTeams.length > 2000)
        alert(
          "Too many BOTs! Device storage is full. Please remove some bots.",
        );
    }
  }, [savedTeams]);
  ...
  useEffect(() => {
    ...
    const unsubSyncMeta = onSnapshot(
      doc(db, "gameData", "sync_meta"),
      async (snap) => {
        ...
        const reloadBots = async () => {
          ...
          if (adminTeamsData) {
            setSavedTeams((prev) => {
              const userTeams = prev.filter(
                (t) =>
                  t.userId !== "admin_bot" && t.userId !== "admin_bot_boot",
              );
              const newTeams = [...userTeams, ...adminTeamsData];
              return Array.from(
                new Map(newTeams.map((item) => [item.id, item])).values(),
              );
            });
          }
        };
    ...
    const unsubPoints = onSnapshot(
      doc(db, "gameData", "live_player_points"),
      (snapshot) => {
        if (snapshot.exists()) {
            ...
            setAppPlayers((prev) => {
              ...
              const newPlayers = prev.map((p) => {
                ...
                if (ptsData !== undefined) {
                  const totalPts =
                    typeof ptsData === "number"
                      ? ptsData
                      : (ptsData.total ?? p.livePoints);
                  const newBreakup =
                    typeof ptsData === "object" ? ptsData.breakup : undefined;
                  if (
                    p.livePoints !== totalPts ||
                    JSON.stringify(p.breakup) !== JSON.stringify(newBreakup)
                  ) {
                    changed = true;
                    return {
                      ...p,
                      livePoints: totalPts,
                      breakup: newBreakup ?? p.breakup,
                    };
                  }
                }
                return p;
  ...
  const distributePrizes = async (matchId: string) => {
    ...
    setSavedTeams((currentTeams) => {
      ...
      const matchTeams = updatedTeams.filter(
        (t) => t.match?.id === matchId && !t.prizeDistributed,
      );
      if (matchTeams.length === 0) return currentTeams;

      const contestNames = Array.from(
        new Set(matchTeams.map((t) => t.contestName)),
      );
      ...
      contestNames.forEach((cName) => {
        ...
        const contest =
          appContests.find((c) => c.name === cName) || DEFAULT_CONTESTS[0];
          ...
          if (botKey && memoizedBotPoints[botKey] !== undefined) {
            ...
            computedPoints = (t.players || []).reduce(
              (acc: number, player: Player) => {
                ...
                return (
                  acc + (livePlayer.livePoints ?? livePlayer.points) * mult
                );
              },
              0,
            );
        ...
        if (
          contest.type === "Mega" &&
          contest.spots > 0 &&
          !contest.hasCustomPayouts
        ) {
          const scaleRatio = Math.min(
            1,
            Math.max(0, contestTeams.length / contest.spots),
          );
          payouts = payouts.map((p) => ({
            ...
            amount:
              (typeof p.amount === "number"
                ? p.amount
                : parseFloat(p.amount.toString().replace(/[^0-9.]/g, ""))) *
              scaleRatio,
          }));
        }
        ...
        if (payouts.length === 0 && contestTeams.length > 0) {
          if (contest.type !== "Mega") {
            payouts = [
              { rank: "1", amount: contest.firstPrize || contest.prizeText },
            ];
          } else {
            if (contestTeams.length === 1) {
              payouts.push({ rank: "1", amount: totalPrizePool });
        ...
        sortedTeams.forEach((t, i) => {
          ...
          if (payoutStr) {
            amt =
              typeof payoutStr.amount === "number"
                ? payoutStr.amount
                : parseFloat(
                    payoutStr.amount.toString().replace(/[^0-9.]/g, ""),
                  );
            if (amt) {
              if (t.userId === user?.id) {
          ...
          currentBatch.set(
            teamRef,
            { prizeDistributed: true, amountWon: amt, rank: rank },
            { merge: true },
          );
          batchCount++;
          commitBatchIfNeeded();
      ...
      if (batchChunks.length > 0) {
        Promise.all(batchChunks).catch((e) =>
          console.error("Error distributing prizes to DB:", e),
        );
      }
      ...
      if (localBalanceUpdate > 0 && user?.id) {
        updateWallet((prev: any) => ({
          ...prev,
          winning: prev.winning + localBalanceUpdate,
          profits: (prev.profits || 0) + localBalanceUpdate,
          wins: (prev.wins || 0) + localWinCount,
        }));
        setTimeout(() => {
          alert(
            `🎉 Match Completed! Prizes have been distributed.\n\nYou won a total of ₹${localBalanceUpdate.toFixed(2)} distributed to your wallet!\n${anyWonInfo}`,
          );
        }, 500);
      } else {
        setTimeout(() => {
  ...
  const [previewSource, setPreviewSource] = useState<
    "CREATE_TEAM" | "MY_MATCHES" | "CONTEST_DETAILS"
  >("CREATE_TEAM");
  const [previewTeamInfo, setPreviewTeamInfo] = useState<{
    name: string;
    points?: number;
  } | null>(null);
  const [showPlayerInfoPopup, setShowPlayerInfoPopup] = useState<Player | null>(
    null,
  );
  const [adminEditPlayerBreakup, setAdminEditPlayerBreakup] =
    useState<Player | null>(null);
  const [adminEditingEvent, setAdminEditingEvent] = useState<{
    event: string;
    actual: string;
    points: string;
  } | null>(null);
  ...
  const [adminContestType, setAdminContestType] = useState<
    "Mega" | "H2H" | "H2H_3" | "H2H_4" | "H2H_5"
  >("Mega");
  ...
  const [adminFirstPrizePercent, setAdminFirstPrizePercent] =
    useState<string>("15");
  const [adminContestAutoPayouts, setAdminContestAutoPayouts] =
    useState<boolean>(false);
  const [adminCustomPayouts, setAdminCustomPayouts] = useState<
    { rankFrom: string; rankTo: string; amount: string }[]
  >([{ rankFrom: "1", rankTo: "1", amount: "" }]);
  const [adminAutoFillRemaining, setAdminAutoFillRemaining] =
  ...
  const [adminMatchTimeValue, setAdminMatchTimeValue] =
    useState<string>("20:00"); // 8 PM default
  ...
  const [adminScoringLiveMatchId, setAdminScoringLiveMatchId] = useState<
    string | null
  >(null);
  const [playerScoringSearch, setPlayerScoringSearch] = useState<string>("");
  const [updateSuccessIds, setUpdateSuccessIds] = useState<
    Record<string, boolean>
  >({});
  const [playerAdjustments, setPlayerAdjustments] = useState<{
    [key: string]: { adjustment: number; manual: number; reason: string };
  }>({});
  ...
  const [customTeam1Players, setCustomTeam1Players] =
    useState<{ name: string; role: string }[]>(defaultT1Players);
  const [customTeam2Players, setCustomTeam2Players] =
    useState<{ name: string; role: string }[]>(defaultT2Players);
  ...
  const [customMatchDate, setCustomMatchDate] = useState<string>(
    formatDateISO(today),
  );
  ...
  const [adminMatchesFilter, setAdminMatchesFilter] = useState<
    "Upcoming" | "Live" | "Completed" | "All"
  >("Upcoming");
  const [showManageContests, setShowManageContests] = useState<boolean>(false);
  const [showManagePrizePools, setShowManagePrizePools] =
    useState<boolean>(false);
  const [showManageMatchContests, setShowManageMatchContests] =
    useState<boolean>(false);
  const [selectedMatchForContests, setSelectedMatchForContests] =
    useState<string>("");
  const [selectedContestsForMatch, setSelectedContestsForMatch] = useState<
    Record<string, boolean>
  >({});
  const [selectedContestForPrize, setSelectedContestForPrize] =
    useState<string>("");
  const [adminPrizeRank, setAdminPrizeRank] = useState<string>("");
  const [adminPrizeAmount, setAdminPrizeAmount] = useState<string>("");
  const [showManageUserTeams, setShowManageUserTeams] =
    useState<boolean>(false);
  ...
  const [adminTeamEditMatchId, setAdminTeamEditMatchId] = useState<
    string | null
  >(null);
  ...
  const [adminExpandedPlayerId, setAdminExpandedPlayerId] = useState<
    string | null
  >(null);
  const [adminLiveMatchId, setAdminLiveMatchId] = useState<string | null>(null);
  const [adminUpcomingLineupMatchId, setAdminUpcomingLineupMatchId] = useState<
    string | null
  >(null);
  const [showManageLineups, setShowManageLineups] = useState<boolean>(false);
  const [isAdminBotEditMode, setIsAdminBotEditMode] = useState<string | null>(
    null,
  );
  const [botInputAuto, setBotInputAuto] = useState<{
    [contestId: string]: string;
  }>({});
  const [botInputBoot, setBotInputBoot] = useState<{
    [contestId: string]: string;
  }>({});

  const [expandedBotsContest, setExpandedBotsContest] = useState<string | null>(
    null,
  );
  const [myMatchesTab, setMyMatchesTab] = useState<
  ...
  const [editingSavedTeamIndex, setEditingSavedTeamIndex] = useState<
    number | null
  >(null);
  const [editReturnView, setEditReturnView] = useState<"ADMIN" | "MY_MATCHES">(
    "ADMIN",
  );

  const [showManageDeposits, setShowManageDeposits] = useState<boolean>(false);
  const [showManageWithdrawals, setShowManageWithdrawals] =
    useState<boolean>(false);
  const [showManageKYC, setShowManageKYC] = useState<boolean>(false);
  const [showManageHelp, setShowManageHelp] = useState<boolean>(false);
  const [helpSettingsInput, setHelpSettingsInput] = useState<{
    email: string;
    telegram: string;
    barcode: string;
    whatsapp: string;
  }>({ email: "", telegram: "", barcode: "", whatsapp: "" });
  ...
  const [adminProfileModalUser, setAdminProfileModalUser] = useState<
    any | null
  >(null);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(db, "wallets", adminProfileModalUser.id),
      (docS) => {
        if (docS.exists()) {
          const walletData = docS.data();
          // Use functional update to avoid stale state issues
          setAdminProfileModalUser((prev) =>
            prev && prev.id === adminProfileModalUser.id
              ? { ...prev, ...walletData }
              : prev,
          );
        }
  ...
  const [walletSaveStatus, setWalletSaveStatus] = useState<{
    [key: string]: "idle" | "saving" | "success";
  }>({});
  ...
  const [appFormats, setAppFormats] = useState<string[]>(() => {
    ...
    try {
      const p = JSON.parse(saved);
      return Array.isArray(p) ? p : ["T20", "ODI", "Test", "IPL"];
    } catch (e) {
      return ["T20", "ODI", "Test", "IPL"];
    }
  });
  useEffect(() => {
  ...
  const [appTeamsList, setAppTeamsList] = useState<
    {
      id: string;
      name: string;
      shortName: string;
  ...
  const [adminTeamDashboard, setAdminTeamDashboard] = useState<string | null>(
    null,
  );
  const [adminContestDashboard, setAdminContestDashboard] = useState<
    string | null
  >(null);
  const [adminMatchDashboard, setAdminMatchDashboard] = useState<string | null>(
    null,
  );
  const [adminUserDashboard, setAdminUserDashboard] = useState<string | null>(
    null,
  );
  ...
  const [adminTeamPlayerRoleFilter, setAdminTeamPlayerRoleFilter] = useState<
    "ALL" | "BAT" | "BOWL" | "AR" | "WK"
  >("ALL");
  ...
  >([]);
  const [showCreateMatchFromTeamsModal, setShowCreateMatchFromTeamsModal] =
    useState(false);
  ...
  const team1Count = team.filter(
    (p) => p.team === activeMatch?.team1?.shortFrame,
  ).length;
  const team2Count = team.filter(
    (p) => p.team === activeMatch?.team2?.shortFrame,
  ).length;
  ...
  const handleSelectMatch = (match: Match) => {
    ...
    setMatchTab(
      match.status === "Upcoming" || isAdmin ? "Contests" : "My Contests",
    );
    setView("MATCH");
  };
  ...
  useEffect(() => {
    if ((view === "CREATE_TEAM" || view === "SELECT_CAPTAIN") && activeMatch) {
      const currentMatchUpdated = appMatches.find(
        (m) => m.id === activeMatch.id,
      );
      const isTimeUp =
        (activeMatch.matchDateISO &&
          new Date(activeMatch.matchDateISO).getTime() <=
            currentTime.getTime()) ||
        currentMatchUpdated?.status !== "Upcoming";
      if (isTimeUp) {
      ...
      setAppMatches((prev) => {
        ...
        const updated = prev.map((m) => {
          if (
            m.status === "Upcoming" &&
            m.matchDateISO &&
            new Date(m.matchDateISO).getTime() <= now.getTime()
          ) {
            changed = true;
  ...
  const togglePlayer = (player: Player) => {
    ...
    if (isSelected) {
      ...
      if (player.role === "WK" && roleCount >= 4)
        return alert("Max 4 WKs allowed");
      if (player.role === "BAT" && roleCount >= 6)
        return alert("Max 6 Batsmen allowed");
      if (player.role === "AR" && roleCount >= 4)
        return alert("Max 4 All-rounders allowed");
      if (player.role === "BOWL" && roleCount >= 6)
        return alert("Max 6 Bowlers allowed");

      const sameTeamCount = team.filter((p) => p.team === player.team).length;
      if (sameTeamCount >= 7)
        return alert(`You can select maximum 7 players from ${player.team}`);
  ...
  const handleSaveTeamAndJoin = () => {
    if (!captain || !viceCaptain)
      return alert("Please select Captain and Vice-Captain");
    ...
    if (isAdminBotEditMode) {
      ...
      localStorage.setItem(
        `dreamApp_bot_blueprint_${activeMatch?.id}`,
        JSON.stringify(bp),
      );
        ...
        const newTeams = prev.map((t) => {
          if (t.match?.id === activeMatch?.id && t.userId === "admin_bot") {
            return {
              ...
              captain,
              viceCaptain,
              _ref: { ...t, players: team, captain, viceCaptain },
            };
          }
          return t;
        });
      ...
      alert(
        "✅ Auto Team updated successfully. All currently joined auto teams will use this lineup!",
      );
    ...
    if (editingSavedTeamIndex !== null) {
      ...
      if (editedTeamMeta.userId && editedTeamMeta.userId !== "guest") {
        ...
        setDoc(doc(db, "userTeams", editedTeamMeta.id), editedTeamMeta, {
          merge: true,
        }).catch((e) => handleFsError(e, "update_user_team", userTeamPath));
      }
    ...
    const contestName = selectedContest
      ? selectedContest.name
      : "Mega Contest (₹55 Crore)";

    const teamIdStr = `T${savedTeams.length + 1}`;
    const newId = Date.now().toString();
    const contestDef = appContests?.find((c) =>
      selectedContest?.id
        ? c.id === selectedContest.id
        : c.name === contestName,
    ) ||
      (DEFAULT_CONTESTS && DEFAULT_CONTESTS[0]) || {
        spots: 2,
        name: "Contest",
        fee: 59,
      };
    const instanceSpots =
      contestDef && contestDef.spots > 0 ? contestDef.spots : 2;
    const sameContestTeams = (savedTeams || []).filter(
      (t) =>
        t.match?.id === activeMatch?.id &&
        (t.contestId
          ? t.contestId === selectedContest?.id
          : t.contestName === contestName),
    );

    const userTeamsInSameContest = sameContestTeams.filter(
      (t) => t.userId === (user?.id || "guest"),
    );
    if (
      contestDef &&
      contestDef.maxTeams &&
      userTeamsInSameContest.length >= contestDef.maxTeams
    ) {
      alert(
        `You can only join this contest a maximum of ${contestDef.maxTeams} times.`,
      );
      return;
    }
    ...
    if (
      ...
      if (availableBalance < fee) {
        alert(
          `Insufficient valid balance (₹${availableBalance.toFixed(0)}) to pay ₹${fee} fee. Max ₹10 can be used from bonus. Please add cash.`,
        );
        setView("WALLET");
        return;
      }
    ...
    const liteMatch = activeMatch
      ? {
          id: activeMatch.id,
          series: activeMatch.series,
    ...
    const newTeamMeta = {
      ...
      userNumericId: user?.numericId,
      userName:
        user?.name || String(user?.email || "").split("@")[0] || "Guest Player",
    ...
    setSavedTeams((prev) => {
      ...
      if (userTeams.length >= 10) {
        ...
        const sortedUserTeams = [...userTeams].sort(
        ...
        if (user?.id) {
          idsToDelete.forEach((id) => {
            deleteDoc(doc(db, "userTeams", id)).catch((e) =>
              console.error("Could not delete old team", e),
            );
          });
        }
      }
    ...
    if (user?.id) {
      ...
      setDoc(doc(db, "userTeams", newId), newTeamMeta).catch((e) =>
        handleFsError(e, "save_user_team", userTeamPath),
      );
    }

    alert(
      `🎉 Successfully joined the ${contestName}!\n\n₹${fee} deducted from Wallet.`,
    );
    setView("MY_MATCHES");
  };
  ...
  const renderBottomNav = () => (
    <nav className="bg-app-bg border-t border-app-border flex justify-around p-2 pb-safe shadow-[0_-5px_15px_rgba(0,0,0,0.5)] z-20 shrink-0">
      <button
        ...
        className={`flex flex-col items-center p-2 gap-1 ${view === "HOME" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
      >
      ...
      <button
        ...
        className={`flex flex-col items-center p-2 gap-1 ${view === "MY_MATCHES" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
      >
        <Clock
          size={22}
          fill={view === "MY_MATCHES" ? "currentColor" : "none"}
        />
        <span className="text-[10px] font-bold">My Matches</span>
      </button>
      <button
        onClick={() => setView("REWARD")}
        className={`flex flex-col items-center p-2 gap-1 ${view === "REWARD" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
      >
      ...
      <button
        ...
        className={`flex flex-col items-center p-2 gap-1 ${view === "WALLET" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
      >
      ...
      {isAdmin && (
        <button
          ...
          className={`flex flex-col items-center p-2 gap-1 ${view === "ADMIN" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
        >
      ...
      <button
        ...
        className={`flex flex-col items-center p-2 gap-1 ${view === "PROFILE" ? "text-[#FF3B5C]" : "text-app-text-muted"}`}
      >
  ...
  const renderTeamCardUI = (
    st: any,
    i: number,
    source: "MATCH" | "MY_MATCHES",
    currentMatchStatus: string,
    totalPoints: number,
  ) => {
    const t1Count = (st.players || []).filter(
      (p: any) =>
        p.team === st.match?.team1?.shortFrame ||
        p.team === st.match?.team1?.name,
    ).length;
    const t2Count = (st.players || []).filter(
      (p: any) =>
        p.team === st.match?.team2?.shortFrame ||
        p.team === st.match?.team2?.name,
    ).length;

    const wkCount = (st.players || []).filter(
      (p: any) => p.role === "WK",
    ).length;
    const batCount = (st.players || []).filter(
      (p: any) => p.role === "BAT",
    ).length;
    const arCount = (st.players || []).filter(
      (p: any) => p.role === "AR",
    ).length;
    const bowlCount = (st.players || []).filter(
      (p: any) => p.role === "BOWL",
    ).length;
    ...
    return (
      <div
        key={i}
        className={`rounded-xl shadow-md border border-app-border overflow-hidden mb-4 ${currentMatchStatus === "Completed" ? "opacity-80" : ""}`}
        onClick={() => {
          ...
          const c =
            appContests.find((c: any) => c.name === st.contestName) ||
            appContests[0];
      ...
      >
        <div className="relative h-[130px] bg-[#2a7a37] overflow-hidden">
          ...
          <div
            className="absolute top-2 right-2 z-10 flex bg-black/60 backdrop-blur-sm rounded-lg overflow-hidden border border-white/20"
          ...
          >
            ...
            {currentMatchStatus === "Upcoming" && (
              <button
                onClick={() => {
                  const realIndex = savedTeams.findIndex(
                    (orig) => orig.id === st.id,
                  );
                  setEditingSavedTeamIndex(realIndex);
                  setEditReturnView(source);
                  setActiveMatch(st.match);
                  const cMatch = appContests.find(
                    (cc: any) => cc.name === st.contestName,
                  );
            ...
            <div className="flex gap-2">
              {currentMatchStatus === "Upcoming" ? (
                ...
                <div className="flex flex-col justify-center items-start pl-2 mb-1">
                  <span className="text-white text-xs font-semibold mb-0 drop-shadow-md opacity-90">
                    Points
                  </span>
                  <span className="text-white text-2xl font-bold drop-shadow-md">
                    {Number(totalPoints) % 1 !== 0
            ...
            <div className="flex gap-4">
              {cap && (
                <div className="flex flex-col items-center relative">
                  <div className="absolute -top-1 -left-1 bg-[#d32f2f] text-white text-[10px] font-black w-4 h-4 flex items-center justify-center rounded-full border border-white z-10 shadow-sm">
                    C
                  </div>
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-slate-200 border-2 border-white shadow-md relative">
                    {cap.image ? (
                      ...
                      <User
                        size={28}
                        className="text-slate-400 absolute bottom-0 left-1/2 -translate-x-1/2"
                      />
                    )}
                  </div>
                  <span className="bg-white text-black text-[9px] font-bold px-2 py-0.5 mt-1 rounded shadow-sm border border-slate-200 truncate max-w-[70px]">
                    {cap.name.split(" ")[0]}
                  </span>
                </div>
              )}
              {vcap && (
                <div className="flex flex-col items-center relative">
                  <div className="absolute -top-1 -left-1 bg-[#546e7a] text-white text-[10px] font-black w-4 h-4 flex items-center justify-center rounded-full border border-white z-10 shadow-sm">
                    VC
                  </div>
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-slate-200 border-2 border-white shadow-md relative">
                    {vcap.image ? (
    ...
    <div className="flex flex-col h-full bg-app-bg">
      <header className="p-4 flex items-center justify-between pb-2 bg-app-bg relative">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar justify-end flex-nowrap w-full ml-2">
          <div
            ...
            onClick={() => setView("PROFILE")}
          >
            <span className="font-bold text-sm text-app-text truncate max-w-[80px]">
              Hey{" "}
              {user
            ...
            <span className="bg-yellow-500 text-black text-[10px] px-1.5 py-0.5 rounded flex items-center font-bold shrink-0">
              ⚡ Lvl 3
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-1 border-l border-slate-800 pl-2">
            ...
            {isAdmin && (
              <button
                onClick={() => setShowAdminQuickAdd(true)}
                className="bg-green-500/20 text-green-500 border border-green-500/50 px-1.5 py-[2px] rounded text-[8px] uppercase font-black transition-colors hover:bg-green-500/30 flex items-center gap-[2px] shrink-0"
              >
            ...
            <button
              onClick={() => setView("WALLET")}
              className="flex items-center gap-1 bg-app-card/80 border border-app-border px-1.5 py-[2px] rounded text-[10px] font-bold text-app-text transition-colors hover:bg-app-card-hover shrink-0"
            >
              <Wallet size={10} />₹
      ...
      <div className="flex-1 overflow-y-auto px-0 pb-20 space-y-5 pt-2">
        {appBanners.filter((b) => b.imageUrl).length > 0 ? (
          <div className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar gap-3 px-4 pb-1">
            {appBanners
              ...
              .map((banner) => (
                <div
                  key={banner.id}
                  className="min-w-[88%] sm:min-w-[80%] h-40 rounded-2xl overflow-hidden snap-center shrink-0 relative shadow-[0_4px_15px_rgba(0,0,0,0.3)]"
                >
                  <img
                    src={banner.imageUrl}
                    alt="Banner"
          ...
          <div
            className="mx-4 bg-app-accent/90 rounded-xl p-4 flex flex-col relative overflow-hidden mt-2 cursor-pointer"
            onClick={() => {
              if (appMatches.length > 0) {
                handleSelectMatch(appMatches[0]);
              }
            }}
          >
            <div className="text-[10px] font-bold text-app-text/80 uppercase">
              Mega Contest
            </div>
            <div className="text-4xl font-black text-app-text my-1 tracking-tighter">
              ₹1 Cr
            </div>
            <div className="text-xs text-app-text/90">
              Win a share of the biggest prize
              <br />
              pool today
            </div>
            <Trophy
              size={80}
              className="absolute -right-4 -bottom-4 text-app-text/20"
            />
          </div>
        )}
        ...
        <div className="px-4">
          ...
          {appMatches.filter((m) => m.status === "Upcoming").length > 0 && (
            <>
              <h2 className="font-bold mt-4 text-app-text text-lg border-b border-app-border pb-2 flex justify-between">
                IPL 2026
                <span className="text-xs text-app-text-muted font-normal self-end mb-1">
                  {appMatches.filter((m) => m.status === "Upcoming").length}{" "}
                  matches
                </span>
              </h2>
              {appMatches
                ...
                .sort((a, b) => {
                  const tA = a.matchDateISO
                    ? new Date(a.matchDateISO).getTime()
                    : Infinity;
                  const tB = b.matchDateISO
                    ? new Date(b.matchDateISO).getTime()
                    : Infinity;
                  return tA - tB;
                })
                .map((match) => renderMatchCard(match))}
          ...
          {appMatches.filter(
            ...
            <div className="flex flex-col items-center justify-center p-10 opacity-50 text-center">
              <h3 className="font-bold text-app-text-muted">
  ...
  const renderMatchCard = (match: Match) => {
    const isLineupsOut =
      match.lineupStatus === "OUT" && match.status === "Upcoming";

    // Calculate user stats for this match if it's on the home screen
    const userTeamsForMatch = savedTeams.filter(
      (t) => t.match?.id === match.id && t.userId === (user?.id || "guest"),
    );
    const distinctContestsCount = new Set(
      userTeamsForMatch
        .filter((t) => t.contestId || t.contestName)
        .map((t) => t.contestId || t.contestName),
    ).size;
    const distinctTeamsCount = new Set(userTeamsForMatch.map((t) => t.teamId))
      .size;

    // Using gradient hints based on team colors, but muted for dark theme
    const t1Bg = !match?.team1?.color?.startsWith("bg-")
      ? match?.team1?.color
      : "transparent";
    const t2Bg = !match?.team2?.color?.startsWith("bg-")
      ? match?.team2?.color
      : "transparent";
    ...
    return (
      ...
      >
        ...
        <div className="px-3 pt-4 pb-5 flex justify-between items-center relative z-10 mt-1">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-2/5 overflow-hidden rounded-l-xl z-0 pointer-events-none opacity-20">
            <div
              className="absolute inset-0 bg-gradient-to-r from-current to-transparent"
              style={{ color: t1Bg }}
            ></div>
          </div>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-2/5 overflow-hidden rounded-r-xl z-0 pointer-events-none opacity-20">
            <div
              className="absolute inset-0 bg-gradient-to-l from-current to-transparent"
              style={{ color: t2Bg }}
            ></div>
          </div>
          ...
          <div className="flex flex-col items-start gap-1 z-10 w-[35%] relative">
            <span className="text-[10px] sm:text-[11px] font-semibold text-app-text truncate w-full">
              {match.team1.name}
            </span>
            <div className="flex items-center gap-2">
              <div
                className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full shrink-0 border border-app-border flex items-center justify-center overflow-hidden bg-slate-800 ${match?.team1?.color?.startsWith("bg-") ? match.team1.color : ""}`}
                style={
                  !match?.team1?.color?.startsWith("bg-")
              ...
              >
                {match?.team1?.flagUrl ? (
                  <img
                    ...
                    alt={match?.team1?.shortFrame}
                    className={`w-full h-full ${match?.team1?.flagFit === "contain" ? "object-contain" : "object-cover"}`}
                  />
                ) : (
              ...
              <span className="text-xs sm:text-sm font-bold text-app-text">
                {match.team1.shortFrame}
              </span>
            </div>
          </div>
          ...
          {/* VS Center & Timer */}
          <div className="z-10 flex flex-col items-center absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-[60%] w-full">
            {/* Timer Pill */}
            <div className="mb-1 flex justify-center w-full">
              {match.status === "Completed" ? (
                <div className="bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 rounded-full px-2.5 py-[2px] text-[8px] font-bold shadow-sm tracking-wide text-center leading-none">
                  Completed
                </div>
              ) : match.status === "Live" ? (
                <div className="bg-white dark:bg-black text-red-600 rounded-full px-2.5 py-[2px] text-[8px] font-bold shadow-sm border border-red-200 dark:border-red-800 tracking-wide flex items-center justify-center gap-1 leading-none">
                  Live
                </div>
              ) : (
                <div className="bg-[#cc1d24] text-white rounded-full px-2.5 py-[2px] text-[8px] font-bold shadow-sm tracking-wide whitespace-nowrap text-center leading-none">
                  {getFormattedTimer(match)}
                </div>
              )}
            </div>
            <div
              className="text-3xl sm:text-4xl font-black italic text-red-600 drop-shadow-md"
              style={{ WebkitTextStroke: "1px rgba(255,255,255,0.8)" }}
            >
              VS
            </div>
            {isLineupsOut && (
              <div className="mt-1 bg-green-500/20 text-green-400 border border-green-500/30 text-[9px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-sm whitespace-nowrap shadow-[0_0_8px_rgba(34,197,94,0.3)]">
                Lineups Out
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-end gap-1 z-10 w-[35%] relative text-right">
            ...
            <div className="flex items-center gap-2">
              ...
              <div
                className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full shrink-0 border border-app-border flex items-center justify-center overflow-hidden bg-slate-800 ${match?.team2?.color?.startsWith("bg-") ? match.team2.color : ""}`}
                style={
                  !match?.team2?.color?.startsWith("bg-")
              ...
              >
                {match?.team2?.flagUrl ? (
                  <img
                    ...
                    alt={match?.team2?.shortFrame}
                    className={`w-full h-full ${match?.team2?.flagFit === "contain" ? "object-contain" : "object-cover"}`}
                  />
                ) : (
                  <span className="text-[9px] font-bold text-white">
                    {match?.team2?.shortFrame}
        ...
        <div className="bg-slate-50 dark:bg-[#1a1d24] px-3 py-2.5 text-[10px] text-slate-500 font-bold font-sans text-center relative z-10 border-t border-app-border/40">
          {distinctTeamsCount} {distinctTeamsCount === 1 ? "Team" : "Teams"} -{" "}
          {distinctContestsCount}{" "}
          {distinctContestsCount === 1 ? "Contest" : "Contests"} Joined
        </div>
      </div>
    );
  };
  ...
  const handleAddBot = (
    ...
    const availablePlayers = appPlayers.filter(
      (p) =>
        (p.team === activeMatch?.team1?.shortFrame ||
          p.team === activeMatch?.team2?.shortFrame) &&
        (!p.format || activeMatch?.series?.includes(p.format)),
    );
    const variations = [];
    ...
    for (let i = 0; i < safeAmount; i++) {
      ...
      newBots.push({
        ...
        teamId:
          type === "BOOT"
            ? `BOT-${timestamp.toString().slice(-4)}${i}`
            : `T${Math.floor(Math.random() * 89999) + 10000}`,
        ...
        userName:
          type === "BOOT"
            ? `BOOT ${timestamp.toString().slice(-3)}${i}`
            : randomName,
  ...
  const handleRemoveBot = (
    ...
    e.stopPropagation();
    setSavedTeams((prev) => {
      const bots = prev.filter(
        (t) =>
          t.match?.id === activeMatch?.id &&
          (t.contestId
            ? t.contestId === contest.id
            : t.contestName === contest.name) &&
          t.userId === (type === "BOOT" ? "admin_bot_boot" : "admin_bot"),
      );
      ...
      const adminTeams = newTeams.filter(
        (t) => t.userId === "admin_bot" || t.userId === "admin_bot_boot",
      );
  ...
  const renderBotControls = (contest: Contest) => {
    ...
    const botTeamsAuto = savedTeams.filter(
      (t) =>
        t.match?.id === activeMatch?.id &&
        (t.contestId
          ? t.contestId === contest.id
    ...
    const botTeamsBoot = savedTeams.filter(
      (t) =>
        t.match?.id === activeMatch?.id &&
        (t.contestId
          ? t.contestId === contest.id
      ...
      <div className="flex items-center justify-end z-20 gap-2">
        {!isExpanded ? (
          <button
            onClick={(e) => {
              e.stopPropagation();
              setExpandedBotsContest(contest.id);
            }}
          ...
          <div className="flex gap-2">
            <div
              className="flex flex-col items-center bg-app-card-inner border border-app-border rounded shadow-sm p-1"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="text-[9px] font-bold text-[#f0b90b] flex items-center gap-1 justify-center text-center px-1 rounded pb-1">
                <span>
                  Auto Team{" "}
                  {botCountAuto > 0
                    ? `(${botCountAuto.toLocaleString("en-IN")})`
                    : ""}
                </span>
              </div>
  ...
  const renderContests = () => {
    ...
    userTeamsInMatch.forEach((t) => {
      ...
      if (
        !distinctMyContests.find(
          (existing) =>
            (t.contestId
              ? existing.contestId === t.contestId
              : existing.contestName === t.contestName) &&
            existing.instanceId === iId,
        )
      ) {
        distinctMyContests.push({
    ...
    return (
      <div className="flex flex-col h-full bg-app-bg">
        <header className="bg-app-bg text-app-text p-4 flex items-center justify-between shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-sm block">
                {activeMatch?.team1?.shortFrame} vs{" "}
          ...
          {activeMatch?.status === "Upcoming" && (
            <span className="text-[10px] font-bold text-app-text-muted flex items-center gap-1">
              <Clock size={12} /> {getFormattedTimer(activeMatch)} left
            </span>
          )}
        </header>
        ...
        <div className="bg-app-card p-4 flex justify-between items-center z-10 shrink-0 border-b border-app-border">
          <div className="flex flex-col items-center">
            <div
              className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl text-app-text border-2 border-app-border-hover relative overflow-hidden shrinkage-0 ${activeMatch?.team1?.color}`}
            >
              {activeMatch?.team1?.flagUrl ? (
                <img
                  src={activeMatch?.team1?.flagUrl}
                  alt={activeMatch?.team1?.shortFrame}
                  className={`w-full h-full ${activeMatch?.team1?.flagFit === "contain" ? "object-contain" : "object-cover"}`}
                />
              ) : (
                activeMatch?.team1?.shortFrame
              )}
            </div>
            <span className="text-xs text-app-text mt-1 font-bold">
              {activeMatch?.team1?.shortFrame}
            </span>
          </div>
          <div className="flex flex-col items-center text-center px-2">
            {activeMatch?.status === "Live" ? (
              <div className="flex flex-col items-center">
                <span className="text-app-accent font-bold text-base tracking-widest flex items-center gap-1 mb-1">
                  <div className="w-2 h-2 rounded-full bg-app-accent animate-pulse"></div>
                  LIVE
                </span>
                {liveScore && (
                  <span className="text-yellow-400 font-black text-xs text-center border-t border-app-border-hover pt-1 mt-1 break-words">
                    {liveScore}
                  </span>
                )}
              </div>
            ) : (
              <span className="text-[#FFD700] font-bold text-sm block">
                {String(activeMatch?.status || "").toUpperCase()}
              </span>
            )}
          ...
          <div className="flex flex-col items-center">
            <div
              className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl text-app-text border-2 border-app-border-hover relative overflow-hidden shrinkage-0 ${activeMatch?.team2?.color}`}
            >
              {activeMatch?.team2?.flagUrl ? (
                <img
                  src={activeMatch?.team2?.flagUrl}
                  alt={activeMatch?.team2?.shortFrame}
                  className={`w-full h-full ${activeMatch?.team2?.flagFit === "contain" ? "object-contain" : "object-cover"}`}
                />
              ) : (
                activeMatch?.team2?.shortFrame
              )}
        ...
        <div className="flex bg-app-bg text-xs font-bold shrink-0 border-b border-app-border">
          {(activeMatch?.status === "Upcoming" || isAdmin) && (
            <button
              className={`flex-1 py-3 ${matchTab === "Contests" ? "text-app-accent border-b-2 border-app-accent" : "text-app-text-muted"}`}
          ...
          <button
            className={`flex-1 py-3 ${matchTab === "My Contests" ? "text-app-accent border-b-2 border-app-accent" : "text-app-text-muted"}`}
            onClick={() => setMatchTab("My Contests")}
          >
            {`My Contests (${myContestsCount})`}
          </button>
          <button
            className={`flex-1 py-3 ${matchTab === "My Teams" || (activeMatch?.status !== "Upcoming" && !isAdmin && matchTab !== "My Contests") ? "text-app-accent border-b-2 border-app-accent" : "text-app-text-muted"}`}
            onClick={() => setMatchTab("My Teams")}
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-20">
          {matchTab === "Contests" &&
            (activeMatch?.status === "Upcoming" || isAdmin) && (
              <>
                {megaContests.map((c) => {
                  const totalTeamsCount = savedTeams.filter(
                    (t) =>
                      t.match?.id === activeMatch?.id &&
                      (t.contestId
                        ? t.contestId === c.id
                        : t.contestName === c.name),
                  ).length;
                  ...
                  return (
                    <div
                      ...
                      className="bg-app-card rounded-xl shadow-sm border border-app-border overflow-hidden mb-2 cursor-pointer active:scale-[0.99] transition-transform relative"
                    >
                      <div className="p-2 bg-app-card-inner">
                        <div className="flex justify-between items-center mb-1">
                          <p className="text-[9px] text-app-text-muted font-semibold uppercase tracking-wider">
                            Prize Pool
                          </p>
                          {renderBotControls(c)}
                        </div>
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (activeMatch?.status !== "Upcoming") {
                                alert("Match is already live or completed!");
                                return;
                              }
                              setActiveContestInstanceId(null);
                              setActiveContestDetails(c);
                        ...
                        <div className="flex justify-between text-[9px] text-app-text-muted font-semibold pb-1">
                          <span className="text-red-400">
                            {Number(spotsLeft || 0).toLocaleString("en-IN")}{" "}
                            spots left
                          </span>
                          <span>
                      ...
                      <div className="p-1.5 px-2 bg-slate-50/5 dark:bg-app-bg border-t border-app-border flex shrink-0 items-center justify-start gap-3 text-[9px] font-semibold text-app-text-muted">
                        <span className="flex items-center gap-1">
                          <div className="w-3.5 h-3.5 rounded-full border border-yellow-500 text-[#e5c158] flex items-center justify-center text-[7px] font-bold bg-[#e5c158]/10">
                            1st
                          </div>{" "}
                          {c.firstPrize || "₹10 Lakh"}
                        </span>
                        <span className="flex items-center gap-1">
                          <Trophy size={9} className="text-app-text-muted" />{" "}
                          {c.winPercentage || 48}%
                        </span>
                        <span className="flex items-center gap-1">
                          <div className="w-3.5 h-3.5 border border-slate-500 rounded flex items-center justify-center text-[7px] font-bold text-app-text-muted">
                            M
                          </div>{" "}
                          {c.maxTeams || 20}
                        </span>
                  ...
                  const groupedContests = [
                    {
                      type: "H2H",
                      title: "Head To Head",
                      subtitle: "2 member 1 winner",
                      list: h2hContests.filter((c) => c.type === "H2H"),
                    },
                    {
                      type: "3 Spots",
                      title: "3 Spots",
                      subtitle: "3 member 1 winner",
                      list: h2hContests.filter((c) => c.type === "3 Spots"),
                    },
                    {
                      type: "4 Spots",
                      title: "4 Spots",
                      subtitle: "4 member 1 winner",
                      list: h2hContests.filter((c) => c.type === "4 Spots"),
                    },
                    {
                      type: "5 Spots",
                      title: "5 Spots",
                      subtitle: "5 member 1 winner",
                      list: h2hContests.filter((c) => c.type === "5 Spots"),
                    },
                  ];

                  return groupedContests.map((group) => {
                    if (group.list.length === 0) return null;
                    const isExpanded = expandedContestTypes.includes(
                      group.type,
                    );
                    const displayList = isExpanded
                      ? group.list
                      : group.list.slice(0, 4);
                    ...
                    return (
                      <div key={group.type} className="mb-4">
                        ...
                        <div className="flex items-center justify-between mb-2 px-1">
                          <div className="flex items-center gap-2 py-1 px-2.5 bg-[#f8eaea]/80 dark:bg-red-500/10 border border-red-500/20 rounded-lg w-fit">
                            ...
                            <div className="flex flex-col pr-1">
                              <span className="font-extrabold text-[10px] text-app-text leading-tight">
                                {group.title}
                              </span>
                              <span className="text-[8px] text-app-text-muted leading-tight font-medium mt-0.5">
                                {group.subtitle}
                              </span>
                            </div>
                          </div>
                          ...
                          {group.list.length > 4 && (
                            <button
                              onClick={() => {
                                if (isExpanded) {
                                  setExpandedContestTypes(
                                    expandedContestTypes.filter(
                                      (t) => t !== group.type,
                                    ),
                                  );
                                } else {
                                  setExpandedContestTypes([
                                    ...expandedContestTypes,
                                    group.type,
                                  ]);
                                }
                              }}
                              className="py-1 px-2.5 rounded-lg flex items-center justify-center gap-1 text-[10px] font-bold text-app-text-muted hover:text-app-text transition-colors bg-app-card border border-app-border shadow-sm active:scale-95"
                            >
                        ...
                        {displayList.map((c) => {
                          ...
                          const teamsInCurrentInstance =
                            totalTeamsCount % cspots;
                          const spotsLeft = cspots - teamsInCurrentInstance;
                          const fillPercent =
                            (teamsInCurrentInstance / cspots) * 100;
                          ...
                          return (
                            <div
                              key={c.id}
                              onClick={() => {
                                setActiveContestInstanceId(
                                  Math.floor(totalTeamsCount / cspots),
                                );
                                setActiveContestDetails(c);
                                setView("CONTEST_DETAILS");
                              }}
                                ...
                                <div className="flex justify-between items-center mb-2">
                                  ...
                                  <button
                                    onClick={(e) => {
                                      ...
                                      if (activeMatch?.status !== "Upcoming") {
                                        alert(
                                          "Match is already live or completed!",
                                        );
                                        return;
                                      }
                                      setActiveContestDetails(c);
                        ...
                        {/* Expand/Collapse buttons remmoved from here */}
                      </div>
                    );
                  });
                })()}
            ...
            <>
              {distinctMyContests.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full opacity-60 mt-10">
                  ...
                  <p className="font-bold text-app-text-muted">
                    No contests joined!
                  </p>
                ...
                distinctMyContests.map((dc, i) => {
                  const c = appContests.find((cc) =>
                    dc.contestId
                      ? cc.id === dc.contestId
                      : cc.name === dc.contestName,
                  );
                  if (!c) return null;
                  const totalTeamsCount = savedTeams.filter(
                    (t) =>
                  ...
                  if (c.type === "Mega") {
                    ...
                    const teamsInThisInstance = savedTeams.filter(
                      (t) =>
                        t.match?.id === activeMatch?.id &&
                        (t.contestId
                          ? t.contestId === c.id
                          : t.contestName === c.name) &&
                        t.instanceId === dc.instanceId,
                    ).length;
                  ...
                  if (isLiveOrCompleted) {
                    const instanceTeams = savedTeams.filter(
                      (t) =>
                        t.match?.id === activeMatch?.id &&
                        (t.contestId
                          ? t.contestId === c.id
                    ...
                    const teamsWPoints = instanceTeams.map((t) => {
                      ...
                      if (botKey && memoizedBotPoints[botKey] !== undefined) {
                        ...
                        pts = (t.players || []).reduce(
                          (acc: number, player: Player) => {
                            const livePlayer =
                              appPlayers.find((p) => p.id === player.id) ||
                              player;
                            let mult = 1;
                            if (livePlayer.id === t.captain) mult = 2;
                            else if (livePlayer.id === t.viceCaptain)
                              mult = 1.5;
                            return (
                              acc +
                              (livePlayer.livePoints ?? livePlayer.points) *
                                mult
                            );
                          },
                          0,
                    ...
                    teamsWPoints.forEach((t, i) => {
                      if (i > 0 && teamsWPoints[i - 1].pts > t.pts)
                        currentRank = i + 1;
                      t.calcRank = currentRank;
                    });
                    myRenderTeams = teamsWPoints.filter(
                      (t) => t.userId === (user?.id || "guest"),
                    );
                  }
                    ...
                    >
                      <div className="p-2 bg-app-card-inner">
                        <div className="flex justify-between items-center mb-1 border-b border-app-border pb-1">
                          <p className="text-[11px] text-app-text-muted font-bold tracking-wider flex items-center gap-1.5">
                            <div className="w-4 h-4 rounded-full bg-red-100 flex items-center justify-center shrink-0">
                              <Trophy size={9} className="text-red-500" />
                            </div>{" "}
                            {c.prizeText}
                          </p>
                          <span className="flex items-center gap-1 text-[11px] font-bold text-app-text-muted">
                            <Trophy size={11} className="text-[#e5c158]" />{" "}
                            {c.winPercentage || (c.type === "Mega" ? 48 : 50)}%{" "}
                            <div className="ml-1 w-4 h-4 border border-slate-300 rounded flex items-center justify-center text-[9px] font-black text-slate-500">
                              M
                            </div>
                          </span>
                        </div>
                        ...
                        {!isLiveOrCompleted ? (
                          <>
                            ...
                            <div className="flex justify-between text-[9px] text-app-text-muted font-semibold pb-1">
                              ...
                              <span>
                                {c.type === "Mega" ? c.spots : cspots} spots
                              </span>
                            </div>
                          </>
                        ) : (
                          <div className="mt-2 -mx-2 mb-0">
                            ...
                            {myRenderTeams.map((t, idx) => {
                              ...
                              const userName =
                                t.userName || user?.name || "User";

                              return (
                                <div
                                  key={t.id}
                                  className="flex justify-between items-center px-3 py-2 border-b border-app-border last:border-b-0 bg-white dark:bg-app-card"
                                >
                                  <div className="w-16 flex items-center gap-2">
                                    <div className="w-6 h-6 border bg-[#faeee8] dark:bg-[#342420] border-[#ee5723]/30 flex items-center justify-center font-bold text-[9px] text-app-text rounded">
                                      {teamLabel}
                                    </div>
                                  </div>
                                  <div className="flex-1 flex flex-col items-center">
                                    <span className="font-bold text-app-text text-[11px] mb-0.5">
                                      {userName}
                                    </span>
                                    <div className="flex flex-col items-center">
                                      ...
                                      <span className="font-bold text-[11px] text-app-text">
                                        {Number(t.pts) % 1 !== 0
                                          ? Number(t.pts).toFixed(1)
                                          : t.pts}
                                      </span>
                                    </div>
                                  </div>
                                  <div className="w-16 text-right font-bold text-[12px] text-app-text">
                                    #{t.calcRank}{" "}
            ...
            <>
              {savedTeams.filter(
                ...
                savedTeams
                  ...
                  .map((st, i) => {
                    const updatedMatch = appMatches.find(
                      (m) => m.id === activeMatch?.id,
                    );
                    const currentMatchStatus =
                      ...
                      activeMatch?.status ||
                      "Completed";
                    const totalPoints = (st.players || []).reduce(
                      (acc: number, player: Player) => {
                        const livePlayer =
                          appPlayers.find((p) => p.id === player.id) || player;
                        let mult = 1;
                        ...
                        return (
                          acc +
                          (livePlayer.livePoints ?? livePlayer.points) * mult
                        );
                      },
                      0,
                    );
                    return renderTeamCardUI(
                      st,
                      i,
                      "MATCH",
                      currentMatchStatus,
  ...
  const renderCreateTeam = () => {
    ...
    const displayedPlayers = appPlayers.filter(
      (p) =>
        p.role === activeRole &&
        liveTeams.includes(p.team) &&
        (!isLineupsOut || p.isPlaying === true),
    );
      ...
      <div className="flex flex-col h-full bg-app-card relative">
        <header className="bg-app-bg text-app-text border-b border-app-border p-4 pb-2 z-10 shrink-0">
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="p-1 -ml-1 active:bg-app-card/10 rounded-full"
            >
              <ArrowLeft size={24} />
            </button>
            <div className="font-bold text-sm flex gap-2">
              <span className="text-[#f0b90b]">
                {getFormattedTimer(activeMatch)}
              </span>
            </div>
            <button className="p-1 active:bg-app-card/10 rounded-full">
              <Info size={20} />
            </button>
          </div>
          ...
          <div className="bg-app-card-hover rounded-lg p-3 relative overflow-hidden shadow-inner">
            <div className="flex justify-between items-center mb-3">
              <div className="text-center">
                <p className="text-[10px] text-app-text-muted font-bold uppercase mb-0.5">
                  Players
                </p>
                <p className="font-bold text-lg leading-none">
                  {team.length}
                  <span className="text-app-text-muted text-sm">/11</span>
                </p>
              </div>
              <div className="flex gap-4">
                <div className="text-center flex flex-col items-center">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shadow-md border border-white/20 relative overflow-hidden shrinkage-0 ${activeMatch?.team1?.color?.startsWith("bg-") ? activeMatch.team1.color : ""}`}
                    style={
                      !activeMatch?.team1?.color?.startsWith("bg-")
                        ? { backgroundColor: activeMatch?.team1?.color }
                        : {}
                    }
                  >
                    {activeMatch?.team1?.flagUrl ? (
                  ...
                  <span className="text-[10px] font-bold mt-1 text-app-text-muted">
                    {team1Count}
                  </span>
                </div>
                <div className="text-center flex flex-col items-center">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shadow-md border border-white/20 relative overflow-hidden shrinkage-0 ${activeMatch?.team2?.color?.startsWith("bg-") ? activeMatch.team2.color : ""}`}
                    style={
                      !activeMatch?.team2?.color?.startsWith("bg-")
                        ? { backgroundColor: activeMatch?.team2?.color }
                        : {}
                    }
                  >
                    {activeMatch?.team2?.flagUrl ? (
          ...
          {(["WK", "BAT", "AR", "BOWL"] as Role[]).map((r) => {
            ...
            return (
              ...
              >
                {r}{" "}
                <span
                  className={`text-[10px] ${count > 0 ? (activeRole === r ? "text-app-accent" : "text-app-text") : "opacity-0"}`}
                >
                  ({count})
                </span>
              </button>
          ...
          {activeRole === "WK"
            ...
            : activeRole === "BAT"
              ...
              : activeRole === "AR"
                ? "1-8 All Rounder"
                : "1-8 Bowler"}{" "}
          <Info size={12} className="inline-block ml-1 opacity-70 mb-0.5" />
        </div>
          ...
          <div className="bg-app-card">
            <AnimatePresence>
              {displayedPlayers.map((player) => {
                ...
                const tColorRaw = isTeam1
                  ? activeMatch?.team1?.color || "bg-blue-600"
                  : activeMatch?.team2?.color || "bg-rose-600";
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowPlayerInfoPopup(player);
                      }}
                    >
                      <div className="absolute top-0 -left-1 opacity-50 z-20 hover:opacity-100">
                        <Info size={14} className="text-app-text" />
                      </div>
                      <div className="w-[50px] h-[50px] flex items-end justify-center relative overflow-visible z-10 drop-shadow-sm mt-1">
                        ...
                        <div className="w-full h-full relative flex items-end justify-center overflow-hidden rounded-t-[20px] bg-transparent">
                          <User
                            size={50}
                            className={`text-[#d1d5db] dark:text-[#475569] translate-y-3 block ${isSelected ? "opacity-90" : "opacity-100"}`}
                            fill="currentColor"
                            strokeWidth={1}
                          />
                        </div>
                      ...
                      <div className="absolute -bottom-1 -left-1 z-20 px-[5px] py-[2px] rounded-[4px] text-[8px] tracking-widest font-black uppercase shadow-[0_2px_4px_rgba(0,0,0,0.1)] border border-app-border bg-app-bg text-app-text dark:bg-app-card-inner">
                        {player.team}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 pl-4">
                      <h4 className="font-bold text-[14px] text-app-text tracking-wide">
                        {player.name}
                      </h4>
                      <div className="flex flex-col gap-0.5 mt-0.5">
                        <p className="text-[10px] text-app-text-muted font-medium">
                          ...
                          {(
                            player.selPercent ||
                            ((player.id.charCodeAt(0) - 40) % 70) + 10
                          ).toFixed(2)}
                          %
                        </p>
                        {player.isPlaying === true && (
                          <span className="flex items-center gap-1 mt-0.5 text-[9px] text-[#2563eb] dark:text-[#60a5fa] font-bold">
                            <div className="w-[5px] h-[5px] bg-[#2563eb] dark:bg-[#60a5fa] rounded-full"></div>{" "}
                            Played last match
                          </span>
                        )}
                    ...
                    <div className="w-[70px] flex items-center justify-between text-base pr-4">
                      <span className="font-bold text-[13px] text-app-text">
                        {player.credits}
                      </span>
                      <button
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-app-card/90 backdrop-blur-md border-t border-app-border flex justify-between gap-4 z-20">
          ...
          <button
            onClick={() => {
              if (team.length < 11)
                return alert(`Select ${11 - team.length} more players`);
              ...
              if (wkCount < 1)
                return alert("Select at least 1 Wicket Keeper (WK)");
              if (batCount < 1) return alert("Select at least 1 Batsman (BAT)");
              if (arCount < 1)
                return alert("Select at least 1 All-Rounder (AR)");
              if (bowlCount < 1)
                return alert("Select at least 1 Bowler (BOWL)");
              ...
              if (team1Count > 7)
                return alert(
                  `You can select maximum 7 players from ${team1Short}`,
                );
              if (team2Count > 7)
                return alert(
                  `You can select maximum 7 players from ${team2Short}`,
                );
            ...
            className={`flex-1 font-bold py-3 mx-auto rounded-lg shadow-md transition-colors ${
              team.length === 11
                ...
                : "bg-slate-300 text-app-text-muted cursor-not-allowed"
            }`}
          >
            NEXT
          </button>
  ...
  const renderTeamPreview = () => {
    const isLiveOrCompleted =
      activeMatch?.status === "Live" || activeMatch?.status === "Completed";
      ...
      <div className="flex flex-col h-full bg-[#3f9e4d] relative overflow-hidden font-sans">
        ...
        <header className="flex items-center px-4 py-3 bg-[#13151c] text-white z-20 relative border-b border-[#701516]/50">
          <button
            onClick={() =>
              setView(
                previewSource === "MY_MATCHES"
                  ? "MY_MATCHES"
                  : previewSource === "CONTEST_DETAILS"
                    ? "CONTEST_DETAILS"
                    : "CREATE_TEAM",
              )
            }
            className="w-6 h-6 flex items-center justify-center bg-white text-black rounded-full mr-3 shrink-0"
          >
            <X
              size={16}
          ...
          <h2 className="font-medium flex-1 text-lg leading-none truncate">
            {previewTeamInfo ? previewTeamInfo.name : "Team Preview"}
          </h2>
          {!isLiveOrCompleted && previewSource === "MY_MATCHES" && (
            <button
              onClick={() => setView("CREATE_TEAM")}
              className="p-1.5 active:bg-white/10 rounded-full bg-white/5 hover:bg-white/10 transition-colors shrink-0"
            >
        ...
        <div className="bg-[#701516] px-4 py-3 flex justify-between items-center text-white z-20 relative">
          ...
          <div className="flex items-center justify-center gap-3 w-1/3">
            <div className="px-2.5 py-0.5 border border-white/40 rounded-3xl text-[10px] font-bold bg-transparent text-white truncate max-w-[40px]">
              {activeMatch?.team1?.shortFrame}
            </div>
            <span className="text-sm font-bold whitespace-nowrap">
              {team1Count} : {team2Count}
            </span>
            <div className="px-2.5 py-0.5 border border-white/40 rounded-3xl text-[10px] font-bold bg-[#13151c] text-white truncate max-w-[40px]">
              {activeMatch?.team2?.shortFrame}
            </div>
          </div>
            ...
            {isLiveOrCompleted ? (
              <>
                ...
                <span className="text-sm font-bold">
                  {previewTeamInfo?.points ?? totalPoints}
                </span>
              </>
            ) : (
              <>
        ...
        <div
          className="absolute inset-x-0 bottom-0 top-[90px] z-0 overflow-hidden bg-[#229933]"
          style={{
            backgroundImage:
              "repeating-linear-gradient(90deg, rgba(0,0,0,0.02) 0px, rgba(0,0,0,0.02) 40px, transparent 40px, transparent 80px)",
          }}
        >
          {/* Oval pitch line */}
              ...
              >
                ...
                <div className="flex justify-center gap-0.5 sm:gap-2 w-full px-0">
                  {playersInRole.map((p) => {
                    if (!p) return null;
                    const latestP =
                      appPlayers.find((ap) => ap.id === p.id) || p;
                    const isC = p.id === captain;
                    const isVC = p.id === viceCaptain;
                    const pName = p.name || "Player";
                    const pPoints = Number(
                      (latestP.livePoints ?? latestP.points) || 0,
                    );
                    return (
                      <div
                        key={p.id}
                        className="flex flex-col items-center cursor-pointer relative group scale-95 sm:scale-100"
                      >
                        <div className="relative mb-[3px] flex flex-col items-center">
                          ...
                          {(() => {
                            ...
                            const tColorRaw = isTeam1
                              ...
                              : activeMatch?.team2?.color || "bg-rose-600";
                            const custStyle =
                              !tColorRaw?.startsWith("bg-") &&
                              !tColorRaw?.startsWith("text-") &&
                              !tColorRaw?.startsWith("border-")
                                ? { color: tColorRaw, borderColor: tColorRaw }
                                : {};
                            return (
                              <div className="relative shrink-0 w-[34px] h-[38px] flex flex-col items-center justify-start drop-shadow-md">
                                <div className="relative w-[34px] h-[34px] flex items-end justify-center z-10 pt-[6px]">
                                  ...
                                  <div
                                    ...
                                    style={
                                      custStyle.color
                                        ? {
                                            backgroundColor: custStyle.color,
                                ...
                                <div className="absolute -bottom-[2px] z-20 px-[3px] py-[0px] rounded-[3px] text-[7px] tracking-widest font-black uppercase shadow-[0_2px_4px_rgba(0,0,0,0.6)] border border-white/20 bg-[#0f172a] text-white">
                                  {p.team}
                                </div>
                              </div>
                            );
                          ...
                          {(isC || isVC) && (
                            <div
                              className={`absolute -top-[3px] -left-[3px] w-[14px] h-[14px] border border-white text-white rounded-full flex items-center justify-center text-[7.5px] font-bold shadow-[0_0_6px_rgba(0,0,0,0.8)] z-30 ${isC ? "bg-[#d32f2f]" : "bg-slate-700 text-white"}`}
                            >
                              {isC ? "C" : "VC"}
                            </div>
                          )}
                        </div>
                        <div className="bg-black/90 backdrop-blur-md border border-white/20 text-white text-[8px] font-medium px-1 py-0.5 rounded shadow-xl whitespace-nowrap overflow-hidden text-ellipsis w-full max-w-[45px] text-center relative z-10">
                          {pName}
                        </div>
                        <span className="text-white text-[8px] mt-0.5 font-bold drop-shadow-md bg-black/30 px-1 rounded">
                          {isLiveOrCompleted
                            ? `${(pPoints * (isC ? 2 : isVC ? 1.5 : 1)).toFixed(1)} Pt`
                            : `${p.credits || 0} Cr`}
                        </span>
                      </div>
                    );
          ...
          {team &&
            (team || []).map((player) => {
              ...
              const tColorRaw = isTeam1
                ? activeMatch?.team1?.color || "bg-blue-600"
                : activeMatch?.team2?.color || "bg-rose-600";
              const tTextCls = tColorRaw?.startsWith("bg-")
                ? tColorRaw.replace("bg-", "text-")
                : tColorRaw?.startsWith("border-")
                  ? tColorRaw.replace("border-", "text-")
                  : tColorRaw?.startsWith("text-")
                    ? tColorRaw
                    : "";
              const tBorderCls = tColorRaw?.startsWith("bg-")
                ? tColorRaw.replace("bg-", "border-")
                : tColorRaw?.startsWith("text-")
                  ? tColorRaw.replace("text-", "border-")
                  : tColorRaw?.startsWith("border-")
                    ? tColorRaw
                    : "";
              const custStyle =
              ...
              return (
                <div
                  key={player.id}
                  className="flex items-center p-3 border-b border-white/5"
                >
                  <div className="relative shrink-0 w-[54px] h-[58px] flex flex-col items-center justify-start mt-1">
                    <div
                      ...
                      style={
                        custStyle.borderColor
                          ? { borderColor: custStyle.borderColor }
                          : {}
                      }
                    >
                      <div className="w-full h-full flex flex-col items-center justify-end bg-gradient-to-b from-transparent to-black/40">
                        <User
                          size={34}
                          className="text-slate-400 translate-y-1 relative z-0 drop-shadow-md"
                        />
                      </div>
                    </div>

                    <div className="absolute bottom-[2px] z-20 px-[6px] py-[2px] rounded-[4px] text-[8px] tracking-widest font-black uppercase shadow-[0_2px_4px_rgba(0,0,0,0.1)] border border-app-border bg-app-bg text-app-text dark:bg-app-card-inner">
                      {player.team}
                    </div>
                  </div>
                  <div className="flex-1 pl-3">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {player.isPlaying === true && (
                        <span className="flex items-center gap-1 text-[10px] text-green-500 font-bold bg-green-500/10 px-1 py-0.5 rounded-sm border border-green-500/20">
                          <Check size={8} strokeWidth={4} /> Playing
                        </span>
                      )}
                      {player.isPlaying === false && (
                        <span className="flex items-center gap-1 text-[10px] text-red-400 font-bold bg-red-500/10 px-1 py-0.5 rounded-sm border border-red-500/20">
                  ...
                  <div className="w-[100px] flex items-center justify-around px-1 gap-1">
                    ...
                    <button
                      onClick={() => {
                        if (captain === player.id) setCaptain(null);
                        setViceCaptain(
                          viceCaptain === player.id ? null : player.id,
                        );
                      }}
  ...
  const renderMyMatches = () => {
    ...
    const uniqueMatchIds = Array.from(
      new Set(filteredTeams.map((st) => st.match?.id)),
    ).filter(Boolean);
    const joinedMatches = uniqueMatchIds
      .map((id) => appMatches.find((m) => m.id === id))
  ...
  const renderWallet = () => (
    <div className="flex flex-col h-full bg-app-bg relative">
      <header className="p-4 flex items-center justify-between pb-4 bg-app-bg">
        ...
        <button className="text-app-text-muted" onClick={() => setView("HOME")}>
          <ArrowLeft />
        </button>
      </header>
      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto pb-20">
        <div className="bg-app-card rounded-xl shadow-sm p-5 border border-app-border">
          ...
          <h1 className="text-[36px] font-bold text-app-text mt-1 leading-none tracking-tight">
            ₹{balance.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
          </h1>
          ...
          <div className="flex justify-between gap-2 mt-4 mb-5 border-b border-app-border-hover pb-4">
            <div className="flex flex-col">
              ...
              <span className="font-bold text-app-text text-sm">
                ₹
                {Number(wallet.deposit || 0).toLocaleString("en-IN", {
                  maximumFractionDigits: 0,
                })}
              </span>
            </div>
            ...
            <div className="flex flex-col">
              ...
              <span className="font-bold text-[#FFD700] text-sm">
                ₹
                {Number(wallet.bonus || 0).toLocaleString("en-IN", {
                  maximumFractionDigits: 0,
                })}
              </span>
              {wallet.bonus > 0 && wallet.bonusExpiry && (
                <span className="text-[9px] text-[#FFD700]/70 mt-0.5">
                  Expires: {new Date(wallet.bonusExpiry).toLocaleDateString()}
                </span>
              )}
            </div>
          </div>
        ...
        <h3 className="font-bold text-app-text mt-6 mb-2 px-1">
          Recent Transactions
        </h3>
        <div className="space-y-3">
          {(() => {
            const ud = depositRequests
              .filter((r) => r.userId === user?.id)
              .map((r) => ({
                ...r,
                t: "d",
                num: parseInt(r.id.split("_")[1] || "0"),
              }));
            const uw = withdrawRequests
              .filter((r) => r.userId === user?.id)
              .map((r) => ({
                ...r,
                t: "w",
            ...
            if (allTx.length === 0) {
              return (
                <p className="text-sm font-bold text-app-text-muted p-4 border border-app-border bg-app-card rounded-xl flex items-center justify-center gap-2">
            ...
            return allTx.map((req) => (
              <div
                key={req.id}
                className="bg-app-card rounded-xl p-4 border border-app-border flex justify-between items-center"
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`p-2 rounded-full ${req.status === "Approved" ? (req.t === "d" ? "bg-[#153B25] text-[#4ADE80]" : "bg-red-900/30 text-app-accent") : req.status === "Rejected" ? "bg-red-900/30 text-app-accent" : "bg-orange-900/30 text-orange-500"}`}
                  >
                    {req.status === "Approved" ? (
                      <Check size={20} />
                    ) : req.status === "Rejected" ? (
                  ...
                  <div className="flex flex-col">
                    <p className="font-bold text-sm text-app-text">
                      {req.status === "Rejected"
                        ? "Payment Failed"
                        : req.status === "Pending"
                          ? `Pending ${req.t === "d" ? "Deposit" : "Withdrawal"}`
                          : req.t === "d"
                            ? "Added to wallet"
                            : "Withdrawn"}
                    </p>
                ...
                <span
                  className={`font-bold text-lg ${req.status === "Approved" ? (req.t === "d" ? "text-[#4ADE80]" : "text-app-text") : req.status === "Rejected" ? "text-app-accent opacity-80" : "text-orange-500"}`}
                >
                  {req.status === "Rejected"
                    ? "Failed"
                    : `${req.t === "d" ? "+" : "-"}₹${req.amount}`}
          ...
          <div className="bg-app-card w-full rounded-t-3xl p-6 shadow-2xl pb-safe pt-8 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6 border-b border-app-border pb-4">
              ...
              <button
                ...
                className="text-app-text-muted p-2"
              >
                <Minus size={20} />
              </button>
            </div>
            ...
            {!paymentMethod ? (
              <>
                <label className="text-xs font-bold text-app-text-muted mb-2 block">
                  Amount to Add (₹)
                </label>
                <input
                  type="number"
                  ...
                  className="w-full text-2xl font-black text-app-text border-2 border-app-border rounded-xl py-3 px-4 mb-4 outline-none focus:border-app-accent text-center"
                />
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                  {[100, 500, 1000, 2000].map((amt) => (
                    <button
                      ...
                      className="bg-app-bg text-app-text font-bold text-xs py-2 rounded-lg border border-app-border active:bg-app-card-hover"
                    >
                      +₹{amt}
                    </button>
                  ))}
                ...
                <p className="text-[10px] font-bold text-app-text-muted uppercase mb-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="w-full bg-app-card-inner border border-app-border hover:border-blue-500 rounded-xl shadow-sm flex items-center active:bg-app-bg text-left overflow-hidden h-10"
                  >
                    <div className="w-[45px] h-full bg-white flex items-center justify-center shrink-0 border-r border-app-border p-1.5">
                      <img
                    ...
                    <div className="flex-1 px-2 text-app-text font-bold text-[11px] truncate">
                      GPay
                    </div>
                  </button>
                  <button
                    ...
                    className="w-full bg-app-card-inner border border-app-border hover:border-purple-500 rounded-xl shadow-sm flex items-center active:bg-app-bg text-left overflow-hidden h-10"
                  >
                    <div className="w-[45px] h-full bg-white flex items-center justify-center shrink-0 border-r border-app-border p-1.5">
                      <img
                  ...
                  <button
                    ...
                    className="w-full bg-app-card-inner border border-app-border hover:border-blue-400 rounded-xl shadow-sm flex items-center active:bg-app-bg text-left overflow-hidden h-10"
                  >
                    <div className="w-[45px] h-full bg-white flex items-center justify-center shrink-0 border-r border-app-border p-1.5">
                      <img
                  ...
                  <button
                    ...
                    className="w-full bg-app-card-inner border border-app-border hover:border-orange-500 rounded-xl shadow-sm flex items-center active:bg-app-bg text-left overflow-hidden h-10"
                  >
                    <div className="w-[45px] h-full bg-white flex items-center justify-center shrink-0 border-r border-app-border p-1.5">
                      <img
                ...
                <div className="mt-4 bg-[#e5c158]/10 border border-[#e5c158]/30 rounded-xl p-3 flex gap-2.5 text-app-text-muted text-[11px]">
                  <Info className="text-[#e5c158] shrink-0 mt-0.5" size={14} />
                  <p>
                    Maximum deposit time is{" "}
                    <strong className="text-[#e5c158]">30 minutes</strong>. If
                    your deposit is not credited within this time, please
              ...
              <div className="animate-in fade-in slide-in-from-right-4">
                <p className="text-sm text-app-text-muted mb-2">
                  You selected <strong>{paymentMethod}</strong> to pay ₹
                  {paymentAmount}
                </p>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  <p className="font-bold text-blue-800 text-xs uppercase mb-1">
                    Step 1: Pay to this UPI ID
                  </p>
                  <div className="flex flex-col items-center justify-center gap-2 mb-2">
                    {adminUpiQR && (
                      <img
                        ...
                        onError={(e) => {
                          e.currentTarget.src =
                            "https://placehold.co/150x150?text=Invalid+Image+URL";
                        }}
                        alt="UPI QR Code"
                        className="w-[150px] h-[150px] object-cover rounded-xl border-2 border-blue-500 shadow-sm"
                      />
                    )}
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-lg font-mono font-black text-app-text tracking-wide">
                      {adminUPI || "admin@ybl"}
                    </p>
                    <button
                ...
                <div className="mb-4">
                  <p className="font-bold text-app-text text-xs uppercase mb-2">
                ...
                <div className="mb-6">
                  ...
                  {depositScreenshot && (
                    <img
                      ...
                      className="h-16 mt-2 rounded border border-app-border object-contain"
                    />
                  )}
                  <p className="text-[10px] text-app-text-muted mt-1">
                    Upload the successful payment screenshot
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    disabled={isScanningPayment}
                    onClick={() => {
                      if (paymentUtr.length < 8)
                        return alert("Please enter a valid UTR number.");
  ...
  const renderProfile = () => {
    const userTeams = savedTeams.filter(
      (t) => t.userId === (user?.id || "guest"),
    );
    ...
    const claimReward = (levelIndex: number) => {
      ...
      alert(
        `🎉 Congratulations! You received ₹${rewardAmount} for reaching Level ${levelIndex}!`,
      );
    };

    const totalWins = wallet?.wins || 0;
    const winningRate =
      contestsJoined > 0 ? Math.round((totalWins / contestsJoined) * 100) : 0;
        ...
        <div className="p-4 flex-1 overflow-y-auto pb-20">
          <div className="bg-app-accent rounded-xl shadow-sm p-5 flex flex-col mb-4 text-white">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col text-app-text">
                ...
                <p className="text-sm opacity-90 mb-2 font-mono">
                  ID: {user?.numericId || "Loading..."}
                </p>
                <div className="flex flex-col gap-1 w-full">
                  <div className="flex gap-2 items-center">
                    <span className="bg-yellow-500 text-black text-[10px] px-2 py-0.5 rounded flex items-center font-bold w-fit">
                      ⚡ Level {userLevel}
                    </span>
                    <span className="text-xs font-bold text-white/90">
                      {contestsJoined % 10} / 10 to Lvl {userLevel + 1}
                    </span>
                  </div>
                  <div className="w-full bg-black/30 h-1.5 rounded-full overflow-hidden mt-1">
                    <div
                      className="bg-yellow-500 h-full rounded-full"
                      style={{ width: `${(contestsJoined % 10) * 10}%` }}
                    ></div>
                  </div>
                </div>
              </div>
          ...
          {unclaimedRewards.length > 0 && (
            <div className="bg-[#e5c158]/10 border border-[#e5c158]/30 rounded-xl p-4 mb-4 flex flex-col gap-3">
              <h3 className="font-bold text-[#e5c158] flex items-center gap-2 text-sm">
                🎁 Milestone Rewards!
              </h3>
              {unclaimedRewards.map((level) => {
                ...
                return (
                  <div
                    key={level}
                    className="flex justify-between items-center bg-app-card-inner border border-app-border p-2 px-3 rounded-lg"
                  >
                    ...
                    <button
                      ...
                      className="bg-yellow-500 text-black px-3 py-1.5 rounded font-bold text-xs active:scale-95 transition-transform uppercase shadow-sm"
                    >
                      Claim ₹{rewardAmount}
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="bg-app-card rounded-xl flex-1 p-3 flex flex-col items-center justify-center border border-app-border shadow-sm">
              ...
              <span className="text-[10px] text-app-text-muted mt-1 text-center font-medium uppercase tracking-wider">
                Matches
                <br />
                Played
              </span>
            ...
            <div className="bg-app-card rounded-xl flex-1 p-4 flex flex-col items-center justify-center border border-app-border shadow-sm col-span-2">
              <span className="text-xs text-app-text-muted mb-2 text-center font-bold uppercase tracking-wider">
                Total Profit Earned
              </span>
          ...
          <div className="bg-app-card rounded-xl shadow-sm border border-app-border overflow-hidden mb-6">
            ...
            <div className="grid grid-cols-3 divide-x divide-app-border">
              <div className="p-3 flex flex-col items-center justify-center hover:bg-app-card-hover transition-colors">
                <span className="text-app-text-muted text-[10px] font-bold uppercase tracking-wider mb-1">
                  Deposit
                </span>
          ...
          <div className="bg-app-card rounded-xl shadow-sm border border-app-border overflow-hidden flex flex-col">
            <button
              onClick={() => setView("KYC")}
              className="p-4 text-left border-b border-app-border flex items-center gap-4 active:bg-app-card-hover transition-colors"
            >
              <Shield className="text-app-text-muted" size={22} />
              <div className="flex-1">
                <div className="text-app-text font-bold text-sm">
                  Complete KYC
                </div>
              ...
              <div className="flex items-center gap-2">
                {kycStatus === "Approved" ? (
                  <span className="bg-[#153B25] text-[#4ADE80] text-[10px] px-2 py-1 rounded font-bold">
                    VERIFIED
                  </span>
                ) : kycStatus === "Pending Review" ? (
                  <span className="bg-orange-900/30 text-orange-500 text-[10px] px-2 py-1 rounded font-bold">
                    PENDING
                  </span>
                ) : null}
            ...
            <button
              onClick={() => setShowFairPlayPolicy(true)}
              className="p-4 text-left border-b border-app-border flex items-center gap-4 active:bg-app-card-hover transition-colors"
            >
          ...
          <div className="bg-app-card-inner rounded-xl shadow-sm border border-app-border overflow-hidden flex flex-col mb-6 mt-6">
            <div className="p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-xs font-bold text-app-text-muted uppercase w-16">
                  Mode
                </span>
                <button
                  onClick={() => setThemeMode("Dark")}
                  className={`flex-1 py-1.5 rounded text-xs font-bold border transition-colors ${themeMode === "Dark" ? "bg-app-accent/20 text-app-accent border-app-accent/30" : "bg-app-card text-app-text-muted border-app-border"}`}
                >
                  Dark
                </button>
                <button
                  onClick={() => setThemeMode("Light")}
                  className={`flex-1 py-1.5 rounded text-xs font-bold border transition-colors ${themeMode === "Light" ? "bg-app-accent/20 text-app-accent border-app-accent/30" : "bg-app-card text-app-text-muted border-app-border"}`}
                >
                  White
                </button>
              </div>
              <div className="flex items-center gap-2 mt-1">
                ...
                <button
                  ...
                  className={`flex-1 py-1.5 rounded text-xs font-bold border transition-colors flex items-center justify-center gap-1 ${themeColor === "Red" ? "bg-red-500/20 text-red-500 border-red-500/30" : "bg-app-card text-app-text-muted border-app-border"}`}
                >
                  <div className="w-2 h-2 rounded-full bg-red-500"></div> Red
                </button>
                <button
                  onClick={() => setThemeColor("Blue")}
                  className={`flex-1 py-1.5 rounded text-xs font-bold border transition-colors flex items-center justify-center gap-1 ${themeColor === "Blue" ? "bg-blue-500/20 text-blue-500 border-blue-500/30" : "bg-app-card text-app-text-muted border-app-border"}`}
                >
                  <div className="w-2 h-2 rounded-full bg-blue-500"></div> Blue
                </button>
                <button
                  onClick={() => setThemeColor("Green")}
                  className={`flex-1 py-1.5 rounded text-xs font-bold border transition-colors flex items-center justify-center gap-1 ${themeColor === "Green" ? "bg-green-500/20 text-green-500 border-green-500/30" : "bg-app-card text-app-text-muted border-app-border"}`}
                >
  ...
  const renderKyc = () => {
    ...
    const handleKycSubmit = () => {
      ...
      setDoc(doc(db, "kyc", newReq.id), newReq).then(() => {
        alert(
          "KYC Details Submitted successfully! Admin will verify your documents.",
        );
      });
    };
    ...
    return (
      <div className="flex flex-col h-full bg-app-bg">
        <header className="p-4 flex items-center justify-between pb-2 bg-app-bg">
          ...
          <button
            onClick={() => setView("PROFILE")}
            className="text-app-text-muted absolute left-4"
          >
            <ArrowLeft />
          </button>
        </header>
        ...
        <div className="p-4 flex-1 overflow-y-auto pb-20">
          {isSubmitted ? (
            <div className="bg-app-card p-6 rounded-xl border border-app-border flex flex-col items-center text-center mt-8">
              <div
                className={`w-20 h-20 rounded-full flex items-center justify-center mb-4 ${userKycObj.status === "Approved" ? "bg-[#153B25] text-[#4ADE80]" : userKycObj.status === "Rejected" ? "bg-red-900/30 text-app-accent" : "bg-orange-900/30 text-orange-500"}`}
              >
                {userKycObj.status === "Approved" ? (
                  <Check size={40} />
              ...
              <p className="text-sm text-app-text-muted">
                {userKycObj.status === "Approved"
                  ? "Your KYC is approved! You can now withdraw funds."
                  : userKycObj.status === "Rejected"
                    ? "Your KYC was rejected. Please contact support or try again later."
                    : "Your documents have been submitted and are under review by our admin team."}
              </p>
              {userKycObj.status === "Rejected" && (
                <button
                  onClick={async () => {
                    await setDoc(doc(db, "kyc", userKycObj.id), {
                      ...userKycObj,
                      status: "Archived",
                    }); // Or delete: await deleteDoc(doc(db, 'kyc', userKycObj.id))
                  }}
            ...
            <div className="bg-app-card rounded-xl border border-app-border p-5 mt-4">
              <div className="flex items-center gap-3 border-b border-app-border pb-4 mb-4">
                ...
                <div className="flex flex-col">
                  <h3 className="text-app-text font-bold">
                    Identity Verification
                  </h3>
                  <p className="text-xs text-app-text-muted">
                    Required to withdraw your winnings
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    value={aadharInput}
                    onChange={(e) =>
                      setAadharInput(e.target.value.substring(0, 12))
                    }
                ...
                {aadharInput.length === 12 && (
                  <div className="flex flex-col gap-3 py-2 animate-in fade-in zoom-in-95 duration-300">
                    <label className="text-xs font-bold text-app-accent uppercase tracking-wider">
                      Aadhar Front Image
                    </label>
                    <input
                      type="file"
                    ...
                    {aadharFrontImg && (
                      <img
                        src={aadharFrontImg}
                        alt="Aadhar Front"
                        className="h-20 object-contain rounded border border-app-border"
                      />
                    )}

                    <label className="text-xs font-bold text-app-accent uppercase tracking-wider mt-2">
                      Aadhar Back Image
                    </label>
                    <input
                      type="file"
      ...
      try {
        if (authMode === "SIGNUP") {
          ...
          if (/^\d{10}$/.test(loginEmail)) {
            ...
            loginEmail =
              snap.docs[0].data().email || `${loginEmail}@dreamapp.com`;
          } else {
            setAuthLoading(false);
      ...
      return (
        ...
        >
          <div className="flex-1 flex flex-col items-center justify-center p-8">
            ...
            <h1 className="text-2xl font-black mb-2 text-center text-app-text">
              Account Created!
            </h1>
            <p className="text-sm font-bold text-app-text-muted text-center mb-8">
              Please copy your ID and Password to login. Do not lose them!
            </p>
    ...
    return (
      ...
      >
        <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-y-auto">
          ...
          <h1 className="text-3xl font-black tracking-tight italic mb-1 text-app-text">
            Fantasy<span className="text-app-accent">11</span>
          </h1>
          <p className="text-app-text-muted font-bold mb-8 text-center text-[10px] uppercase tracking-widest leading-none">
            Play Fantasy Sports & Win Cash Prizes!
          </p>
          ...
          <div className="w-full">
            ...
            <div className="text-center mt-6">
              ...
              >
                {authMode === "LOGIN" ? (
                  <>
                    New here?{" "}
                    <span className="text-app-accent group-hover:underline ml-1">
                      Create an account
                    </span>
                  </>
                ) : (
            ...
            <div className="relative flex items-center py-6">
              ...
              <span className="flex-shrink-0 mx-4 text-app-text-muted text-[10px] font-black uppercase tracking-widest">
                OR
              </span>
              <div className="flex-grow border-t border-app-border"></div>
            </div>
            ...
            <button
              onClick={(e) => {
                ...
                if (!auth) {
                  ...
                  alert(
                    "Database integration is under configuration... please try again shortly!",
                  );
                  return;
                }
                ...
                signInWithPopup(auth, provider)
                  .catch((error: any) => {
                    ...
                    if (error.code === "auth/unauthorized-domain") {
                      ...
                      console.warn(
                        "Popup blocked or closed. Attempting redirect login fallback...",
                      );
                      alert(
                        "Browser ne popup ko block kar diya hai! Agar login nahi ho raha hai to Google Login par dobara click karein ya apne browser ke popup blocker ko band karke try karein.",
                      );
              ...
              onClick={async () => {
                try {
                  ...
                  if (err.code === "auth/operation-not-allowed") {
                    alert(
                      "Email/Password Sign-In is not enabled! Please go to your Firebase Console -> Authentication -> Sign-in Method, and enable 'Email/Password'.",
                    );
                  } else {
          ...
          <p className="text-[10px] text-app-text-muted text-center mt-12 px-6 leading-relaxed">
            ...
            <span className="text-app-text font-bold">Terms of Service</span>{" "}
            and <span className="text-app-text font-bold">Privacy Policy</span>.
            Responsible gaming only.
          </p>
        </div>
        ...
        {showPhonePermissionDialog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-app-card rounded-2xl w-full max-w-sm p-6 shadow-2xl border border-app-border animate-in fade-in zoom-in duration-200">
              ...
              <h3 className="text-xl font-black text-center text-app-text mb-2 tracking-tight">
                Phone Access Required
              </h3>
              <p className="text-sm text-center text-app-text-muted mb-6 leading-relaxed">
                Fantasy11 needs permission to access your phone state and SMS to
                securely send and verify your OTP for login.
              </p>
  ...
  if (!authInitialized) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-[#0d1321] text-[#e5c158]">
        ...
        <p className="mt-4 font-bold text-xs uppercase tracking-widest animate-pulse">
          Initializing...
        </p>
      </div>
    );
  ...
  const syncCategoryToCloud = async (
    key: string,
    data: any[],
    itemsPerChunk = 50,
  ) => {
    ...
    try {
      ...
      if (metaSnap.exists()) {
        ...
        if (prevCount > chunks.length) {
          ...
          for (let i = chunks.length; i < prevCount; i++) {
            deletePromises.push(
              deleteDoc(doc(db, "gameData", `${key}_chunk_${i}`)).catch(
                () => {},
              ),
            );
          }
      ...
      await setDoc(doc(db, "gameData", "sync_meta"), {
        lastUpdate: ts,
        type: key,
      });
    } catch (e: any) {
  ...
  const syncBotsOnlyToCloud = async () => {
    ...
    try {
      const ts = Date.now();
      const adminTeams = savedTeams.filter(
        (t) => t.userId === "admin_bot" || t.userId === "admin_bot_boot",
      );
      ...
      if (e.message?.includes("quota")) {
        throw new Error(
          "Firestore free limit reached for today. Please wait for quota to reset.",
        );
      }
      throw e;
    }
  ...
  const syncActiveDataToCloud = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "gameData", "banners"), {
        data: JSON.parse(JSON.stringify(appBanners)),
        timestamp: ts,
      });
  ...
  const handleSyncToCloud = async () => {
    ...
    try {
      ...
      setSyncMessage({
        type: "success",
        text: "✅ Successfully synced all data to cloud.",
      });
    } catch (e: any) {
      console.error(e);
      ...
      if (errorMsg.includes("payload size exceeds the limit")) {
        errorMsg =
          "Data size issue. Automated cleaning of old data recommended. (डेटा बहुत बड़ा है, पुराना डेटा ऑटोमैटिक डिलीट करने की सलाह दी जाती है)";
      }
      ...
      <div className="flex flex-col h-full bg-[#090b10] text-slate-200 font-sans">
        <header className="bg-[#13151c] border-b border-[#e5c158]/20 p-4 shrink-0 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              <h2 className="font-black text-transparent bg-clip-text bg-gradient-to-r from-[#e5c158] to-[#f0b90b] uppercase tracking-widest text-lg drop-shadow-[0_0_12px_rgba(229,193,88,0.4)]">
                VIP
              </h2>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={async () => {
                  try {
                    ...
                    setWithdrawRequests(
                      wdSnap.docs.map((d) => d.data() as any),
                    );
                    setDepositRequests(
                      depSnap.docs.map((d) => d.data() as any),
                    );
                ...
                id="adminRefreshBtn"
                className="w-9 h-9 rounded-xl bg-[#e5c158]/10 border border-[#e5c158]/30 flex items-center justify-center text-[#e5c158] overflow-hidden shadow-[0_0_10px_rgba(229,193,88,0.2)] hover:bg-[#e5c158]/20 transition-all cursor-pointer"
              >
                <RefreshCw size={18} />
              </button>
              <button
        ...
        <div className="flex-1 p-4 overflow-y-auto bg-[#090b10]">
            ...
            <>
                ...
                <div className="mb-6">
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl mb-6">
                    <p className="text-xs text-yellow-200 font-medium leading-relaxed">
                      ...
                      Use the{" "}
                      <span className="font-black text-green-400 uppercase">
                        "Update Apps & Player"
                      </span>{" "}
                      button in the top header to instantly sync matches,
                      contests, and teams to all players' devices.
                    </p>
                  </div>
                  {syncMessage && (
                    <div
                      className={`mt-3 p-3 text-sm rounded-lg ${syncMessage.type === "success" ? "bg-green-900/50 text-green-400 border border-green-500/50" : "bg-red-900/50 text-red-400 border border-red-500/50"}`}
                    >
                      {syncMessage.text}
                    </div>
                  )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                ...
                <div className="lg:col-span-5 flex flex-col gap-5">
                  ...
                  <div className="bg-[#13151c] rounded-2xl border border-[#e5c158]/20 p-5 shadow-lg flex-1">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-6">
                      <div className="p-2 -m-2 rounded transition-colors">
                        ...
                        <p className="text-lg xl:text-xl font-bold text-slate-200 flex items-center gap-1">
                          {adminUserList.length}{" "}
                          <User size={14} className="text-[#e5c158]" />
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider mb-1">
                        ...
                        <p className="text-lg xl:text-xl font-bold text-slate-200 flex items-center gap-1">
                          {appContests.length + 3200}{" "}
                          <ChevronUp size={14} className="text-green-500" />
                        </p>
                      </div>
                      <button
                        onClick={() => setAdminTab("FINANCIALS")}
                        className="text-left group transition-all hover:bg-yellow-500/10 p-2 -m-2 rounded"
                      >
                        <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider mb-1">
                          Pending Deposits
                        </p>
                        <p className="text-lg xl:text-xl font-bold text-slate-200 flex items-center gap-1">
                          ...
                          <span className="text-[10px] text-yellow-500 bg-yellow-500/20 px-1 py-0.5 rounded">
                            Action Req
                          </span>
                        </p>
                      </button>
                  ...
                  <div className="bg-[#13151c] rounded-2xl border border-[#e5c158]/20 p-5 shadow-lg">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <button
                        onClick={() => setAdminTab("FINANCIALS")}
                        className="px-3 py-1 rounded bg-[#090b10] border border-slate-700 text-xs text-slate-400 flex items-center gap-1 hover:border-[#e5c158]/50 transition-colors"
                      >
                        View Finance <ChevronDown size={12} />
                      </button>
                    </div>
                    {(() => {
                      let totalRev = 0;
                      depositRequests.forEach((d) => {
                        if (d.status === "Approved")
                          totalRev += parseFloat(d.amount?.toString() || "0");
                      });

                      let totalPay = 0;
                      withdrawRequests.forEach((w) => {
                        if (w.status === "Approved")
                          totalPay += parseFloat(w.amount?.toString() || "0");
                      });
                      ...
                      return (
                        <div className="grid grid-cols-3 gap-2 xl:gap-4">
                          <div>
                            <p className="text-slate-400 text-[9px] xl:text-[10px] uppercase font-bold tracking-wider mb-1">
                              Total Revenue
                            </p>
                            <p className="text-base xl:text-lg font-black text-slate-200 mb-3">
                              {formatFinance(totalRev)}
                            </p>
                          ...
                          <div>
                            ...
                            <p className="text-base xl:text-lg font-black text-slate-200 mb-3 flex items-center gap-1">
                              {formatFinance(totalPay)}{" "}
                              {totalPay > 0 ? (
                ...
                <div className="lg:col-span-4 bg-[#13151c] rounded-2xl border border-[#e5c158]/20 p-5 shadow-lg flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded bg-[#090b10] border border-slate-700 text-xs text-slate-400 flex items-center gap-1 hover:border-[#e5c158]/50 transition-colors"
                    >
                      Go to Matches <ChevronDown size={12} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex flex-col items-center gap-2 cursor-pointer bg-[#e5c158]/10 border border-[#e5c158]/30 px-3 py-2 rounded-xl">
                      ...
                      <span className="text-[10px] xl:text-xs text-[#e5c158] font-bold">
                        Cricket
                      </span>
                    </div>
                    <div className="flex flex-col items-center gap-2 cursor-pointer hover:bg-slate-800 px-3 py-2 rounded-xl transition-all">
                      ...
                      <span className="text-[10px] xl:text-xs text-slate-400 font-bold">
                        Football
                      </span>
                    </div>
                    <div className="flex flex-col items-center gap-2 cursor-pointer hover:bg-slate-800 px-3 py-2 rounded-xl transition-all">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] xl:text-xs text-[#e5c158] font-bold uppercase tracking-widest mb-3 border-b border-slate-800 pb-2">
                      Upcoming Matches
                    </p>
                    ...
                    {appMatches
                      ...
                      .map((m, i) => (
                        <div
                          key={i}
                          className="flex items-center justify-between bg-[#090b10] border border-slate-800 rounded-xl p-2 xl:p-3 mb-3"
                        >
                          <div className="flex gap-2 xl:gap-3 items-center">
                            <div className="flex gap-1 items-center">
                              ...
                              <span className="text-[8px] xl:text-[10px] text-slate-400">
                                VS
                              </span>
                              <div className="w-5 h-5 xl:w-6 xl:h-6 rounded-full overflow-hidden">
                                <img
                            ...
                            <div>
                              <p className="text-[10px] xl:text-xs font-bold text-slate-200">
                                {m.seriesName}
                              </p>
                              <p className="text-[8px] xl:text-[10px] text-slate-500">
                                {m.time}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="text-[#e5c158] font-bold text-xs xl:text-sm">
                              Upcoming
                            </p>
                          </div>
                        </div>
                ...
                <div className="lg:col-span-3 flex flex-col gap-5">
                        ...
                        .map((tu, i) => (
                          <div
                            ...
                            className={
                              ...
                              (i === 0
                                ? "bg-[#e5c158]/5 border-[#e5c158]/30 shadow-[0_0_10px_rgba(229,193,88,0.1)]"
                                : "border-slate-800 bg-[#090b10] hover:bg-slate-800")
                            }
                          >
                            <div className="flex items-center gap-2 xl:gap-3 overflow-hidden flex-1 mr-2">
                              <span className="text-slate-500 font-mono text-xs w-3 xl:w-4">
                                {i + 1}.
                              </span>
                              <div
                                onClick={() => {
                                ...
                                className="w-8 h-8 xl:w-10 xl:h-10 rounded-full bg-slate-800 border border-slate-700 overflow-hidden shrink-0 cursor-pointer hover:scale-105 transition-transform"
                                title="Open Profile"
                              >
                                <img
                                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${tu.id}`}
                                  alt="avatar"
                                  className="w-full h-full object-cover"
                                />
                              </div>
                              <div className="min-w-0">
                                <p className="text-[10px] xl:text-xs font-bold text-slate-200 leading-tight truncate">
                                  {tu.name ||
                                    (tu.email &&
                                      String(tu.email).split("@")[0]) ||
                                    "User"}
                                </p>
                                <div className="flex gap-2 items-center mt-1">
                                  <p className="text-[8px] xl:text-[9px] text-slate-500 truncate font-mono">
                                    {String(tu.id).substring(0, 8)}...
                                  </p>
                                </div>
                              </div>
                            </div>
                            <span className="text-[#e5c158] font-bold text-xs xl:text-sm shrink-0 pl-2">
                              ₹{Number(tu.winning || 0).toLocaleString()}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="bg-[#13151c] rounded-2xl border border-[#e5c158]/20 p-5 shadow-lg flex-1">
                    <h3 className="text-lg font-bold text-slate-200 tracking-wide mb-6">
                      Recent Support Tickets
                    </h3>
                    <div className="space-y-4">
                      {[1, 2, 4].map((i) => (
                        <div
                          key={i}
                          className="border-b border-slate-800 pb-3 last:border-0 last:pb-0 cursor-pointer group"
                        >
                          <p className="text-xs xl:text-sm font-bold text-slate-300 group-hover:text-white transition-colors">
                            Recent Subject Tick-00{i}
                          </p>
                          <p className="text-[8px] xl:text-[10px] text-slate-500 mt-1">
                            {i}. Subject Ticket-00{i} • 1 hour ago
                          </p>
                        </div>
                      ))}
                    </div>
          ...
          {adminTab === "SETTINGS" && (
            <>
              <h3 className="font-bold text-[#e5c158]/80 uppercase tracking-widest text-[10px] mb-3 ml-1 mt-6">
              ...
              <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mb-6 relative overflow-hidden backdrop-blur-sm">
                <div className="flex justify-between items-center mb-2">
                  <p className="font-bold text-slate-200">
                    Winning Distribution Rate
                  </p>
                  <p className="text-xl font-black text-[#e5c158] drop-shadow-[0_0_8px_rgba(234,179,8,0.4)]">
                    {winningPercentage}%
                  </p>
                </div>
                <p className="text-xs text-slate-400 mb-4">
                  Set the percentage of the dynamic prize pool that goes to the
                  winners. The rest ({100 - winningPercentage}%) is kept as
                  platform fee.
                </p>
                ...
                <div className="flex gap-2 items-center">
                  <input
                    ...
                    onChange={(e) =>
                      setWinningPercentage(parseInt(e.target.value))
                    }
              ...
              <h3 className="font-bold text-[#e5c158]/80 uppercase tracking-widest text-[10px] mb-3 ml-1">
                System Control: Deposit Gateway
              </h3>
              <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mb-6 relative overflow-hidden backdrop-blur-sm">
                <p className="text-xs text-slate-400 mb-4">
                  Set your UPI ID and QR Code image URL so users can make
                  deposits. The app will auto-scan users payments.
                </p>

                <div className="mb-4">
                  <label className="text-xs font-bold text-[#e5c158]/80 uppercase tracking-widest mb-1.5 block">
                    Your UPI ID
                  </label>
                  <div className="flex gap-2">
                    <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (file) {
                        ...
                        try {
                          ...
                          setDoc(
                            doc(db, "gameData", "settings"),
                            { adminUPI, adminUpiQR: compressed },
                            { merge: true },
                          ).then(() => alert("QR Code uploaded and saved!"));
                        } catch (err) {
                          alert("Failed to process image.");
                        }
                      }
                ...
                {adminUpiQR && (
                  <div className="mt-2 text-center bg-black/30 p-4 rounded-xl border border-yellow-500/10">
                    <img
                      ...
                      alt="QR Code Preview"
                      className="w-[100px] h-[100px] object-cover rounded-xl shadow-[0_0_15px_rgba(234,179,8,0.15)] inline-block border-2 border-[#e5c158]/50"
                    />
                  </div>
                )}
              </div>
          ...
          {adminTab === "BANNERS" && (
            <>
              <h3 className="font-bold text-[#e5c158]/80 uppercase tracking-widest text-[10px] mb-3 ml-1 mt-6">
                System Control: App Banners
              </h3>
              <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mb-6 relative overflow-hidden backdrop-blur-sm">
                <p className="text-xs text-slate-400 mb-4">
                  Set the promotional banners that appear at the top of the user
                  home screen. Upload 16:9 images for best results.
                </p>

                {appBanners.map((banner, index) => (
                  <div
                    key={banner.id}
                    className="mb-4 bg-black/30 p-3 rounded-xl border border-slate-700/50"
                  >
                    <div className="flex justify-between items-center mb-2">
                    ...
                    {banner.imageUrl ? (
                      <div className="relative w-full h-32 rounded-lg overflow-hidden border border-slate-700 bg-black mb-3 group">
                        ...
                        <div
                          ...
                          onClick={() =>
                            document
                              .getElementById(`banner-upload-${banner.id}`)
                              ?.click()
                          }
                        >
                      ...
                      <div
                        className="w-full h-32 rounded-lg border-2 border-dashed border-slate-700 relative flex flex-col items-center justify-center cursor-pointer hover:border-[#e5c158]/50 hover:text-[#e5c158] text-slate-500 transition-colors bg-black/50 mb-3"
                        onClick={() =>
                          document
                            .getElementById(`banner-upload-${banner.id}`)
                            ?.click()
                ...
                <button
                  onClick={() =>
                    setAppBanners((prev) => [
                      ...prev,
                      { id: "b" + Date.now(), imageUrl: "", linkUrl: "" },
                    ])
                  }
                  className="w-full py-2.5 rounded-xl border border-dashed border-[#e5c158]/50 text-[#e5c158] bg-[#e5c158]/5 hover:bg-[#e5c158]/10 text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 mb-4"
                >
                ...
                <button
                  onClick={() => {
                    const ts = Date.now();
                    Promise.all([
                      setDoc(doc(db, "gameData", "banners"), {
                        data: JSON.parse(JSON.stringify(appBanners)),
                        timestamp: ts,
                      }),
                      setDoc(doc(db, "gameData", "sync_meta"), {
                        lastUpdate: ts,
                      }),
                    ])
                      .then(() =>
                        alert("Banners saved to server successfully!"),
                      )
                      .catch((e) => alert("Failed to save banners: " + e));
                  }}
                  className="w-full py-3 bg-[#e5c158] text-black rounded-xl font-bold uppercase tracking-widest text-[11px] shadow-[0_0_15px_rgba(229,193,88,0.4)] transition-all active:scale-95"
                >
                  Push Banners Live
                </button>
              </div>
          ...
          {adminTab === "CONTESTS" && (
            <>
              {!adminContestDashboard && (
                <button
                  onClick={() => setAdminContestDashboard("MANAGE_CONTESTS")}
                ...
                >
                  ...
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
                    <Trophy size={16} className="text-[#e5c158] mr-2" /> Manage
                    Contests
                  </h3>
                </button>
              )}

              {adminContestDashboard === "MANAGE_CONTESTS" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-base font-black text-slate-100 flex items-center gap-2">
                        <Trophy size={16} className="text-[#e5c158]" /> Manage
                        Contests
                      </h2>
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                      Create new public Mega Contests or Head to Head matches
                      with custom fees and rewards.
                    </p>
                    <div className="space-y-4 relative z-10">
                      <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl">
                        <label className="text-[10px] font-black text-[#e5c158]/70 uppercase tracking-widest block mb-2">
                          Contest Type
                        </label>
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={() => setAdminContestType("Mega")}
                            className={`flex-1 min-w-[140px] py-2.5 rounded-lg text-sm font-bold border transition-all ${adminContestType === "Mega" ? "bg-[#e5c158]/20 text-[#e5c158] border-[#e5c158]/50 shadow-[0_0_15px_rgba(234,179,8,0.15)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
                          >
                            Mega Contest
                          </button>
                          <button
                            onClick={() => setAdminContestType("H2H")}
                            className={`flex-1 min-w-[140px] py-2.5 rounded-lg text-sm font-bold border transition-all ${adminContestType === "H2H" ? "bg-[#e5c158]/20 text-[#e5c158] border-[#e5c158]/50 shadow-[0_0_15px_rgba(234,179,8,0.15)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
                          >
                            Head to Head (2 Spots)
                          </button>
                          <button
                            onClick={() => setAdminContestType("H2H_3")}
                            className={`flex-1 min-w-[140px] py-2.5 rounded-lg text-sm font-bold border transition-all ${adminContestType === "H2H_3" ? "bg-[#e5c158]/20 text-[#e5c158] border-[#e5c158]/50 shadow-[0_0_15px_rgba(234,179,8,0.15)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
                          >
                            3 Spots (3 to 1)
                          </button>
                          <button
                            onClick={() => setAdminContestType("H2H_4")}
                            className={`flex-1 min-w-[140px] py-2.5 rounded-lg text-sm font-bold border transition-all ${adminContestType === "H2H_4" ? "bg-[#e5c158]/20 text-[#e5c158] border-[#e5c158]/50 shadow-[0_0_15px_rgba(234,179,8,0.15)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
                          >
                            4 Spots (4 to 1)
                          </button>
                          <button
                            onClick={() => setAdminContestType("H2H_5")}
                            className={`flex-1 min-w-[140px] py-2.5 rounded-lg text-sm font-bold border transition-all ${adminContestType === "H2H_5" ? "bg-[#e5c158]/20 text-[#e5c158] border-[#e5c158]/50 shadow-[0_0_15px_rgba(234,179,8,0.15)]" : "bg-black/50 text-slate-400 border-slate-700 hover:border-slate-500"}`}
                          >
                            5 Spots (5 to 1)
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl">
                          ...
                          <input
                            type="text"
                            placeholder="e.g., Grand League 1"
                            value={adminContestName}
                            onChange={(e) =>
                              setAdminContestName(e.target.value)
                            }
                            className="w-full bg-black/60 border border-slate-700 rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 outline-none focus:border-yellow-500 focus:shadow-[0_0_10px_rgba(234,179,8,0.2)] transition-all"
                          />
                        </div>
                        <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl">
                          <label className="text-[10px] font-black text-[#e5c158]/70 uppercase tracking-widest block mb-2">
                            Total Prize Pool (Text)
                          </label>
                          <input
                            type="text"
                            placeholder="e.g., ₹20 Crores or ₹5000"
                            value={adminContestPrize}
                            onChange={(e) =>
                              setAdminContestPrize(e.target.value)
                            }
                            className="w-full bg-black/60 border border-slate-700 rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 outline-none focus:border-yellow-500 focus:shadow-[0_0_10px_rgba(234,179,8,0.2)] transition-all"
                          />
                        </div>
                        <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl">
                          ...
                          <input
                            type="number"
                            placeholder="e.g., 49"
                            value={adminContestEntry}
                            onChange={(e) =>
                              setAdminContestEntry(e.target.value)
                            }
                            className="w-full bg-black/60 border border-slate-700 rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 outline-none focus:border-yellow-500 focus:shadow-[0_0_10px_rgba(234,179,8,0.2)] transition-all"
                          />
                        </div>
                        {adminContestType === "Mega" && (
                          <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl">
                            ...
                            <input
                              ...
                              value={adminContestSpots}
                              onChange={(e) =>
                                setAdminContestSpots(e.target.value)
                              }
                              className="w-full bg-black/60 border border-slate-700 rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 outline-none focus:border-yellow-500 focus:shadow-[0_0_10px_rgba(234,179,8,0.2)] transition-all"
                            />
                          </div>
                      ...
                      <div className="bg-black/40 border border-[#e5c158]/20 p-4 rounded-xl mt-4">
                        <div className="flex justify-between items-center mb-3 border-b border-[#e5c158]/20 pb-2">
                          <h4 className="text-[10px] font-black text-[#e5c158] uppercase tracking-widest">
                        ...
                        <div className="space-y-3">
                          ...
                          {adminCustomPayouts.map((cp, idx) => (
                            <div key={idx} className="flex gap-2 items-center">
                              ...
                              <button
                                onClick={() => {
                                  setAdminCustomPayouts(
                                    adminCustomPayouts.filter(
                                      (_, i) => i !== idx,
                                    ),
                                  );
                                }}
                          ...
                          <div className="flex justify-between items-center mt-2">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                type="checkbox"
                                checked={adminAutoFillRemaining}
                                onChange={(e) =>
                                  setAdminAutoFillRemaining(e.target.checked)
                                }
                                className="rounded border-slate-700 bg-black/50 text-[#e5c158] focus:ring-[#e5c158]"
                              />
                              <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">
                                Auto-fill remaining ranks
                              </span>
                            </label>
                          </div>
                        </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (adminContestType === "Mega")
                            spots = parseInt(adminContestSpots) || 5000000;
                          ...
                          let parsedPrize = 0;
                          const lowerPrize = (adminContestPrize || "")
                            .toString()
                            .toLowerCase();
                          const numPart = parseFloat(
                            lowerPrize.replace(/[^0-9.]/g, ""),
                          );
                          if (!isNaN(numPart) && numPart > 0) {
                            if (lowerPrize.includes("cr"))
                              parsedPrize = numPart * 10000000;
                            else if (
                              lowerPrize.includes("lakh") ||
                              lowerPrize.includes("l")
                            )
                              parsedPrize = numPart * 100000;
                            else if (lowerPrize.includes("k"))
                              parsedPrize = numPart * 1000;
                            else parsedPrize = numPart;
                          }

                          const platMargin =
                            parseFloat(adminPlatformMargin) || 20;
                          const winSpotsPerc =
                            parseFloat(adminWinnersPercent) || 48;
                          const firstPrizePerc =
                            parseFloat(adminFirstPrizePercent) || 15;

                          const totalFeesCollected = (fee || 0) * spots;
                          const calculatedPool =
                            totalFeesCollected * ((100 - platMargin) / 100);
                          const actualPool =
                            parsedPrize > 0 ? parsedPrize : calculatedPool || 0;

                          if (adminContestType === "Mega") {
                            const formatAmount = (amt: number) =>
                              amt >= 100000
                                ? `₹${Number.isInteger(amt / 100000) ? amt / 100000 : (amt / 100000).toFixed(2)} Lakhs`
                                : `₹${Math.round(amt).toLocaleString("en-IN")}`;
                            let customRanksCost = 0;
                            let highestCustomRank = 0;

                            let validCustomPayouts = adminContestAutoPayouts
                              ? []
                              : adminCustomPayouts.filter(
                                  (cp) => cp.rankFrom && cp.amount,
                                );
                            ...
                            if (!adminContestAutoPayouts) {
                              validCustomPayouts.forEach((cp) => {
                                ...
                                let rawAmountLabel = (cp.amount || "")
                                  .toString()
                                  .toLowerCase()
                                  .replace(/,/g, "");
                                let numericAmount =
                                  parseFloat(
                                    rawAmountLabel.replace(/[^0-9.]/g, ""),
                                  ) || 0;
                                if (
                                  rawAmountLabel.includes("lakh") ||
                                  rawAmountLabel.includes("l")
                                )
                                  numericAmount *= 100000;
                                else if (rawAmountLabel.includes("cr"))
                                  numericAmount *= 10000000;
                                else if (rawAmountLabel.includes("k"))
                                  numericAmount *= 1000;

                                if (spotsInTier > 0 && numericAmount > 0) {
                                  customRanksCost +=
                                    spotsInTier * numericAmount;
                                }
                                if (to > highestCustomRank)
                                  highestCustomRank = to;
                              });
                            } else {
                              // Default Auto Payouts: auto generate Rank 1
                              let rank1Amount =
                                actualPool * (firstPrizePerc / 100);
                              payouts.push({
                                rank: "# 1",
                                amount: formatAmount(rank1Amount),
                              });
                              highestCustomRank = 1;
                            ...
                            const shouldAutoFill =
                              adminContestAutoPayouts || adminAutoFillRemaining;

                            const remainingPool = actualPool - customRanksCost;

                            if (shouldAutoFill && remainingPool > 0) {
                              let nextStart = highestCustomRank + 1;
                              let desiredLastRankEnd = Math.floor(
                                spots * (winSpotsPerc / 100),
                              );

                              let minPrizeForRest = Math.max(fee || 0, 1);
                              let maxAffordableWinners = Math.floor(
                                remainingPool / minPrizeForRest,
                              );

                              let lastRankEnd = desiredLastRankEnd;
                              if (
                                lastRankEnd >
                                nextStart + maxAffordableWinners - 1
                              ) {
                                lastRankEnd =
                                  nextStart + maxAffordableWinners - 1;
                              }
                              if (lastRankEnd < nextStart)
                                lastRankEnd = nextStart;

                              let remainingWinners =
                                lastRankEnd - nextStart + 1;
                              ...
                              if (remainingWinners > 0) {
                                ...
                                for (let tier of rawTiers) {
                                  if (
                                    currStart <= tier.maxEnd &&
                                    currStart <= lastRankEnd
                                  ) {
                                    let actualEnd = Math.min(
                                      tier.maxEnd,
                                      lastRankEnd,
                                    );
                                    validTiers.push({
                                ...
                                if (excessPool < 0) {
                                  ...
                                  let evenPrize = Math.floor(
                                    remainingPool / remainingWinners,
                                  );
                                  payouts.push({
                                    rank:
                                      nextStart === lastRankEnd
                                        ? `# ${nextStart}`
                                        : `# ${nextStart.toLocaleString("en-IN")} - ${lastRankEnd.toLocaleString("en-IN")}`,
                                    amount: `₹${evenPrize.toLocaleString("en-IN")}`,
                                  });
                                } else {
                                  // We have excess to distribute based on weights
                                  let totalWeightSum = validTiers.reduce(
                                    (sum, t) => sum + t.weight,
                                    0,
                                  );
                                  ...
                                  let tierPrizes = validTiers.map((t) => {
                                    let alloc =
                                      (t.weight / totalWeightSum) * excessPool;
                                    let perSpot =
                                      Math.floor(alloc / t.spots) + minPrize;
                                    return perSpot;
                                  });
                                  ...
                                  validTiers.forEach((t, i) => {
                                    ...
                                    if (
                                      finalTiers.length > 0 &&
                                      finalTiers[finalTiers.length - 1].amt ===
                                        amt
                                    ) {
                                      finalTiers[finalTiers.length - 1].end =
                                        t.end;
                                    } else {
                                      finalTiers.push({
                                        start: t.start,
                                    ...
                                    if (t.amt >= 1) {
                                      payouts.push({
                                        rank:
                                          t.start === t.end
                                            ? `# ${t.start}`
                                            : `# ${t.start.toLocaleString("en-IN")} - ${t.end.toLocaleString("en-IN")}`,
                          ...
                          let displayType = "H2H";
                          if (adminContestType === "H2H_3")
                            displayType = "3 Spots";
                          else if (adminContestType === "H2H_4")
                            displayType = "4 Spots";
                          else if (adminContestType === "H2H_5")
                            displayType = "5 Spots";
                          else if (adminContestType === "Mega")
                            displayType = "Mega";

                          let calculatedWinPercentage =
                            adminContestType === "Mega"
                              ? winSpotsPerc
                              : adminContestType === "H2H"
                                ? 50
                                : Math.floor(100 / spots);
                          let calculatedFirstPrize =
                            adminContestType === "Mega"
                              ? payouts?.[0]?.amount || "₹8 L"
                              : parsedPrize > 0
                                ? adminContestPrize.toString().includes("₹")
                                  ? adminContestPrize
                                  : `₹${adminContestPrize}`
                                : `₹${Math.floor(actualPool)}`;
                          ...
                          if (
                            ...
                            if (
                              adminCustomPayouts &&
                              adminCustomPayouts.length > 0
                            ) {
                              const lastCustom =
                                adminCustomPayouts[
                                  adminCustomPayouts.length - 1
                                ];
                              const maxRank =
                                parseInt(
                                  lastCustom.rankTo || lastCustom.rankFrom,
                                ) || 1;
                              calculatedWinPercentage = parseFloat(
                                ((maxRank / spots) * 100).toFixed(1),
                              );
                            }
                          }
                          ...
                          const newContest: Contest = {
                            ...
                            prizeText:
                              parsedPrize > 0
                                ? parsedPrize >= 100000
                                  ? `₹${Number.isInteger(parsedPrize / 100000) ? parsedPrize / 100000 : (parsedPrize / 100000).toFixed(2)} Lakhs`
                                  : `₹${parsedPrize}`
                                : adminContestPrize,
                            entryFee: fee,
                            spots: spots,
                          ...
                          localStorage.setItem(
                            "dreamApp_contests",
                            JSON.stringify(updatedContests),
                          );
                          syncCategoryToCloud("contests", updatedContests, 20);

                          alert(
                            `Successfully added ${adminContestType} contest!`,
                          );
                          setAdminContestName("");
              ...
              {!adminContestDashboard && (
                <button
                  onClick={() => setAdminContestDashboard("MANAGE_PRIZES")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
              ...
              {adminContestDashboard === "MANAGE_PRIZES" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                      Select a contest and manage its prize distribution (payout
                      rules).
                    </p>

                    <div className="space-y-4 relative z-10">
                      <select
                        value={selectedContestForPrize}
                        onChange={(e) =>
                          setSelectedContestForPrize(e.target.value)
                        }
                        className="w-full bg-black border border-slate-600 rounded-lg p-3 text-white focus:outline-none focus:border-[#e5c158]/50 mb-3"
                      >
                        <option value="">-- Select Contest --</option>
                        {appContests.map((c) => (
                          <option key={c.id} value={c.id}>
                            {c.name} - Fee: ₹{c.entryFee} ({c.type})
                          </option>
                        ))}
                      </select>

                      {selectedContestForPrize && (
                        <>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                type="text"
                                value={adminPrizeRank}
                                onChange={(e) =>
                                  setAdminPrizeRank(e.target.value)
                                }
                                placeholder="Rank"
                                className="w-full bg-black/50 border border-slate-700 text-slate-200 text-sm rounded-lg focus:ring-1 focus:ring-[#e5c158]/50 focus:border-[#e5c158]/50 block p-3"
                              />
                            </div>
                            <div>
                              <label className="text-[10px] font-black text-[#e5c158]/70 uppercase tracking-widest block mb-2">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setAdminPrizeAmount(e.target.value)
                                }
                                placeholder="Amount"
                                className="w-full bg-black/50 border border-slate-700 text-slate-200 text-sm rounded-lg focus:ring-1 focus:ring-[#e5c158]/50 focus:border-[#e5c158]/50 block p-3"
                              />
                          ...
                          <button
                            onClick={() => {
                              if (!adminPrizeRank || !adminPrizeAmount)
                                return alert("Enter both rank and amount");
                              const updatedContests = appContests.map((c) => {
                                if (c.id === selectedContestForPrize) {
                                  const newPayouts = [
                                    ...(c.payouts || []),
                                    {
                                      rank: adminPrizeRank,
                                      amount: adminPrizeAmount,
                                    },
                                  ];
                                  return { ...c, payouts: newPayouts };
                                }
                                return c;
                              });
                              setAppContests(updatedContests);
                            ...
                            className="w-full bg-[#1e232d] hover:bg-[#252b36] border border-slate-600 hover:border-[#e5c158]/50 text-white font-bold py-3 rounded-lg transition-colors mt-2"
                          >
                            Add Payout Level
                          </button>

                          <div className="mt-6 border-t border-[#e5c158]/20 pt-4">
                            <p className="text-[10px] font-black text-[#e5c158]/70 uppercase tracking-widest mb-3">
                              Current Payout Structure
                            </p>
                            <div className="space-y-2">
                              {appContests
                                .find((c) => c.id === selectedContestForPrize)
                                ?.payouts?.map((p, idx) => (
                                  <div
                                    key={idx}
                                    className="bg-black/60 border border-slate-700 hover:border-[#e5c158]/30 rounded-lg p-2.5 flex justify-between items-center text-xs"
                                  >
                                    <div>
                                ...
                                <p className="text-xs text-slate-500 py-2 border border-slate-800 rounded-lg text-center border-dashed">
                                  No payouts defined for this contest.
                                </p>
                              )}
                            </div>
                          </div>
              ...
              {!adminContestDashboard && (
                <button
                  onClick={() => setAdminContestDashboard("ATTACH_CONTESTS")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
                    <Trophy size={16} className="text-[#e5c158] mr-2" /> Attach
                    Contests to Matches
                  </h3>
                </button>
              )}
                ...
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-base font-black text-slate-100 flex items-center gap-2">
                        <Trophy size={16} className="text-[#e5c158]" /> Attach
                        Contests to Matches
                      </h2>
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                      Select a match and choose which contests should be
                      available for it.
                    </p>

                    <div className="space-y-4 relative z-10">
                      <select
                        value={selectedMatchForContests}
                        onChange={(e) => {
                          setSelectedMatchForContests(e.target.value);
                          const match = appMatches.find(
                            (m) => m.id === e.target.value,
                          );
                          const activeContestIds = match?.contestIds || [];
                          const newSelected: Record<string, boolean> = {};
                          appContests.forEach((c) => {
                            newSelected[c.id] = activeContestIds.includes(c.id);
                          });
                        ...
                        {appMatches
                          ...
                          .map((m) => (
                            <option key={m.id} value={m.id}>
                              {m.team1.name} vs {m.team2.name} - {m.series} (
                              {m.status})
                            </option>
                          ))}
                      </select>
                          ...
                          <div className="mt-4">
                            ...
                            <div className="space-y-2 max-h-[300px] overflow-y-auto no-scrollbar pr-1">
                              {appContests.map((c) => (
                                <div
                                  key={c.id}
                                  className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${selectedContestsForMatch[c.id] ? "bg-[#e5c158]/10 border-[#e5c158]/50" : "bg-black/40 border-slate-700 hover:border-slate-500"}`}
                                >
                                  <input
                                    type="checkbox"
                                    className="w-4 h-4 accent-[#e5c158] cursor-pointer"
                                    checked={!!selectedContestsForMatch[c.id]}
                                    onChange={(e) => {
                                      setSelectedContestsForMatch((prev) => ({
                                        ...prev,
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                      ...
                                      <span className="text-[10px] text-[#e5c158]">
                                        Pool: {c.prizeText}
                                      </span>
                                    </div>
                                  </div>
                          ...
                          <button
                            onClick={() => {
                              const selectedIds = Object.keys(
                                selectedContestsForMatch,
                              ).filter((id) => selectedContestsForMatch[id]);
                              const updatedMatches = appMatches.map((m) => {
                                if (m.id === selectedMatchForContests) {
                                  return { ...m, contestIds: selectedIds };
                                }
                                return m;
                              });
              ...
              <div className="flex flex-col gap-4 mt-2 mb-6">
                <div className="flex gap-2 p-2 bg-black/40 border border-slate-700/50 rounded-xl overflow-x-auto no-scrollbar items-center">
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-lg bg-black/50 border border-slate-700 hover:border-[#e5c158]/50 text-slate-400 hover:text-[#e5c158] flex items-center justify-center transition-colors"
                  >
                    <Plus size={18} />
                  </button>
                </div>
                  ...
                  <div className="bg-green-900/40 border border-green-500/50 p-4 rounded-xl flex justify-between items-center sticky top-[70px] z-20 shadow-lg backdrop-blur-sm mt-2">
                    <div className="text-white">
                      ...
                      <div className="flex items-center gap-2 font-black text-lg">
                        ...
                        {selectedTeamsForMatch[1] ? (
                          selectedTeamsForMatch[1].shortName
                        ) : (
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => setSelectedTeamsForMatch([])}
                        className="p-2 bg-black/40 rounded-lg text-slate-400 hover:text-red-400 transition-colors"
                      >
                        <X size={20} />
                      </button>
                      {selectedTeamsForMatch.length === 2 && (
                        <button
                          onClick={() => setShowCreateMatchFromTeamsModal(true)}
                          className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg font-bold shadow-[0_0_15px_rgba(34,197,94,0.4)] whitespace-nowrap transition-all"
                        >
                          Create
                        </button>
                      )}
                ...
                <div className="flex justify-between items-center mt-2">
                  <h3 className="font-bold text-slate-300 tracking-wide uppercase text-[10px]">
                    Teams for {selectedFormat}
                  </h3>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowAddFormatModal(true)}
                ...
                <div className="grid grid-cols-1 gap-3">
                  {appTeamsList
                    ...
                    .map((team) => {
                      const teamPlayers = appPlayers.filter(
                        (p) =>
                          p.team === team.shortName &&
                          (!p.format || p.format === team.format),
                      );
                      const isSelectedForMatch = selectedTeamsForMatch.some(
                        (st) => st.id === team.id,
                      );
                      ...
                      return (
                        <div
                          key={team.id}
                          className={`bg-[#13151c] border ${isSelectedForMatch ? "border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.2)]" : "border-slate-800"} rounded-xl overflow-hidden transition-all duration-300`}
                        >
                          <div
                            className="p-4 flex items-center justify-between cursor-pointer"
                            onClick={() => setAdminTeamDashboard(team.id)}
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-inner overflow-hidden relative shrink-0 ${team?.color?.startsWith("bg-") ? team.color : ""}`}
                                style={
                              ...
                              >
                                {team.flagUrl ? (
                                  <img
                                    src={team.flagUrl}
                                    alt={team.shortName}
                                    className={`w-full h-full ${team.flagFit === "contain" ? "object-contain" : "object-cover"}`}
                                  />
                                ) : (
                                  team.shortName
                                )}
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  if (isSelectedForMatch) {
                                    setSelectedTeamsForMatch((prev) =>
                                      prev.filter((t) => t.id !== team.id),
                                    );
                                  } else {
                                    if (selectedTeamsForMatch.length >= 2) {
                                      alert(
                                        "You can only select 2 teams for a match!",
                                      );
                                    } else {
                                      setSelectedTeamsForMatch((prev) => [
                                        ...prev,
                  ...
                  {appTeamsList.filter((t) => t.format === selectedFormat)
                    .length === 0 && (
                    <div className="text-center py-12 bg-black/20 border border-slate-800/50 rounded-xl border-dashed">
                      <p className="text-slate-500 font-bold mb-2">
                        No teams found for {selectedFormat}
                      </p>
                      <button
                        onClick={() => setShowAddTeamModal(true)}
                        className="text-[#e5c158] text-sm font-semibold hover:underline border-none bg-transparent"
                      >
                        Create a Team
                      </button>
                    </div>
                ...
                <div className="flex justify-between items-center mt-8 mb-3">
                  <h3 className="font-bold text-slate-300 tracking-wide uppercase text-[10px]">
                    Matches for {selectedFormat}
                  </h3>
                </div>

                <div className="space-y-4">
                  {appMatches.filter(
                    (m) => m.series && m.series.includes(selectedFormat),
                  ).length === 0 ? (
                    <div className="text-center py-12 bg-black/20 border border-slate-800/50 rounded-xl border-dashed">
                      <p className="text-slate-500 font-bold mb-2">
                        No matches found for {selectedFormat}
                      </p>
                    </div>
                  ) : (
                    appMatches
                      ...
                      .map((m) => (
                        <AdminMatchEditCard
                          ...
                          onUpdate={(updatedMatch) => {
                            const newMatches = appMatches.map((mm) =>
                              mm.id === m.id ? updatedMatch : mm,
                            );
                            setAppMatches(newMatches);
                            syncCategoryToCloud("matches", newMatches, 20);
                          }}
                          onDelete={() => {
                            const newMatches = appMatches.filter(
                              (mm) => mm.id !== m.id,
                            );
                            setAppMatches(newMatches);
                            syncCategoryToCloud("matches", newMatches, 20);
                          ...
                          onStatusChange={async (status) => {
                            if (
                              ...
                              const updatedPlayers = appPlayers.map((p) => {
                                if (
                                  p.livePoints !== undefined &&
                                  p.livePoints !== p.points
                                ) {
                              ...
                              const updatedPlayers = appPlayers.map((p) => {
                                if (
                                  p.isPlaying &&
                                  (p.team === m.team1.shortFrame ||
                                    p.team === m.team2.shortFrame) &&
                                  (!p.format || m.series?.includes(p.format))
                                ) {
                                  const pts = Math.max(
                                    p.livePoints ?? p.points,
                                    4,
                                  ...
                                  const breakup = {
                                    "starting 11": { points: 4, actual: "Yes" },
                                  };
                                  startingPoints[p.id] = {
                                    total: pts,
                                    breakup,
          ...
          {adminTab === "TEAMS" &&
            adminTeamDashboard &&
            (() => {
              const team = appTeamsList.find(
                (t) => t.id === adminTeamDashboard,
              );
              if (!team) return null;
              const teamPlayers = appPlayers.filter(
                (p) =>
              ...
              return (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="bg-[#13151c] border-b border-slate-800 p-4 pt-6 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => setAdminTeamDashboard(null)}
                        className="p-2 -ml-2 bg-slate-800/50 hover:bg-slate-800 rounded-full text-slate-400 transition-colors"
                      >
                        <ArrowLeft size={20} />
                      </button>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 font-bold">
                            {team.shortName} &bull; {team.format} &bull;{" "}
                            {teamPlayers.length} Players
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 pb-24">
                    <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar pb-1">
                      {(["ALL", "WK", "BAT", "AR", "BOWL"] as const).map(
                        (f) => {
                          const count =
                            f === "ALL"
                              ? teamPlayers.length
                              : teamPlayers.filter((p) => p.role === f).length;
                          return (
                            <button
                              key={f}
                            ...
                            >
                              {f}{" "}
                              <span
                                className={`px-1.5 py-0.5 rounded-sm text-[9px] ${adminTeamPlayerRoleFilter === f ? "bg-black/20" : "bg-slate-800"}`}
                              >
                                {count}
                              </span>
                            </button>
                    ...
                    {teamPlayers.length === 0 ? (
                      <div className="text-center py-12 bg-[#13151c] border border-slate-800 rounded-2xl">
                        ...
                        <button
                          ...
                          className="text-[#e5c158] text-sm hover:underline font-bold"
                        >
                          Add first player
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {(adminTeamPlayerRoleFilter === "ALL"
                          ? teamPlayers
                          : teamPlayers.filter(
                              (p) => p.role === adminTeamPlayerRoleFilter,
                            )
                        ).map((p: any) => (
                          <div
                            key={p.id}
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-[10px] font-black text-slate-500 bg-slate-800 px-1.5 py-0.5 rounded tracking-wide">
                                    {p.role}
                                  </span>
                                  <span className="text-[10px] font-bold text-[#e5c158]">
                                    {p.credits} Cr
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="flex items-center gap-2">
                              <button className="text-slate-500 hover:text-white p-2 rounded-lg bg-slate-800/50 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Edit2 size={14} />
                              </button>
                              <button
                                onClick={(e) => {
                                ...
                                className="text-red-500/50 hover:text-red-500 hover:bg-red-500/10 p-2 rounded-lg transition-all"
                              >
                                <Trash2 size={16} />
                              </button>
                            </div>
          ...
          {adminTab === "MATCHES" && (
            <>
              {!adminMatchDashboard && (
                <button
                  onClick={() => setAdminMatchDashboard("LIVE_SCORING")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
              ...
              {adminMatchDashboard === "LIVE_SCORING" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => setAdminMatchDashboard(null)}
                        className="p-2 -ml-2 rounded-full hover:bg-slate-800 transition-colors"
                      >
                        <ArrowLeft size={18} className="text-slate-400" />
                      </button>
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    {(() => {
                      if (!adminScoringLiveMatchId) {
                        const liveMatches = appMatches.filter(
                          (m) => m.status === "Live" || m.status === "LIVE",
                        );
                          ...
                          return (
                            <div className="text-center text-slate-400 py-8 border border-slate-800 rounded-lg bg-black/20 relative z-10">
                              ...
                              <p className="text-xs mt-2 text-slate-500">
                                Go to Match Status to mark a match as LIVE to
                                edit players.
                              </p>
                            </div>
                          );
                        ...
                        return (
                          <div className="relative z-10 space-y-3">
                            <p className="text-sm text-slate-400 mb-4">
                              Select a LIVE match to adjust player points:
                            </p>
                            {liveMatches.map((match) => (
                              <button
                                key={match.id}
                                onClick={() =>
                                  setAdminScoringLiveMatchId(match.id)
                                }
                      ...
                      const activeMatch = appMatches.find(
                        (m) => m.id === adminScoringLiveMatchId,
                      );
                      if (!activeMatch) return null;
                      ...
                      const filteredPlayers = appPlayers.filter(
                        (p) =>
                          liveTeams.includes(p.team) &&
                          p.isPlaying === true &&
                          ((p.name || "")
                            .toLowerCase()
                            .includes(
                              (playerScoringSearch || "").toLowerCase(),
                            ) ||
                            (p.team || "")
                              .toLowerCase()
                              .includes(
                        ...
                        <>
                          <div className="mb-6 relative z-10">
                            <div className="flex items-center gap-3 mb-4">
                              <button
                                onClick={() => setAdminScoringLiveMatchId(null)}
                                className="p-1bg-black/40 border border-slate-700 rounded hover:border-[#e5c158]/50"
                              >
                                <ArrowLeft
                                  size={14}
                            ...
                            <div className="flex items-center gap-2 bg-black/40 border border-slate-700/50 rounded p-1.5">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setPlayerScoringSearch(e.target.value)
                                }
                            ...
                            <table className="w-full text-left text-[10px] min-w-[700px]">
                              <thead className="bg-[#090b10] border-b border-slate-800/50 text-slate-400">
                                <tr>
                                  ...
                                  <th className="px-1 py-2 font-normal text-center">
                                    Pts
                                  </th>
                                  <th className="px-1 py-2 font-normal text-center">
                                    Adj
                              ...
                              <tbody className="divide-y divide-slate-800/50 bg-black/20">
                                {filteredPlayers.slice(0, 50).map((p) => {
                                  const st = playerAdjustments[p.id] || {
                                    adjustment: 0,
                                    manual: p.points,
                                    reason: "",
                                  };
                                  return (
                                    ...
                                    >
                                      <td
                                        className="px-2 py-1.5 font-bold text-slate-200 cursor-pointer hover:text-[#e5c158] transition-colors whitespace-nowrap"
                                        onClick={() =>
                                          setAdminEditPlayerBreakup(p)
                                        }
                                      >
                                        {p.name}
                                      </td>
                                      <td className="px-1 py-1.5 text-slate-400 font-mono text-[9px]">
                                        {String(p.team || "")
                                          .substring(0, 3)
                                          .toUpperCase()}
                                      </td>
                                      <td className="px-1 py-1.5 text-center text-slate-300 font-bold">
                                        {p.points}
                                      </td>
                                      <td className="px-1 py-1.5 text-center">
                                        <div className="flex items-center justify-center gap-1 bg-[#090b10] border border-slate-700 rounded p-0.5 w-[60px] mx-auto">
                                          <button
                                            ...
                                            className="text-slate-400 hover:text-white font-mono font-bold px-1\"
                                          >
                                            -
                                          </button>
                                          <span
                                            className={`font-mono font-bold w-4 text-[9px] text-center ${st.adjustment > 0 ? "text-green-500" : st.adjustment < 0 ? "text-red-500" : "text-slate-400"}`}
                                          >
                                            {st.adjustment > 0 ? "+" : ""}
                                            {st.adjustment}
                                          </span>
                                      ...
                                      <td className="px-1 py-1.5">
                                        <div className="flex flex-wrap gap-1 justify-center max-w-[260px] mx-auto">
                                          <button
                                            ...
                                            className="bg-green-600/20 hover:bg-green-600/40 text-green-500 text-[8px] px-1 py-0.5 rounded border border-green-600/30 whitespace-nowrap font-bold text-center flex items-center justify-center"
                                          >
                                            +25 WKT
                                          </button>
                                          <button
                                          ...
                                          <button
                                            onClick={() => {
                                              const customStr = prompt(
                                                "Enter custom points to add/subtract:",
                                              );
                                              if (
                                                customStr &&
                                                !isNaN(parseInt(customStr))
                                              ) {
                                            ...
                                            className="bg-[#e5c158]/20 hover:bg-[#e5c158]/40 text-[#e5c158] text-[8px] px-1 py-0.5 rounded border border-[#e5c158]/30 flex items-center justify-center font-bold font-mono min-w-[20px]"
                                          >
                                            <Plus size={10} />
                                          </button>
                                        </div>
                                        ...
                                        <button
                                          onClick={async () => {
                                            ...
                                            try {
                                              ...
                                              console.error(
                                                "Could not sync point update to cloud immediately",
                                                e,
                                              );
                                            }

                                            setUpdateSuccessIds((prev) => ({
                                              ...prev,
                                              [p.id]: true,
                                            }));
                                            setTimeout(() => {
              ...
              {!adminMatchDashboard && (
                <button
                  onClick={() => setAdminMatchDashboard("MANAGE_MATCHES")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
                    App Settings: Manage Matches
                  </h3>
                </button>
              )}

              {adminMatchDashboard === "MANAGE_MATCHES" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                      Create new upcoming matches or change the status of
                      existing matches (Live, Completed).
                    </p>

                    <p className="text-[10px] font-black text-[#e5c158]/70 uppercase tracking-widest mb-3 relative z-10">
                      Live / Existing Matches Control
                    </p>
                    <div className="flex gap-2 mb-4 relative z-10 bg-black/40 p-1.5 rounded-xl border border-slate-800">
                      <button
                        onClick={() => setAdminMatchesFilter("Upcoming")}
                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${adminMatchesFilter === "Upcoming" ? "bg-[#e5c158] text-black shadow-md" : "text-slate-400 hover:text-white"}`}
                      >
                        Upcoming
                      </button>
                      <button
                        onClick={() => setAdminMatchesFilter("Live")}
                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${adminMatchesFilter === "Live" ? "bg-[#e5c158] text-black shadow-md" : "text-slate-400 hover:text-white"}`}
                      >
                        Live
                      </button>
                      <button
                        onClick={() => setAdminMatchesFilter("Completed")}
                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${adminMatchesFilter === "Completed" ? "bg-[#e5c158] text-black shadow-md" : "text-slate-400 hover:text-white"}`}
                      >
                        Completed
                      </button>
                    </div>
                    <div className="space-y-4 relative z-10">
                      {appMatches
                        .filter((m) =>
                          adminMatchesFilter === "All"
                            ? true
                            : m.status === adminMatchesFilter,
                        )
                        .map((m) => (
                          <AdminMatchEditCard
                            key={m.id}
          ...
          {adminTab === "ENTRY FEES" && (
            <>
              <h3 className="font-bold text-[#e5c158]/80 uppercase tracking-widest text-[10px] mb-3 ml-1 mt-6">
                System Control: Entry Fees Section
              </h3>
              <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mb-6 relative overflow-hidden backdrop-blur-sm">
                <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                  See all completed/filled contest instances and the entry fees
                  generated.
                </p>
                  ...
                  {(() => {
                    ...
                    appMatches.forEach((match) => {
                      appContests.forEach((contest) => {
                        ...
                        const teamsForThis = savedTeams.filter(
                          (t) =>
                            t.match?.id === match.id &&
                            (t.contestId
                              ? t.contestId === contest.id
                        ...
                        Object.keys(instanceMap).forEach((iId) => {
                          ...
                          if (teamsInInstance.length === cspots) {
                            filledInstances.push({
                              ...
                              instanceId: iId,
                              totalFees: teamsInInstance.reduce(
                                (acc, t) => acc + (t.fee || 0),
                                0,
                              ),
                    ...
                    if (filledInstances.length === 0) {
                      return (
                        <p className="text-sm font-bold text-slate-500 text-center py-6 bg-black/40 border border-slate-700 rounded-xl">
                          No filled contests yet.
                        </p>
                      );
                    }

                    const totalRevenue = filledInstances.reduce(
                      (acc, inst) => acc + inst.totalFees,
                      0,
                    );
                    ...
                    return (
                      <>
                        ...
                        {filledInstances.map((inst, i) => (
                          <div
                            key={i}
                            className="bg-black/40 border border-slate-700/50 p-4 rounded-xl flex flex-col gap-2 relative"
                          >
                            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                              <div>
                                <span className="text-xs text-yellow-500 font-bold">
                                  {inst.contestName} ({inst.contestType})
                                </span>
                                <div className="text-[10px] text-slate-400 flex gap-1 items-center mt-1">
                                  <span className="bg-slate-800 px-1.5 py-0.5 rounded">
                                    {inst.team1} vs {inst.team2}
                                  </span>
                                  <span>
                                    • Instance #{parseInt(inst.instanceId) + 1}
                                  </span>
                                </div>
                              </div>
                              <div className="text-right">
                            ...
                            <div className="flex -space-x-2 overflow-hidden py-1">
                              {inst.teams.map((t: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="inline-block h-6 w-6 rounded-full ring-2 ring-black bg-slate-800 text-[8px] flex items-center justify-center font-bold text-white shrink-0"
                                  title={t.userName}
                                >
                                  {String(t.userName || t.userId || "U")
                                    .substring(0, 2)
                                    .toUpperCase()}
                                </div>
                              ))}
                              <span className="ml-4 text-[10px] text-slate-500 self-center font-bold">
                                {inst.teams.length} Users Joined
                              </span>
                            </div>
                          </div>
                        ))}
            ...
            <>
              <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mb-6 relative overflow-hidden backdrop-blur-sm">
                <h3 className="font-bold text-slate-200 tracking-wide flex items-center gap-2 mb-6">
                  <BarChart2 size={18} className="text-[#e5c158]" /> Real-time
                  Financial Breakdown
                </h3>
                ...
                {(() => {
                  ...
                  depositRequests.forEach((d) => {
                    if (d.status === "Approved" && d.timestamp) {
                      ...
                      const mName = date.toLocaleString("default", {
                        month: "short",
                        year: "2-digit",
                      });
                      if (monthMap[mName]) {
                        monthMap[mName].revenue += parseFloat(
                          d.amount?.toString() || "0",
                        );
                      } else {
                        monthMap[mName] = {
                          name: mName,
                          revenue: parseFloat(d.amount?.toString() || "0"),
                          payout: 0,
                        };
                      }
                    }
                  ...
                  withdrawRequests.forEach((w) => {
                    if (w.status === "Approved" && w.timestamp) {
                      ...
                      if (monthMap[mName]) {
                        monthMap[mName].payout += parseFloat(
                          w.amount?.toString() || "0",
                        );
                      } else {
                        if (monthMap[mName]) {
                          ...
                          monthMap[mName] = {
                            name: mName,
                            revenue: 0,
                            payout: parseFloat(w.amount?.toString() || "0"),
                          };
                        }
                      }
                    }
              ...
              <div className="mb-6 flex justify-end">
                <button
                  onClick={async () => {
                    if (
                      window.confirm(
                        "WARNING: This will permanently DELETE ALL deposit and withdrawal history. It will set Total Lifetime Revenue and Payouts to zero. Are you absolutely sure?",
                      )
                    ) {
          ...
          {adminTab === "MATCHES" && (
            <>
              {!adminMatchDashboard && (
                <button
                  onClick={() => setAdminMatchDashboard("MANAGE_USER_TEAMS")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
                    Manage User Teams (Edit)
                  </h3>
                </button>
              )}

              {adminMatchDashboard === "MANAGE_USER_TEAMS" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    {!adminTeamEditMatchId ? (
                      <>
                        <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                          Select a match to view and edit user teams.
                        </p>
                        <div className="space-y-4 relative z-10">
                      ...
                      <>
                        <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[#e5c158]/20 relative z-10">
                          <button
                            onClick={() => setAdminTeamEditMatchId(null)}
                            className="p-1.5 rounded-lg bg-black border border-slate-700 hover:border-[#e5c158]/50 hover:text-[#e5c158] transition-colors text-slate-300"
                          >
                            <ArrowLeft size={16} />
                          </button>
                        ...
                        <div className="space-y-4 relative z-10">
                          {(() => {
                            const matchTeams = savedTeams.filter(
                              (t) =>
                                t.match?.id === adminTeamEditMatchId &&
                                t.userId !== "admin_bot" &&
                                t.userId !== "admin_bot_boot",
                            );
                            const filtered = matchTeams.filter(
                              (st) =>
                                String(st.userId || "")
                                  .toLowerCase()
                            ...
                            if (matchTeams.length === 0)
                              return (
                                <p className="text-sm font-bold text-slate-500 text-center py-6 bg-black/40 border border-slate-700 rounded-xl">
                                  No user teams for this match.
                                </p>
                              );
                            if (filtered.length === 0)
                              return (
                                <p className="text-sm font-bold text-slate-500 text-center py-6 bg-black/40 border border-slate-700 rounded-xl">
                                  No results for "{teamSearchQuery}"
                                </p>
                              );
                            ...
                            return filtered.map((st, actIdx) => (
                              <div
                                key={st.id || actIdx}
                                className="bg-black/60 border border-slate-700 rounded-xl p-4 text-sm transition-colors hover:border-[#e5c158]/30"
                              >
                                <div className="flex justify-between items-center font-bold text-slate-200 mb-3 pb-2 border-b border-yellow-500/10">
                                  <span className="flex-1 truncate pr-2">
                                    {st.match?.series} (
                                    {st.match?.team1?.shortFrame} vs{" "}
                                    {st.match?.team2?.shortFrame})
                                  </span>
                                  <span className="text-[#e5c158] bg-[#e5c158]/10 border border-[#e5c158]/20 px-2 py-1 rounded text-[10px] tracking-widest">
                                    {st.userId}
                                  </span>
                                </div>
              ...
              {!adminMatchDashboard && (
                <button
                  onClick={() => setAdminMatchDashboard("LIVE_MATCH_DASHBOARD")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
              ...
              {adminMatchDashboard === "LIVE_MATCH_DASHBOARD" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    {!adminLiveMatchId ? (
                      <>
                        <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                          Select a live match to award real-time points.
                        </p>
                        <div className="space-y-4 relative z-10">
                          {appMatches.filter((m) => m.status === "Live")
                            .length > 0 ? (
                            appMatches
                              .filter((m) => m.status === "Live")
                              .map((match) => (
                                ...
                                >
                                  <div className="flex items-center gap-4">
                                    <div className="flex items-center gap-2">
                                      <div
                                        className="w-6 h-6 rounded-full border border-[#e5c158]/30 shadow-sm"
                                        style={{
                                          backgroundColor: match?.team1?.color,
                                        }}
                                      ></div>
                            ...
                            <div className="text-center p-8 bg-black/40 rounded-xl border border-slate-800 relative z-10">
                              ...
                              <span className="text-slate-500 text-sm font-bold block mb-1 tracking-wide">
                                No Live Matches
                              </span>
                              <p className="text-xs text-slate-600">
                                Start a match to see the dashboard.
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="relative z-10">
                        {(() => {
                          const match = appMatches.find(
                            (m) => m.id === adminLiveMatchId,
                          );
                          if (!match) return null;
                          const matchPlayers = appPlayers.filter(
                            (p) =>
                              (p.team === match?.team1?.shortFrame ||
                                p.team === match?.team2?.shortFrame) &&
                              (!p.format || match?.series?.includes(p.format)),
                          );
                          return (
                            <>
                              <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[#e5c158]/20">
                                <button
                                  onClick={() => setAdminLiveMatchId(null)}
                                  className="p-1.5 rounded-lg bg-black border border-slate-700 hover:border-[#e5c158]/50 hover:text-[#e5c158] transition-colors text-slate-300"
                                >
                                  <ArrowLeft size={16} />
                                </button>
                                <h4 className="font-bold text-slate-200 text-sm">
                                  {match?.team1?.shortFrame} vs{" "}
                                  {match?.team2?.shortFrame} Control
                                </h4>
                              </div>
                              <p className="text-xs text-slate-400 mb-5 pl-1">
                                Award real-time points. Leaderboards and user
                                dashboards will update instantly.
                              </p>
                              ...
                              <div className="space-y-4">
                                {matchPlayers.map((player) => (
                                  <div
                                    key={player.id}
                                    className="bg-black/60 rounded-xl border border-slate-700 overflow-hidden transition-all hover:border-[#e5c158]/30"
                                  >
                                    <div className="flex justify-between items-center p-4">
                                      <div className="flex flex-col gap-1.5">
                                        <span className="text-sm font-bold text-slate-200 flex items-center gap-3">
                                          {player.name}{" "}
                                          <span className="bg-[#e5c158]/10 border border-[#e5c158]/20 text-[10px] uppercase font-black px-2 py-0.5 rounded text-[#e5c158] tracking-widest">
                                            {player.team}
                                          </span>
                                        </span>
                                        <span className="text-xs font-bold text-emerald-400 tracking-wide mt-1">
                                          {player.points} Total Pts
                                        </span>
                                      </div>
                                      <button
                                        onClick={() =>
                                          setAdminExpandedPlayerId(
                                    ...
                                    {adminExpandedPlayerId === player.id && (
                                      <div className="p-4 bg-black/80 border-t border-slate-700">
                                        <p className="text-[10px] font-black tracking-widest text-[#e5c158]/70 uppercase mb-4 pl-1">
                                          Award Points
                                        </p>
                                        <div className="grid grid-cols-3 gap-3">
                                          ...
                                          >
                                            <span className="text-red-500 text-sm shadow-[0_0_10px_rgba(239,68,68,0.2)] pb-0.5">
                                              +25
                                            </span>{" "}
                                            Wicket
                                          </button>
                                        ...
                                        <div className="flex gap-3 items-center mt-4 pt-4 border-t border-slate-800">
                                          <span className="text-[10px] font-black tracking-widest flex-1 text-slate-500 uppercase">
                                            Custom Manual:
                                          </span>
                                          <button
                                            onClick={() =>
                                            ...
                                            className="bg-red-950/50 border border-red-900/50 text-red-500 px-4 py-2 rounded-lg font-bold text-xs tracking-wider active:scale-95"
                                          >
                                            -1
                                          </button>
                                          <button
                                            ...
                                            className="bg-emerald-950/50 border border-emerald-900/50 text-emerald-500 px-4 py-2 rounded-lg font-bold text-xs tracking-wider active:scale-95"
                                          >
                                            +10
                                          </button>
                                        </div>
              ...
              {!adminMatchDashboard && (
                <button
                  onClick={() => setAdminMatchDashboard("MANAGE_LINEUPS")}
                  className={`flex items-center justify-between w-full mt-4 bg-[#13151c] border border-slate-800 rounded-xl mb-3 hover:border-[#e5c158]/30 p-4 shadow-lg transition-all relative group overflow-hidden`}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                  <h3 className="font-bold text-slate-200 tracking-wide flex items-center justify-between z-10">
                    App Settings: Upcoming Matches Lineups
                  </h3>
                </button>
              )}

              {adminMatchDashboard === "MANAGE_LINEUPS" && (
                <div className="absolute inset-0 bg-[#090b10] z-50 flex flex-col overflow-hidden animate-in slide-in-from-right-4">
                  <div className="flex-none p-4 sticky top-0 bg-[#090b10] flex items-center justify-between border-b border-slate-800 shadow-sm z-50">
                    <div className="flex items-center gap-3">
                      <button
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-10">
                    {!adminUpcomingLineupMatchId ? (
                      <>
                        <p className="text-xs text-slate-400 mb-5 pl-1 relative z-10">
                          Select an upcoming match to set player lineups.
                        </p>
                        <div className="space-y-4 relative z-10">
                          {appMatches.filter((m) => m.status === "Upcoming")
                            .length > 0 ? (
                            appMatches
                              .filter((m) => m.status === "Upcoming")
                              .map((match) => (
                                <div
                                  key={match.id}
                                  onClick={() =>
                                    setAdminUpcomingLineupMatchId(match.id)
                                  }
                                  className="bg-black/60 rounded-xl border border-slate-700 p-4 flex justify-between items-center cursor-pointer hover:border-[#e5c158]/50 transition-colors"
                                >
                                  <div className="flex items-center gap-4">
                                  ...
                                  <div className="flex items-center gap-4">
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const newMatchesList = appMatches.map(
                                          (m) =>
                                            m.id === match.id
                                              ? {
                                                  ...m,
                            ...
                            <div className="text-center p-8 bg-black/40 rounded-xl border border-slate-800 relative z-10">
                              <span className="text-slate-500 text-sm font-bold block tracking-wide">
                                No Upcoming Matches
                              </span>
                            </div>
                          )}
                      ...
                      <div className="relative z-10">
                        {(() => {
                          const match = appMatches.find(
                            (m) => m.id === adminUpcomingLineupMatchId,
                          );
                          if (!match) return null;
                          const matchPlayers = appPlayers.filter(
                            (p) =>
                          ...
                          return (
                            <>
                              <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[#e5c158]/20">
                                <button
                                  onClick={() =>
                                    setAdminUpcomingLineupMatchId(null)
                                  }
                                  className="p-1.5 rounded-lg bg-black border border-slate-700 hover:border-[#e5c158]/50 hover:text-[#e5c158] transition-colors text-slate-300"
                                >
                                  <ArrowLeft size={16} />
                                ...
                                <h4 className="font-bold text-slate-200 text-sm">
                                  {match?.team1?.shortFrame} vs{" "}
                                  {match?.team2?.shortFrame} - Lineups
                                </h4>
                              </div>
                              <p className="text-xs text-slate-400 mb-5 pl-1">
                                Select players that are playing. They will get a
                                green tick. Unselected players will have a red
                                dot.
                              </p>
                              ...
                              <div className="space-y-3">
                                {matchPlayers.map((player) => (
                                  <div
                                    key={player.id}
                                    onClick={() => {
                                      const updatedPlayers = appPlayers.map(
                                        (p) =>
                                          p.id === player.id
  ...
  if (wallet?.blocked && !isAdmin) {
    return (
      <div
        className={`relative h-[100dvh] w-full max-w-md mx-auto bg-app-bg text-app-text font-sans shadow-2xl overflow-hidden border-x border-app-border flex flex-col items-center justify-center p-8 text-center ${themeMode === "Light" ? "theme-light" : ""} color-${themeColor.toLowerCase()}`}
      >
        <Shield size={64} className="text-red-500 mb-4" />
        <h2 className="text-2xl font-black mb-2 text-red-500">
          Account Blocked
        </h2>
        <p className="text-sm font-bold text-app-text-muted mb-8">
        ...
        <button
          ...
          className="bg-red-600 hover:bg-red-700 font-bold px-6 py-2 rounded-xl text-white shadow-lg active:scale-95 transition-transform"
        >
          Logout
        </button>
      </div>
  ...
  return (
    <div
      className={`relative h-[100dvh] w-full max-w-md mx-auto bg-app-bg text-app-text font-sans shadow-2xl overflow-hidden border-x border-app-border ${themeMode === "Light" ? "theme-light" : ""} color-${themeColor.toLowerCase()}`}
    >
      <>
        {showAddFlagModal && (
          <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 min-h-[100dvh]">
            <div className="bg-[#13151c] border border-slate-700 w-full rounded-2xl p-6 shadow-2xl relative">
              ...
              <h3 className="font-bold text-slate-200 text-lg mb-4">
                Set Team Flag/Image
              </h3>
                ...
                {newFlagUrl ? (
                  <div className="w-full flex flex-col gap-4">
                    ...
                    <button
                      ...
                      className="text-xs text-[#e5c158] font-bold self-center hover:underline"
                    >
                      Change Image
                    </button>
                  </div>
              ...
              <button
                onClick={async () => {
                  if (newFlagUrl && croppedAreaPixels) {
                    try {
                      const finalCroppedImageBase64 = await getCroppedImg(
                        newFlagUrl,
                        croppedAreaPixels,
                      );
                      const newTeams = appTeamsList.map((t) =>
                        t.id === showAddFlagModal
                          ? {
                              ...t,
                              flagUrl: finalCroppedImageBase64,
                              flagFit: "cover" as "cover",
                            }
                          : t,
                      );
          ...
          <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 min-h-[100dvh]">
            <div className="bg-[#13151c] border border-slate-700 w-full rounded-2xl p-6 shadow-2xl relative">
              ...
              <input
                ...
                placeholder="Format Name (e.g. BBL or The Hundred)"
                className="w-full bg-black border border-slate-700 rounded-xl px-4 py-3 text-slate-200 outline-none text-sm mb-4"
              />
              <button
                onClick={() => {
                  if (
                    newFormatName.trim() &&
                    !appFormats.includes(newFormatName.trim())
                  ) {
                ...
                className="w-full py-3 bg-[#e5c158] text-black rounded-xl font-bold uppercase tracking-widest text-sm shadow-[0_0_15px_rgba(229,193,88,0.4)] transition-all active:scale-95"
              >
                Add Format
              </button>
            </div>
        ...
        {showAddTeamModal && (
          <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 min-h-[100dvh]">
            <div className="bg-[#13151c] border border-slate-700 w-full rounded-2xl p-6 shadow-2xl relative">
              ...
              <h3 className="font-bold text-slate-200 text-lg mb-4">
                Add Team to {selectedFormat}
              </h3>
              <input
                value={newTeamName}
                onChange={(e) => setNewTeamName(e.target.value)}
                placeholder="Team Name (e.g. India)"
                className="w-full bg-black border border-slate-700 rounded-xl px-4 py-3 text-slate-200 outline-none text-sm mb-3 focus:border-[#e5c158]/50 transition-colors"
              />
              <input
                value={newTeamShort}
                onChange={(e) => setNewTeamShort(e.target.value)}
                placeholder="Short Name (e.g. IND, max 4)"
                maxLength={4}
                className="w-full bg-black border border-slate-700 rounded-xl px-4 py-3 text-slate-200 outline-none text-sm mb-3 uppercase focus:border-[#e5c158]/50 transition-colors"
              />
              ...
              <div className="flex gap-2 mb-6 justify-between border-t border-slate-800 pt-3">
                {[
                  ...
                  <button
                    ...
                    className={`w-8 h-8 rounded-full ${c} ${newTeamColor === c ? "ring-2 ring-white ring-offset-2 ring-offset-[#13151c]" : ""} hover:scale-110 transition-transform`}
                  ></button>
                ))}
              </div>
              <button
                ...
                className="w-full py-3 bg-[#e5c158] text-black rounded-xl font-bold uppercase tracking-widest text-sm shadow-[0_0_15px_rgba(229,193,88,0.4)] transition-all active:scale-95 border border-[#e5c158]/50"
              >
                Save Team
              </button>
            </div>
        ...
        {showTeamAddPlayerModal && (
          <div className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 min-h-[100dvh]">
            <div className="bg-[#13151c] border border-slate-700 w-full rounded-2xl p-6 shadow-[0_0_50px_rgba(0,0,0,1)] relative">
              ...
              <p className="text-xs text-slate-500 mb-4 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                {editingPlayerId ? "Updating in" : "Adding to"}{" "}
              ...
              <div className="grid grid-cols-4 gap-2 mb-3">
                {(["BAT", "BOWL", "AR", "WK"] as const).map((r) => (
                  <button
                    ...
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPlayerRole === r ? "bg-slate-700 text-[#e5c158] border border-[#e5c158]/50 shadow-[0_0_10px_rgba(229,193,88,0.15)]" : "bg-black text-slate-500 border border-slate-800"}`}
                  >
                    {r}
                  </button>
                ))}
              ...
              <input
                ...
                placeholder="Credits (e.g. 9.0)"
                className="w-full bg-black border border-slate-700 rounded-xl px-4 py-3 text-slate-200 outline-none text-sm mb-6 focus:border-[#e5c158]/50 transition-colors"
              />
              ...
              <button
                onClick={() => {
                  if (newPlayerName.trim() && newPlayerCredits.trim()) {
                    if (editingPlayerId) {
                      const newPlayers = appPlayers.map((p) =>
                        p.id === editingPlayerId
                          ? {
                              ...
                              role: newPlayerRole,
                              credits:
                                parseFloat(newPlayerCredits.trim()) || 8.0,
                            }
                          : p,
                      );
                ...
                className="w-full py-3 bg-[#e5c158] text-black shadow-lg rounded-xl font-bold tracking-widest text-sm text-center transition-all active:scale-95 uppercase"
              >
                ...
                <button
                  onClick={() => {
                    if (
                      ...
                      const newPlayers = appPlayers.filter(
                  ...
                  className="w-full mt-2 py-3 bg-red-600/20 text-red-500 border border-red-500/50 shadow-lg rounded-xl font-bold tracking-widest text-sm text-center transition-all active:scale-95 uppercase"
                >
                  Delete Player
                </button>
              )}
          ...
          selectedTeamsForMatch.length === 2 && (
            <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col justify-end min-h-[100dvh]">
              <div className="bg-[#13151c] border-t border-slate-700 w-full rounded-t-3xl p-6 pt-5 shadow-[0_-10px_40px_rgba(34,197,94,0.1)] relative pb-10 mt-auto shadow-[0_-10px_40px_rgba(0,0,0,0.8)]">
                ...
                <h3 className="font-bold text-slate-200 text-xl tracking-tight text-center mb-1 drop-shadow-sm flex justify-center items-center gap-2">
                  <Trophy size={20} className="text-green-500" /> Create Match
                </h3>
                <div className="flex items-center justify-center gap-3 font-black text-2xl mb-6 text-white drop-shadow-md">
                  <span
                    className={
                      selectedTeamsForMatch[0].color === "bg-blue-600"
                        ? "text-blue-400"
                        : "text-slate-200"
                    }
                  >
                  ...
                  <span className="text-sm font-normal text-slate-500 italic flex items-center justify-center w-6 h-6 rounded-full bg-slate-800 border border-slate-700 pt-0.5">
                    V
                  </span>
                  <span
                    className={
                      selectedTeamsForMatch[1].color === "bg-green-600"
                        ? "text-green-400"
                        : "text-slate-200"
                    }
                  >
                ...
                <div className="space-y-4 mb-8">
                  <div>
                    ...
                    <input
                      value={newMatchTimeForm}
                      onChange={(e) => setNewMatchTimeForm(e.target.value)}
                      type="datetime-local"
                      className="w-full bg-black border border-slate-700 rounded-xl px-4 py-3 text-slate-200 outline-none focus:border-green-500 focus:shadow-[0_0_10px_rgba(34,197,94,0.15)] transition-all text-sm"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setShowCreateMatchFromTeamsModal(false)}
                    className="flex-1 py-4 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl font-bold uppercase tracking-widest text-sm active:scale-95 transition-transform hover:bg-slate-700"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => {
                      if (newMatchTimeForm) {
                        ...
                        let formattedLabel = dateFormat.toLocaleTimeString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        });
                        ...
                        const newMatchObj: Match = {
                          id: "m" + Date.now(),
                          team1: {
                            name: selectedTeamsForMatch[0].name,
                            shortFrame: selectedTeamsForMatch[0].shortName,
                            color: (
                              selectedTeamsForMatch[0].color || "bg-slate-800"
                            ).replace("bg-", "text-"),
                            flagUrl: selectedTeamsForMatch[0].flagUrl,
                            flagFit: selectedTeamsForMatch[0].flagFit,
                          },
                          team2: {
                            name: selectedTeamsForMatch[1].name,
                            shortFrame: selectedTeamsForMatch[1].shortName,
                            color: (
                              selectedTeamsForMatch[1].color || "bg-slate-800"
                            ).replace("bg-", "text-"),
                            flagUrl: selectedTeamsForMatch[1].flagUrl,
                            flagFit: selectedTeamsForMatch[1].flagFit,
                          },
                          time: formattedLabel,
                        ...
                        alert(
                          "Match Created Successfully! You can find it in the matches list.",
                        );
                      } else {
                        alert("Please select match time");
                      }
                    }}
                    className="flex-1 py-4 bg-green-500 border border-green-400 text-white shadow-[0_0_20px_rgba(34,197,94,0.4)] rounded-xl font-black uppercase tracking-widest text-sm active:scale-95 transition-transform hover:bg-green-400"
                  >
                    CREATE
                  </button>
                </div>
      ...
      {showPlayerInfoPopup && (
        <div
          className="fixed inset-[-20px] bg-black/60 z-[100] flex flex-col pt-10 pb-4 px-4 overflow-hidden"
          onClick={() => setShowPlayerInfoPopup(null)}
        >
          <motion.div
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
          ...
          >
            <div className="bg-[#0f0f0f] dark:bg-black text-white p-4 flex items-center gap-4 border-b border-app-border shrink-0">
              <button onClick={() => setShowPlayerInfoPopup(null)}>
                <ArrowLeft size={24} />
              </button>
            ...
            <div className="flex-1 overflow-y-auto bg-app-card text-app-text">
              <div className="p-4 bg-app-card flex items-center gap-4">
                <div className="w-[60px] h-[60px] border border-app-border bg-app-bg rounded-lg shadow-sm flex items-end justify-center overflow-hidden">
                  <User
                    size={65}
                    className="text-[#d1d5db] dark:text-[#475569] translate-y-3"
                    fill="currentColor"
                    strokeWidth={1}
                  />
                </div>
                <div>
                  <h3 className="text-xl font-black">
                    {showPlayerInfoPopup.name}
                  </h3>
                  <p className="text-sm font-bold text-app-text-muted uppercase tracking-widest">
                    {showPlayerInfoPopup.team} | {showPlayerInfoPopup.role}
                  </p>
                </div>
              </div>
              ...
              <div className="w-full text-sm">
                ...
                {POINT_EVENTS.map((evt) => {
                  ...
                  if (
                    ...
                    evtPts =
                      (showPlayerInfoPopup.livePoints ??
                        showPlayerInfoPopup.points) -
                      (showPlayerInfoPopup.isPlaying &&
                      (activeMatch?.status === "Live" ||
                        activeMatch?.status === "Completed")
                        ? 4
                  ...
                  return (
                    <div
                      key={evt}
                      className="flex px-4 py-3 border-b border-app-border bg-app-bg items-center hover:bg-app-card transition-colors"
                    >
                      <div className="flex-1 font-semibold text-app-text">
                        {evt}
                      </div>
                      <div className="w-16 text-center font-bold text-app-text">
                        {evtPts}
                      </div>
                      <div className="w-16 text-right text-app-text-muted">
                        {evtActual}
                      </div>
                    </div>
                  );
                ...
                <div className="flex px-4 py-4 bg-app-bg items-center font-black font-sans text-lg border-t-2 border-t-app-border-hover mt-3 shadow-t-sm">
                  ...
                  <div className="w-16 text-right pr-4 font-black">
                    {showPlayerInfoPopup.livePoints ??
                      showPlayerInfoPopup.points}
                  </div>
                </div>
            ...
            {view === "CREATE_TEAM" && selectedContest && (
              <div className="p-4 bg-app-bg shrink-0 border-t border-app-border shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] text-center relative z-[110]">
                <button
                  onClick={() => {
                    togglePlayer(showPlayerInfoPopup);
                    setShowPlayerInfoPopup(null);
                  }}
                  disabled={
                    !team.find((p) => p.id === showPlayerInfoPopup.id) &&
                    team.length >= 11
                  }
      ...
      {view === "CONTEST_DETAILS" && activeMatch && activeContestDetails && (
        <ContestDetailsView
          ...
          onSetLivePayouts={async (newPayouts, rankedTeams) => {
            setAppContests((prev) => {
              const updated = prev.map((c) =>
                c.id === activeContestDetails.id ||
                c.name === activeContestDetails.name
            ...
            setActiveContestDetails((prev) =>
              prev
                ? { ...prev, payouts: newPayouts, hasCustomPayouts: true }
                : prev,
            );

            alert(
              "Live Payouts applied! It will be distributed when the match is explicitly marked as Completed in Match Status.",
            );
          }}
          winningPercentage={winningPercentage}
          onBack={() => {
            setActiveContestDetails(null);
            setView("MATCH");
          }}
          balance={balance}
          ...
          onJoin={() => {
            setSelectedContest({
              fee: activeContestDetails.entryFee,
              name: activeContestDetails.name,
            });
            setView("CREATE_TEAM");
          }}
          onParticipantClick={(t) => {
            ...
            setPreviewTeamInfo({
              name: `${t.userName || t.userId} (${t.teamId})`,
              points: t.points,
            });
            setView("TEAM_PREVIEW");
          }}
      ...
      {view === "REWARD" &&
        renderPlaceholder(
          "Rewards",
          <PlayCircle size={60} />,
          "Watch videos, earn coins and unlock exciting premium rewards.",
        )}
      {view === "CHAT" &&
        renderPlaceholder(
          "Community Chat",
          <MessageSquare size={60} />,
          "Discuss match predictions, share your team and chat with friends.",
        )}
      {view === "NOTIFICATIONS" &&
        renderPlaceholder(
          "Notifications",
          <Bell size={60} />,
          "Catch all the latest match updates and lineup announcements here.",
        )}
          ...
          <div className="p-4 flex-1 overflow-y-auto pb-20">
            <div className="bg-app-accent rounded-xl shadow-sm p-5 flex flex-col mb-4 text-white">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <h2 className="text-xl font-bold uppercase">
                    {adminProfileModalUser.name || "User"}
                  </h2>
                  <p className="text-sm opacity-90 mb-2 font-mono truncate max-w-[200px]">
                    ID:{" "}
                    {adminProfileModalUser.numericId ||
                      adminProfileModalUser.id}
                  </p>
                  <div className="flex items-center gap-2">
                    <span className="bg-yellow-500 text-black text-[10px] px-2 py-0.5 rounded flex items-center font-bold">
                      ⚡ Level{" "}
                      {Math.floor(
                        savedTeams.filter(
                          (t) => t.userId === adminProfileModalUser.id,
                        ).length / 10,
                      )}
                    </span>
                    <span className="text-xs font-bold text-white/90">
                  ...
                  <div className="w-full bg-black/30 h-1.5 rounded-full overflow-hidden mt-1">
                    <div
                      className="bg-yellow-500 h-full rounded-full"
                      style={{
                        width: `${(savedTeams.filter((t) => t.userId === adminProfileModalUser.id).length % 10) * 10}%`,
                      }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="bg-app-card rounded-xl flex-1 p-3 flex flex-col items-center justify-center border border-app-border shadow-sm col-span-2">
                <span className="text-3xl font-black text-[#4ADE80]">
                  ₹
                  {Number(adminProfileModalUser.winning || 0).toLocaleString(
                    "en-IN",
                  )}
                </span>
                <span className="text-xs text-app-text-muted mt-1 text-center font-bold uppercase tracking-wider">
                  Total Profit Earned
                </span>
              </div>
            </div>
            ...
            <div className="bg-app-card rounded-xl shadow-sm border border-app-border overflow-hidden p-4 mb-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-app-text-muted font-medium mt-1">
              ...
              <button
                onClick={async () => {
                  ...
                  await setDoc(
                    doc(db, "wallets", adminProfileModalUser.id),
                    { ...adminProfileModalUser, blocked: newBlockedStat },
                    { merge: true },
                  );
                  setAdminProfileModalUser({
                    ...adminProfileModalUser,
                    blocked: newBlockedStat,
                  });
            ...
            <div className="bg-[#13151c] rounded-2xl shadow-lg border border-[#e5c158]/20 p-5 mt-4 relative overflow-hidden backdrop-blur-sm">
              <div className="absolute top-0 right-0 bg-yellow-500 text-black text-[9px] uppercase tracking-widest font-black px-3 py-1 rounded-bl-xl shadow-[0_0_10px_rgba(234,179,8,0.5)]">
                VIP ACCESS
              </div>
              <div className="flex justify-between items-center mb-4 mt-2">
                <div>
                  ...
                  <p className="text-xs text-[#e5c158]/70 font-mono truncate max-w-[150px]">
                    {adminProfileModalUser.email || adminProfileModalUser.id}
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className="font-black text-xl text-[#e5c158] drop-shadow-[0_0_8px_rgba(234,179,8,0.4)]">
                    ₹
                    {(
                      (adminProfileModalUser.deposit || 0) +
                      (adminProfileModalUser.winning || 0) +
                      (adminProfileModalUser.bonus || 0)
                    ).toFixed(2)}
                  </p>
                </div>
              </div>
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      onClick={async () => {
                        const sanitized = adminCustomAmount
                          .toString()
                          .replace(/[^\d.]/g, "");
                        const amt = parseFloat(sanitized);
                        if (!isNaN(amt) && amt > 0) {
                          const newVal =
                            (adminProfileModalUser.deposit || 0) + amt;
                          try {
                            await syncWalletToBackend(
                              db,
                              adminProfileModalUser.id,
                      ...
                      className="bg-blue-500/20 text-blue-400 border border-blue-500/50 py-2.5 rounded-xl text-xs font-bold hover:bg-blue-500/30 transition-colors"
                    >
                      + Deposit
                    </button>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (!isNaN(amt) && amt > 0) {
                          const newVal =
                            (adminProfileModalUser.winning || 0) + amt;
                          try {
                            await syncWalletToBackend(
                              db,
                              adminProfileModalUser.id,
                      ...
                      className="bg-green-500/20 text-green-400 border border-green-500/50 py-2.5 rounded-xl text-xs font-bold hover:bg-green-500/30 transition-colors"
                    >
                      + Winning
                    </button>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (!isNaN(amt) && amt > 0) {
                          const newVal =
                            (adminProfileModalUser.bonus || 0) + amt;
                          try {
                            await syncWalletToBackend(
                              db,
                              adminProfileModalUser.id,
                      ...
                      className="bg-purple-500/20 text-purple-400 border border-purple-500/50 py-2.5 rounded-xl text-xs font-bold hover:bg-purple-500/30 transition-colors"
                    >
                      + Bonus
                    </button>
                  </div>
                      ...
                      if (!isNaN(amt) && amt > 0) {
                        ...
                        try {
                          await syncWalletToBackend(
                            db,
                            adminProfileModalUser.id,
                            { deposit: nDep, winning: nWin, bonus: nBon },
                          );
                ...
                <button
                  onClick={async () => {
                    if (
                      window.confirm(
                        "Are you sure you want to wipe this user's wallet to 0?",
                      )
                    ) {
                      try {
                        await updateDoc(
                        ...
                        console.error(e);
                        alert("Failed to wipe funds");
                      }
                    }
                  }}
                  className="w-full bg-black/40 hover:bg-black/60 text-slate-400 border border-slate-700 py-2.5 rounded-xl text-sm font-bold transition-colors"
                >
                  Wipe Wallet to 0
                </button>
                ...
                <div className="pt-2 border-t border-app-border mt-1 flex justify-between gap-1">
                  <div className="flex-1 bg-black/20 p-2 rounded flex flex-col items-center">
                    <span className="text-[9px] uppercase text-app-text-muted font-bold">
                      Deposit
                    </span>
        ...
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 min-h-[100dvh]">
          <div className="bg-[#13151c] border border-green-500/30 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-400 mb-6 font-medium">
              Instantly add money to your wallet without payment.
            </p>

            <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1 block ml-1">
              Amount (₹)
            </label>
            <input
              type="number"
              ...
              onChange={(e) => setPaymentAmount(e.target.value)}
              placeholder="0"
              className="w-full text-2xl font-black bg-black/50 text-white border border-slate-700 rounded-lg p-4 mb-4 outline-none focus:border-green-500 transition-colors text-center"
            />
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              {[100, 500, 1000, 5000].map((amt) => (
                <button
                  key={amt}
                  onClick={() => setPaymentAmount(amt.toString())}
                  className="bg-slate-800 text-slate-300 font-bold py-2 rounded border border-slate-700 active:bg-slate-700 text-xs text-center transition-colors"
                >
                  +₹{amt}
                </button>
              ))}
            ...
            <button
              onClick={async () => {
                if (
                  user?.id &&
                  window.confirm(
                    "Reset your wallet completely to ₹0? This cannot be undone.",
                  )
                ) {
      ...
      {selectedImageUrl && (
        <div className="fixed inset-0 z-[999] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          <img
            ...
            onError={(e) => {
              e.currentTarget.src =
                "https://placehold.co/400x400?text=Format+Not+Supported";
            }}
            alt="View"
      ...
      {showFairPlayPolicy && (
        <div className="fixed inset-0 z-[150] bg-app-bg text-app-text flex flex-col font-sans overflow-hidden max-w-md mx-auto">
          <header className="p-4 flex items-center gap-4 bg-app-card border-b border-app-border shrink-0">
            <button
              onClick={() => setShowFairPlayPolicy(false)}
          ...
          <div className="flex-1 overflow-y-auto p-5 text-sm space-y-4">
            ...
            <p className="text-app-text-muted">
              Our commitment to Fair Play ensures all users have an equal
              opportunity to win based on their skills and sports knowledge.
            </p>

            <h3 className="font-bold text-app-text text-base">Transparency</h3>
            <p className="text-app-text-muted">
              We provide a reliable platform where match point updates are
              consistent and universally applied across all user teams at
            ...
            <p className="text-app-text-muted">
              Any account found using automated bots, multiple accounts by the
              same individual, or attempting to manipulate our systems will be
              permanently banned and funds will be seized.
            </p>
            ...
            <p className="text-app-text-muted">
              Points are derived from official match scorecards and data
              providers.
            </p>
          </div>
        </div>
      ...
      {showEditProfileModal && (
        <div className="fixed inset-0 z-[150] bg-app-bg text-app-text flex flex-col font-sans overflow-hidden max-w-md mx-auto">
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-6">
            <div>
              <label className="block text-xs text-app-text-muted mb-2 font-bold uppercase tracking-wider">
                Full Name
              </label>
              <input
            ...
            <div>
              ...
              <div className="flex">
                <span className="bg-app-bg border border-app-border border-r-0 rounded-l-xl px-4 py-3 text-app-text-muted text-sm flex items-center justify-center">
                  +91
                </span>
                <input
                  type="tel"
                  ...
                  onChange={(e) =>
                    setEditProfilePhone(e.target.value.replace(/\D/g, ""))
                  }
      ...
      {showHelpSupport && (
        <div className="fixed inset-0 z-[150] bg-app-bg text-app-text flex flex-col font-sans overflow-hidden max-w-md mx-auto">
          ...
          <div className="flex-1 overflow-y-auto p-5 flex flex-col items-center">
            <Headphones
              size={60}
              className="text-app-accent mb-6 mt-4 opacity-80"
            />
            <p className="text-center text-sm text-app-text-muted mb-8 max-w-[250px]">
              If you have any issues with deposits, withdrawals, or gameplay,
              ...
              {helpSettings.email ? (
                <div className="flex items-center gap-3 bg-app-card p-3 rounded-lg border border-app-border">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    <span className="text-[10px] uppercase font-bold text-app-text-muted tracking-wider">
                      Email Support
                    </span>
                    <a
                      href={`mailto:${helpSettings.email}`}
                      className="font-medium text-sm text-app-text truncate hover:text-blue-500 transition-colors"
                    >
                      {helpSettings.email}
                    </a>
                  </div>
              ...
              {helpSettings.telegram ? (
                <div className="flex items-center gap-3 bg-app-card p-3 rounded-lg border border-app-border">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <a
                      href={helpSettings.telegram}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-sm truncate text-sky-500 transition-colors line-clamp-1"
                    >
                      {helpSettings.telegram}
                    </a>
                  </div>
              ...
              {helpSettings.whatsapp ? (
                <div className="flex items-center gap-3 bg-app-card p-3 rounded-lg border border-app-border">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <a
                      href={helpSettings.whatsapp}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-sm truncate text-green-500 transition-colors line-clamp-1"
                    >
                      {helpSettings.whatsapp}
                    </a>
                  </div>
            ...
            {helpSettings.barcode ? (
              <div className="mt-8 flex flex-col items-center">
                <span className="text-[10px] text-app-text-muted font-bold uppercase tracking-widest mb-3">
                  Scan to connect
                </span>
                <div className="bg-white p-2 rounded-xl shadow-lg border border-app-border">
                  <img
                    src={helpSettings.barcode}
                    alt="Barcode"
              ...
              !helpSettings.barcode && (
                <div className="text-center p-6 bg-app-card-inner rounded-xl border border-app-border w-full">
                  <p className="text-sm font-bold text-app-text-muted">
                    Support contact info will be added shortly.
                  </p>
                </div>
              )}
          </div>
        ...
        <div className="fixed inset-0 z-[150] bg-app-bg text-app-text flex flex-col font-sans overflow-hidden max-w-md mx-auto">
          ...
          <div className="flex-1 overflow-y-auto p-5 pb-20">
            <div className="bg-gradient-to-br from-[#e5c158]/20 to-orange-500/10 border border-[#e5c158]/50 rounded-2xl p-6 flex flex-col items-center text-center shadow-lg relative overflow-hidden">
              ...
              <Users
                size={60}
                className="text-[#e5c158] drop-shadow-[0_0_15px_rgba(229,193,88,0.6)] mb-4 z-10"
              />
              <h2 className="text-2xl font-black text-app-text z-10 mb-2">
                Invite Friends, <br />
                Earn <span className="text-[#e5c158]">₹100</span>
              </h2>
              <p className="text-sm text-app-text-muted z-10 font-medium">
                When your friend signs up and plays their first cash contest,
                you get ₹100 Bonus!
              </p>
            </div>
            ...
            <div className="mt-8 bg-app-card rounded-xl border border-app-border overflow-hidden">
              ...
              <div className="p-4 flex flex-col gap-3">
                <div className="bg-app-bg border border-app-border rounded-lg p-4 flex items-center justify-between">
                  <span className="font-mono text-lg font-bold tracking-widest text-[#e5c158]">
                    {user?.id?.substring(0, 6).toUpperCase() || "GUEST1"}
                  </span>
            ...
            <div className="mt-8 px-2">
              <h3 className="font-bold text-app-text-muted text-sm uppercase tracking-wider mb-4">
                How it works
              </h3>
              <div className="flex flex-col gap-6 relative before:absolute before:left-3 before:top-4 before:bottom-4 before:w-px before:bg-app-border">
                <div className="flex gap-4 relative">
                  <div className="w-6 h-6 rounded-full bg-app-card border-2 border-[#e5c158] text-[#e5c158] flex items-center justify-center text-xs font-bold font-mono z-10 shrink-0">
                    1
                  </div>
                ...
                <div className="flex gap-4 relative">
                  <div className="w-6 h-6 rounded-full bg-[#e5c158] text-black flex items-center justify-center text-xs font-bold font-mono z-10 shrink-0">
      ...
      {showHowToPlay && (
        <div className="fixed inset-0 z-[150] bg-app-bg text-app-text flex flex-col font-sans overflow-hidden max-w-md mx-auto">
          ...
          <div className="flex-1 overflow-y-auto p-5 text-sm space-y-6">
            <div>
              <h3 className="font-bold text-app-text text-base mb-2">
                1. Select a Match
              </h3>
              <p className="text-app-text-muted">
                Choose an upcoming match from the home screen.
              </p>
            </div>
            <div>
              <h3 className="font-bold text-app-text text-base mb-2">
              ...
              <p className="text-app-text-muted">
                Use your sports knowledge to build a winning fantasy team with
                100 credits.
              </p>
            </div>
            <div>
              ...
              <p className="text-app-text-muted">
                Join cash or practice contests to compete with other users.
              </p>
            </div>
            <div>
              <h3 className="font-bold text-app-text text-base mb-2">
              ...
              <p className="text-app-text-muted">
                Watch your team score points based on real-world performance!
                Top the leaderboard and win real money.
              </p>
            </div>

            <div className="bg-app-card border border-app-border rounded-xl p-4 mt-4">
              <h4 className="font-bold text-[#e5c158] mb-3 text-center uppercase tracking-wider text-xs">
                Standard Scoring Rules
              </h4>
              <ul className="space-y-2 text-xs">
                <li className="flex justify-between items-center">
                  <span className="text-app-text-muted">Run</span>{" "}
                  <span className="font-bold text-app-text">+1</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-app-text-muted">Boundary Bonus</span>{" "}
                  <span className="font-bold text-app-text">+1</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-app-text-muted">Six Bonus</span>{" "}
                  <span className="font-bold text-app-text">+2</span>
                </li>
                <li className="flex justify-between items-center border-t border-app-border/50 pt-2">
                  <span className="text-app-text-muted">Wicket</span>{" "}
                  <span className="font-bold text-app-text">+25</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-app-text-muted">LBW/Bowled Bonus</span>{" "}
                  <span className="font-bold text-app-text">+8</span>
                </li>
                <li className="flex justify-between items-center border-t border-app-border/50 pt-2">
                  <span className="text-app-text-muted">Catch</span>{" "}
                  <span className="font-bold text-app-text">+8</span>
                </li>
                <li className="flex justify-between items-center">
                  <span className="text-slate-400">Stump/Run Out</span>{" "}
      ...
      {adminEditPlayerBreakup && (
        <div
          className="fixed inset-[-20px] bg-black/60 z-[300] flex flex-col pt-10 pb-4 px-4 overflow-hidden"
          onClick={() => {
            setAdminEditPlayerBreakup(null);
        ...
        >
          ...
          >
            <div className="bg-[#0f0f0f] border-b border-[#e5c158]/20 text-[#e5c158] p-4 flex items-center gap-4 shrink-0 shadow-sm">
              ...
              <h2 className="text-xl font-black font-sans uppercase tracking-wider flex-1 truncate">
                {adminEditPlayerBreakup.name}
              </h2>
            </div>
            ...
            <div className="flex-1 overflow-y-auto text-slate-200 hide-scrollbar pb-20 bg-[#13151c]">
              <div className="w-full text-sm">
                ...
                {POINT_EVENTS.map((evt) => {
                  const evtData = adminEditPlayerBreakup.breakup?.[evt] || {
                    points: 0,
                    actual: 0,
                  };
                  return (
                    <div
                      key={evt}
                      className="flex flex-col border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors"
                    >
                      <div className="flex px-4 py-3 items-center">
                        <div className="flex-1 font-semibold text-slate-300 text-xs truncate mr-2">
                          {evt}
                        </div>
                        <div className="w-16 text-center font-bold text-slate-100">
                          {evtData.points}
                        </div>
                        <div className="w-16 text-right text-slate-400 font-mono text-xs truncate max-w-[60px] overflow-hidden">
                          {evtData.actual}
                        </div>
                          ...
                          <button
                            onClick={() =>
                              setAdminEditingEvent({
                                event: evt,
                                actual: String(evtData.actual),
                                points: String(evtData.points),
                              })
                            }
                        ...
                        <div className="bg-[#090b10] p-3 mx-3 mb-3 rounded-xl border border-[#e5c158]/40 flex flex-col gap-3 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
                          <div className="flex gap-3">
                            <div className="flex-1 flex flex-col gap-1.5 text-[9px] text-[#e5c158]/70 font-black uppercase tracking-widest">
                              ...
                              <input
                                type="text"
                                value={adminEditingEvent.actual}
                                onChange={(e) =>
                                  setAdminEditingEvent({
                                    ...adminEditingEvent,
                                ...
                                className="bg-black border border-slate-700 rounded-lg px-3 py-2 text-sm font-mono text-white outline-none focus:border-[#e5c158] focus:shadow-[0_0_10px_rgba(229,193,88,0.2)] transition-all w-full"
                              />
                            </div>
                            <div className="flex-1 flex flex-col gap-1.5 text-[9px] text-[#e5c158]/70 font-black uppercase tracking-widest">
                              Points
                              <input
                                type="number"
                                value={adminEditingEvent.points}
                                onChange={(e) =>
                                  setAdminEditingEvent({
                                    ...adminEditingEvent,
                          ...
                          <div className="flex gap-3 mt-1">
                            <button
                              onClick={() => setAdminEditingEvent(null)}
                              className="flex-1 py-3 border border-slate-700 text-slate-400 rounded-lg text-xs font-bold uppercase transition-colors hover:bg-slate-800 hover:text-white"
                            >
                              Cancel
                            </button>
                            <button
                              onClick={async () => {
                                const pts =
                                  Number(adminEditingEvent.points) || 0;
                                const actual = adminEditingEvent.actual;
                                const newBreakup = {
                                  ...adminEditPlayerBreakup.breakup,
                                  [evt]: { points: pts, actual: actual },
                                };
                                ...
                                try {
                                  ...
                                  setAdminEditPlayerBreakup((prev) =>
                                    prev
                                      ? {
                                          ...prev,
                                          breakup: newBreakup,
                              ...
                              className="flex-1 py-3 bg-[#e5c158] text-black rounded-lg text-[10px] font-black uppercase tracking-widest shadow-[0_0_15px_rgba(229,193,88,0.3)] transition-all hover:bg-yellow-400 hover:shadow-[0_0_20px_rgba(229,193,88,0.5)] active:scale-95"
                            >
                              Save
                            </button>
                          </div>
                ...
                <div className="flex px-4 py-5 bg-black/60 items-center font-black font-sans text-xl border-t border-[#e5c158]/50 mt-4 shadow-[0_-5px_15px_rgba(0,0,0,0.5)] text-[#e5c158]">
                  <div className="flex-1 uppercase tracking-widest text-sm">
                    Total Points
                  </div>
                  <div className="w-16 text-right pr-4 text-2xl">
                    {adminEditPlayerBreakup.livePoints ??
                      adminEditPlayerBreakup.points}
                  </div>
                </div>
              </div>
            </div>
