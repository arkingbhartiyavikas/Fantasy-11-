...
const targetCode = `                         const newContestIds = [];
...
const newCode = `                         const generatedContests = [];
                         const newContestIds = [];
                         for (let spots = 2; spots <= 5; spots++) {
                           for (let pool = 1; pool <= maxPool; pool++) {
                             const entryFee = Math.ceil((pool / (1 - margin / 100)) / spots);
                         ...
                         const updatedContests = [...appContests, ...generatedContests];
                         setAppContests(updatedContests);
                         await syncCategoryToCloud("contests", updatedContests, 20);`;

if(file.includes(targetCode)) {
  file = file.replace(targetCode, newCode);
  fs.writeFileSync('src/App.tsx', file);
  console.log("REPLACED CONTEST GEN");
} else {
